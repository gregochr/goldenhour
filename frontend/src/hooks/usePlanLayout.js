 ...
 *
 * <p>Default is {@link PLAN_V1} and stays there until the pre-pilot sweep passes; flipping the
 * default is the last step of the build, not the first.
 *
 * <p>An unrecognised stored value resolves to v1 rather than throwing or persisting itself. The
 * failure it guards is narrow but real: a half-written key, or a value from a future build the
 * user has since rolled back, would otherwise render neither layout.
 *
 * @returns {[string, function]} `[layout, setLayout]` — layout is always one of {@link VALID}
 */
export default function usePlanLayout() {
  const [stored, setStored] = useLocalStorageState(PLAN_LAYOUT_KEY, PLAN_V1);
  const layout = VALID.includes(stored) ? stored : PLAN_V1;
  const setLayout = (next) => setStored(VALID.includes(next) ? next : PLAN_V1);
