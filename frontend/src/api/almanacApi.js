 ...
 * entry, it evicts the first one, and then every request from either caller rebuilds all five
...
/**
 * Fetches the "Coming up" almanac feed — dated events over the next {@link ALMANAC_DAYS} days.
 *
 * <p>Entries are <em>spans</em> ({@code startDate}/{@code endDate}, both inclusive), ascending by
 * start date then by span length. They may start before today or end past the horizon: a tide run
 * or a supermoon window that straddles an edge is reported with its true extent rather than
 * clipped, so "this began yesterday" does not render as "starts today".
 *
 ...
 * for a LITE user, and it always returns a list — empty at worst, never a 204. A failure here is a
 * real failure and the caller renders it.
 *
 ...
 *
 * @returns {Promise<Array>} the entries, ascending; `[]` when every source failed
 */
