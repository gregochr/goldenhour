...
export default function InfoTip({ text, className = '', position = 'above' }) {
  ...
  return (
    <span ref={ref} className={`relative inline-flex items-center ${className}`}>
      ...
      {open && (
        <span
          className={`absolute left-0 w-max max-w-[min(24rem,calc(100vw-2rem))] px-2.5 py-1.5 text-xs rounded shadow-lg whitespace-pre-line z-50 bg-plex-surface text-plex-text border border-plex-border ${position === 'below' ? 'top-full mt-1.5' : 'bottom-full mb-1.5'}`}
