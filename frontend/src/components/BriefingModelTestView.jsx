...
/**
 * Briefing model comparison test view — runs Haiku/Sonnet/Opus with the same
 * briefing rollup and shows side-by-side pick comparison with agreement highlighting.
 */
export default function BriefingModelTestView() {
  ...
  const handleRunTest = () => {
    openDialog({
      ...
      message: 'This will call 3 Claude models (Haiku, Sonnet, Opus) with the current briefing data. This incurs API costs.',
  ...
  // Agreement highlighting: for each rank, check if models agree on event+region
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-3">
                {MODELS.map((model) => {
                  ...
                  if (!r) return (
                    <div key={model} className="rounded-lg bg-plex-card-bg p-3">
                  ...
                  return (
                    <div key={model} className="rounded-lg bg-plex-card-bg p-3" data-testid={`metric-${model}`}>
