...
**Aurora photography**: NOAA SWPC polling (`AuroraPollingJob`, every 5 min, day and night — see the ⚠️ below) via `NoaaSwpcClient` (Kp, forecast, OVATION, solar wind Bz, G-scale alerts; per-endpoint caching) | `AuroraStateCache` FSM (**two** states: IDLE ⇄ ACTIVE — the alert *level* QUIET/MINOR/MODERATE/STRONG is a separate field, not a state) | `WeatherTriageService` (3-point northward transect triage, any-clear-hour pass at < 75% overcast) | `ClaudeAuroraInterpreter` (single `claude-haiku-4-5` call for all viable locations; returns 1–5★ + summary + detail) | `AuroraOrchestrator` (orchestrates full pipeline; Kp + OVATION dual signal) — ⚠️ **every poll evaluates the state machine at most once, and reads tonight through one figure, `maxKpRestOfTonight`** (blocks still ahead in tonight's dark window, the running one included, ended ones never). A daylight poll's level is that figure's; a night poll's is the higher of it and the Kp for now, through one `AlertLevel.fromKp(kp, kp-threshold)`, so the evening keeps a heads-up for a small-hours peak. The alert is attributed to the forecast (planning wording, tonight's window) when the forecast alone reaches the level, and to `REALTIME` only when the conditions now go beyond it. Until 2026-09-14 a night poll evaluated twice — a lookahead that kept finished blocks, then a real-time path that looked six hours ahead. When the lookahead reached an alert level and the real-time path did not, every poll NOTIFIED, paid for triage and (with a clear location) a Claude call, then CLEARED — up to ~12 an hour; when the real-time path came out higher, the poll that raised the alert paid twice. **Do not split the night poll back into two evaluations, or give anything its own horizon.** One NOAA snapshot and one clock reading per poll decide the night; "the Kp for now" is NOAA's figure for the most recently *completed* block — its published reading once out, and until then the higher of the latest reading and NOAA's estimate for the block (a reading lands only after its block ends and sits in a 15-minute cache). ⚠️ The estimate may raise it, never lower it: lowering would CLEAR a storm NOAA under-estimated until its reading landed, then pay to NOTIFY it again. A daylight poll fetches the full snapshot only when `AuroraStateCache.wouldNotify` says a scoring is coming, and before the state machine moves. `deriveAlertLevel()` is only the (seeded-PAUSED) aurora batch job's rule now, keeps the 6-hour `kp-forecast-lookahead-hours`, and writes scores whatever the state machine's state, so un-pausing it needs reconciling. The admin `POST /api/aurora/admin/run` runs the scheduled cycle itself through `AuroraPollingJob.runCycleIfIdle()` — one guard, 409 while a cycle runs; the orchestrator's poll methods are package-private so nothing else can. `AuroraPollingCycleTest` replays whole nights. Still open, and pre-existing: no de-escalation hysteresis (`kp-clear-threshold`/`ovation-clear-threshold` are unread), nothing clears an alert in daylight, and an alert's first scoring stands until it clears or escalates | `MetOfficeSpaceWeatherScraper` (Jsoup scraper, 60 min refresh) | Bortle enrichment via lightpollutionmap.info (`LightPollutionClient`, `BortleEnrichmentService`, sb_2025 dataset with SQM conversion) | Map tab aurora mode (selected via `WindowControl`'s night rows; `MapCallout` shows the rating on the tab, `ForecastTypeSelector` + `MarkerPopupContent`'s aurora section on the frozen Plan-tab overlay) | `AuroraBanner` React component (shows Kp reading) | Aurora viewline endpoint (OVATION nowcast southernmost visibility boundary, `AuroraViewlineOverlay` with colour-coded zones) | Alert levels: QUIET/MINOR/MODERATE/STRONG
