...
**Aurora photography**: NOAA SWPC polling (`AuroraPollingJob`, 5 min: the forecast lookahead alone by day, the lookahead then the real-time path after dark) via `NoaaSwpcClient` (Kp, forecast, OVATION, solar wind Bz, G-scale alerts; per-endpoint caching) | `AuroraStateCache` FSM (**two** states: IDLE ⇄ ACTIVE — the alert *level* QUIET/MINOR/MODERATE/STRONG is a separate field, not a state) | `WeatherTriageService` (3-point northward transect triage, any-clear-hour pass at < 75% overcast) | `ClaudeAuroraInterpreter` (single `claude-haiku-4-5` call for all viable locations; returns 1–5★ + summary + detail) | `AuroraOrchestrator` (orchestrates full pipeline; Kp + OVATION dual signal) — ⚠️ **both polling paths read tonight through one figure, `maxKpRestOfTonight`** (blocks still ahead in tonight's dark window, the running one included, ended ones never), and the real-time level is the higher of that and the Kp for now, through one `AlertLevel.fromKp(kp, kp-threshold)`, so after dark it can never CLEAR what the lookahead NOTIFIED. Until 2026-09-14 the lookahead kept finished blocks and the real-time path looked six hours ahead; whenever they disagreed, every poll NOTIFIED, paid for a Claude call and CLEARED (~12 an hour). **Do not give either path a horizon of its own again.** One NOAA snapshot and one instant per night poll; "the Kp for now" counts the block that has just ended, because a reading lands only after its block and sits in a 15-minute cache. `deriveAlertLevel()` is only the (seeded-PAUSED) aurora batch job's gate now, and keeps the 6-hour `kp-forecast-lookahead-hours`. The admin `POST /api/aurora/admin/run` runs the scheduled cycle itself through `AuroraPollingJob.runCycleIfIdle()` — one guard, 409 while a cycle runs. `AuroraPollingCycleTest` replays whole nights; `kp-clear-threshold`/`ovation-clear-threshold` are still unread (no de-escalation hysteresis exists) | `MetOfficeSpaceWeatherScraper` (Jsoup scraper, 60 min refresh) | Bortle enrichment via lightpollutionmap.info (`LightPollutionClient`, `BortleEnrichmentService`, sb_2025 dataset with SQM conversion) | Map tab aurora mode (selected via `WindowControl`'s night rows; `MapCallout` shows the rating on the tab, `ForecastTypeSelector` + `MarkerPopupContent`'s aurora section on the frozen Plan-tab overlay) | `AuroraBanner` React component (shows Kp reading) | Aurora viewline endpoint (OVATION nowcast southernmost visibility boundary, `AuroraViewlineOverlay` with colour-coded zones) | Alert levels: QUIET/MINOR/MODERATE/STRONG
