...
### Aurora (PRO/ADMIN; `/admin/*` and `/forecast/preview|run` ADMIN-only)
`GET /api/aurora/status` | `GET /api/aurora/locations` | `GET /api/aurora/viewline` | `GET /api/aurora/viewline/forecast` (Kp-to-latitude line for a forecast-triggered alert, vs `/viewline`'s live OVATION nowcast — all four PRO/ADMIN, `AuroraController` is class-level `@PreAuthorize` PRO_USER/ADMIN, so a LITE account gets 403) | `GET /api/aurora/forecast/results` and `GET /api/aurora/forecast/results/available-dates` (PRO/ADMIN, `AuroraForecastController` — the map's read path) | `GET /api/aurora/forecast/preview` (cheap 3-night Kp preview from cached NOAA data, no Claude call) and `POST /api/aurora/forecast/run` (runs the Claude pipeline for the selected nights and stores results; billable) — **both ADMIN**, see the ⚠️ below | `POST /api/aurora/admin/enrich-bortle` (ADMIN) | `POST /api/aurora/admin/run` (ADMIN — runs the scheduled polling cycle now and waits for it; 409 while a cycle is running; returns `{status, dark, level, action, trigger, held}`) | `POST /api/aurora/admin/reset` (ADMIN) | `POST /api/aurora/admin/simulate` (ADMIN — injects fake NOAA data for testing, no Claude call) | `POST /api/aurora/admin/simulate/clear` (ADMIN — today, an alias for `/admin/reset` under different wording: both call `AuroraStateCache.reset()`, unconditionally wiping the whole state machine — a real alert included — and always answering 200)
