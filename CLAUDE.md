...
**Locations**: Multi-location, rendered on the shared `MapView` (Leaflet/OSM — both the Map tab and the Plan-tab overlay mount this one component, below) | Metadata: `SolarEventType`, `TideType` (H/M/L multi-select), `LocationType` (LANDSCAPE/WILDLIFE/SEASCAPE/WATERFALL) | Regions (geographic grouping) | Sunrise/sunset azimuth lines for the selected location — the tab's Heat view draws these too, same as the overlay; only the tab's Pins mode drops them (D-9) | Marker clustering (`react-leaflet-cluster`) — the same `MarkerClusterGroup` mounts on the tab as well, not overlay-only, but ⚠️ **the tab never SHOWS it**: while `MapHeatLayer` is mounted it holds the marker panes at zero **unconditionally**, in both of the tab's views (Pins draws `PinsLayer`'s dots, Heat draws `MapLabels`' chips, and one location renders in one place). The medallions come back only when that layer **unmounts** — which is the aurora window (`heatOffered` is `Boolean(heat?.enabled) && !isAuroraMode`) and a catalogue with nothing scored at all (`heat.enabled` is `heatSpots.length > 0`, so a fresh install shows the pre-v2 map), plus a tab change or logout. ⚠️ **Two conditions on that hide were tried and both were wrong; do not reintroduce either.** A **zoom** condition (`fadeAt().markers` fading them back past the handover band) resurrected the exact medallion vocabulary the tab replaced, doubled under the chips naming the same spots — the design bundle's Heat view has no markers at any zoom, and `fadeAt().markers` is now only the Legend's `Field → Handing over → Locations` progress fraction, painting nothing. A **point-count** condition (`points.length > 0`, meant as "an unscored window has nothing but markers") was worse: the map is never blank (`MapLabels` mounts with no scoring gate and chips unrated spots grey), the restored pane is often *emptier* than the chips (`visibleLocations` drops unrated non-wildlife locations unless `showUnrated`, default false), and an empty point set is **not** "nothing is scored" — it is also a D-13 filler row, a dark-sky-filtered window, and a served window with an empty `pointsByKey` entry, all of which put the whole medallion set back under the chips. `windowUnscored`'s own javadoc already states the rule: the served `bestRating`, never a point count | Star rating + location type filters | Emoji chip UI for metadata | Editable lat/lon

**Map tab (v2)**: `docs/engineering/map-tab-v2-plan.md`, P1–P12 merged. One chronological window control (`WindowControl` — solar EV rows read the served `bestRating`; astro/aurora night rows carry a served night window and a licensed client max, `mapEvents.bestOfNight`) replaces the old date strip, event-type pills and in-map `<select>`. Heat field (bloom, land-clipped, coastline stroke) is the default view, with a Pins mode toggle (`PinsLayer` — one dot per location, the honest comparison) and an expandable `MapLegendPanel` (ramp, whole-star labels, reach-rings toggle, confidence note). Filters live in `FiltersPopover` (min rating, subject chips, drive-from-origin, dark-sky toggle, scope), not an inline drawer. Selection is a greedy-labelled (`MapLabels`) anchored `MapCallout` (ring + card), not a Leaflet popup. `RegionsJump` ("◎ Regions ▾") is a nearest-measured-drive-first jump list. Phone layout moves Regions/Heat-Pins/Filters into a bottom bar. **The Plan-tab overlay is a deliberately frozen, separate pre-v2 surface** — same `MapView` component, `overlayMode` gate — keeping the marker popup (`MarkerPopupContent`), medallions and `ForecastTypeSelector` as overlay-exclusive pieces; `MarkerClusterGroup` is mounted on the tab too but never visible there (see the Locations bullet — hidden unconditionally in both views, the layer's own unmount being the only way back); the selected location's azimuth lines ARE drawn by the tab's Heat view, and only its Pins mode drops them (D-9). Convergence onto the callout is an open owner decision (plan §4.9, O-6).
