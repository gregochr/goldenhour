...
### Aurora (PRO_USER/ADMIN / ADMIN for writes)
`GET /api/aurora/status` (PRO_USER/ADMIN) | `GET /api/aurora/locations` (PRO_USER/ADMIN) | `GET /api/aurora/viewline` (PRO_USER/ADMIN — `AuroraController` is `@PreAuthorize` PRO_USER/ADMIN, so a LITE account gets 403) | `POST /api/aurora/admin/enrich-bortle` (ADMIN) | `POST /api/aurora/admin/run` (ADMIN — runs the scheduled polling cycle now and waits for it; 409 while a cycle is running; returns `{status, dark, level, action, trigger, held}`) | `POST /api/aurora/admin/reset` (ADMIN)
