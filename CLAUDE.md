...
- **Frontend**: React 19 + Vite + Tailwind — Map tab (Leaflet, v2: heat field default, chronological window control), Plan matrix (day × event heatmap, popup and location sheet), Coming up feed, Operations tab, JWT-authenticated. Outcome recording is API-only (no UI since 2026-02-27 — `docs/engineering/v1-retirement-plan.md` §8).
...
**Aurora photography**: NOAA SWPC polling (`AuroraPollingJob`, every 5 min, day and night — see the ⚠️ below) via `NoaaSwpcClient` (Kp, forecast, OVATION, solar wind Bz, G-scale alerts; per-endpoint caching) | `AuroraStateCache` FSM (**two** states: IDLE ⇄ ACTIVE — the alert *level* QUIET/MINOR/MODERATE/STRONG is a separate field, not a state) | `WeatherTriageService` (3-point northward transect triage, any-clear-hour pass at < 75% overcast) | `ClaudeAuroraInterpreter` (single `claude-haiku-4-5` call for all viable locations; returns 1–5★ + summary + detail) | `AuroraOrchestrator` (orchestrates full pipeline; Kp + OVATION dual signal) — ⚠️ **every poll evaluates the state machine at most once, and reads tonight through one figure, `maxKpRestOfTonight`** (blocks still ahead in tonight's dark window, the running one included, ended ones never). A daylight poll's level is that figure's; a night poll's is the higher of it and the Kp for now, through one `AlertLevel.fromKp(kp, kp-threshold)`, then raised to MODERATE when OVATION reaches `ovation-probability-threshold` — so the evening keeps a heads-up for a small-hours peak. The alert is attributed to the forecast (planning wording, tonight's window) when the forecast alone reaches the level, and to `REALTIME` only when the conditions now go beyond it. Until 2026-09-14 a night poll could evaluate twice — a lookahead that kept finished blocks (whenever tonight's forecast reached `kp-threshold`), then a real-time path that looked six hours ahead. When the lookahead reached an alert level and the real-time path did not, every poll NOTIFIED, paid for triage and (with a clear location) a Claude call, then CLEARED — up to ~12 an hour; when the real-time path came out higher, the poll that raised the alert paid twice. **Do not split the night poll back into two evaluations, and do not give either poll a horizon of its own** — `deriveAlertLevel()` is the one deliberate exception (below). One NOAA snapshot and one clock reading decide each night poll; "the Kp for now" is NOAA's figure for the most recently *completed* block — its published reading once out, NOAA's estimate for it until then (a reading lands only after its block ends and sits in a 15-minute cache). ⚠️ **A night poll does not end an alert on the estimate for the block just ended while that block's reading is due**: for up to an hour after that block ends (`READING_EXPECTED_WITHIN`), a night poll that would CLEAR holds instead (`AuroraStateCache.wouldClear`; the outcome is `held`, action NONE), so a block NOAA under-estimated is not cleared and then re-bought when its reading lands. The hold has three limits, all deliberate: a reading more than an hour late means a late or stale feed, and the estimate ends the alert; the hold lasts no longer than the night — if dawn comes first, the first daylight poll makes the CLEAR the night deferred (its one evaluation, reading nothing), because after dawn no poll acts on the Kp for now and nothing else would end that alert before the next dusk; and it covers only the block that has ended, so an alert that falls mid-block on the running block's low estimate is still cleared, and bought again if that block is published higher. Do not "fix" that by flooring the Kp for now at the latest reading — it was tried in review and reverted: it let a reading three hours old stand for "now", so after a restart or at a dusk just past a boundary it paid for an alert on a block that had ended hours before, which the next reading cleared minutes later, and it held alerts through a stale readings feed. A daylight poll fetches the full snapshot only when `AuroraStateCache.wouldNotify` says a scoring is coming, and before the state machine moves. `deriveAlertLevel()` is only the (seeded-PAUSED) aurora batch job's rule now and keeps the 6-hour `kp-forecast-lookahead-hours`; the batch it gates writes its results through `AuroraResultHandler` whatever the state machine's state, so un-pausing it needs reconciling. The admin `POST /api/aurora/admin/run` runs the scheduled cycle itself through `AuroraPollingJob.runCycleIfIdle()` — one guard, 409 while a cycle runs; the orchestrator's poll methods are package-private, so nothing outside the aurora package can call them, and inside it only the job does. `AuroraPollingCycleTest` replays whole nights. Still open: no de-escalation hysteresis (`kp-clear-threshold`/`ovation-clear-threshold` are unread); no daylight poll ends an alert on its own reading of tonight (only the admin reset and simulate/clear do, and the first daylight poll's settling of a hold), so an alert standing at dawn for any other reason stands until the next dusk; an alert's first scoring stands until it clears or escalates; and an alert raised by NOAA's figure for the running block is worded as a forecast, though that figure is NOAA's estimate of now | `MetOfficeSpaceWeatherScraper` (Jsoup scraper, 60 min refresh) | Bortle enrichment via lightpollutionmap.info (`LightPollutionClient`, `BortleEnrichmentService`, sb_2025 dataset with SQM conversion) | Map tab aurora mode (selected via `WindowControl`'s night rows; `MapCallout` shows the rating on the tab, `ForecastTypeSelector` + `MarkerPopupContent`'s aurora section on the frozen Plan-tab overlay) | `AuroraBanner` React component (shows Kp reading) | Aurora viewline endpoint (OVATION nowcast southernmost visibility boundary, `AuroraViewlineOverlay` with colour-coded zones) | Alert levels: QUIET/MINOR/MODERATE/STRONG | Admin simulation (`POST /api/aurora/admin/simulate`, `AuroraStateCache.activateSimulation`) is isolated from every user-facing surface: `AuroraStateCache.isSimulated()` gates hot topics and the best-bet advisor's prompt, and a forecast run made against it persists with `aurora_forecast_result.simulated = true` (V154), filtered out of every read `AuroraForecastResultRepository` exposes — see the Aurora API section

**Daily Briefing / PhotoCast Planner**: "Go or movie night?" pre-flight check | `BriefingService.refreshBriefing()` runs as the pipeline's BRIEFING phase at the tail of each batch cycle (V103 retired the 04:00/14:00/22:00 `daily_briefing` cron — its target stays registered, dormant, as a revert path) | **One build at a time**: a `ReentrantLock` — the pipeline's `refreshBriefing()` *waits* for a running build and then runs its own (its build is the first to see its cycle's batch results, and `persistPicksForCycle` reads the cache straight after), while the admin `POST /api/briefing/run` takes `refreshBriefingIfIdle()` and answers 409 | Open-Meteo weather + DB tide lookup for all enabled colour locations | Per-slot verdict (GO/MARGINAL/STANDDOWN) based on low/mid cloud, precipitation, visibility, humidity thresholds + building trend checks | Region rollup via majority vote | Lunar-driven tide classification (spring/king via `TideClassificationService`) | `AtomicReference` in-memory cache + `daily_briefing_cache` DB persistence | Served to the Plan tab's `HeatmapGrid` — event-based columns (next 6 solar events), sunrise/sunset sub-columns, aurora grid integration (the "quality slider" component itself was orphaned at `b4b96d4b` and deleted in v1 retirement D2; its gating machinery — `qualityTier`/`isCellVisible`, pinned to always-show since the orphaning — was deleted with it in D3, so every cell is visible unconditionally now) | `BriefingBestBetAdvisor` still runs at build time, but the `/api/briefing` `bestBets`/`bestBetStatus`/`bestBetsWithdrawn`/`bestBetModel` fields it populates have **no frontend renderer** — the advisor itself keeps three live admin consumers (`BriefingModelTestService` → `BriefingModelTestView`, `PipelineRunPickEntity` → `PipelineRunsView`'s picks, `AdvisorReplayController`); see `docs/engineering/v1-retirement-plan.md` §8.1 | `DailyBriefingResponse.stale`/`partialFailure`/`failedLocationCount` also lost their only client reader in v1 retirement D2 — v2 shows the forecast's age only, a v2 "this briefing is partial" surface is a product call (§8.4); `auroraTonight`/`auroraTomorrow` are now **write-only** — their only reader was `WindowFirstDoors.jsx`'s `HotTopicStrip` panel, deleted in the Coming up redesign's P6 (`docs/engineering/coming-up-plan.md` D7); the backend fields stay (an owner call, the same treatment v1 retirement D2 gave the fields above) | Briefing evaluation (`BriefingEvaluationService` + `BriefingEvaluationController`, `GET /scores` + `DELETE /cache` — the SSE endpoint is gone) with drill-down scoring per region | Briefing model comparison test: `BriefingModelTestService` calls Haiku/Sonnet/Opus with same rollup, persists to `briefing_model_test_run/result` tables, `BriefingModelTestView` with agreement highlighting
...
**Two client-derived tide marks** (`docs/engineering/tide-plan-card-plan.md`) ride those same slots:
the best-reachable line's named spot wears a wave glyph when its water is the water it wants
(`card.bestReach.tideAligned`, silent on a miss or an inland pick — the pick is chosen on score, tide
marks nothing else there), and the topics line carries a client-derived `N on tide` chip, emphasised
`best of N` on the run's best live window, wherever half the reachable coast is served
(`card.tideFit`'s gate); below that gate the card says nothing about tide at all — no greyed chip, no
zero, no glyph on a miss. Clicking a card opens **one popup** (`WindowSheetDialog`) holding everything the deleted accordion
...
chronology cover what the door used to hold, and `WindowFirstDoors`' single remaining child goes
full-width with no CSS change (`.wf-doors` is flex, `.wf-door` is `flex: 1`). ⚠️ **At most one dialog may claim
...
field's door is also seeded into the field's label placer as a `target` obstacle, so it never covers
a chip). A new route out of the plan has to do the same; nothing enforces it structurally.
⚠️ **Since 2026-09-05 the sheet has a SECOND host: the Map tab.** The callout's `Four days here ›`
opens `LocationFourDaySheet` over the map without moving the tab (an owner ask — back out of the
...
**Optimisation strategies**: **two**, toggleable per forecast run type on the Run Config screen — `SENTINEL_SAMPLING` (one parameter, the sentinel threshold, default 2) and `TIDE_ALIGNMENT` (adds the tide check to triage for seascapes, standing a slot down without a Claude call; weather triage runs whether it is on or off) | ⚠️ **Both reach only hand-started runs** — the synchronous engine, via the five `ForecastController` endpoints that reach it (Very-Short-Term, Short-Term and Long-Term on Job Runs, and Run Forecast on a map location). Scheduled batches, overnight and intraday, read neither: their `fetchWeatherAndTriage` calls pass `tideAlignmentEnabled = false` and they have no sentinel phase. A hand-started run's rows are still served by `GET /api/forecast`, so what these change can reach readers. The admin card says so | Six more (`SKIP_LOW_RATED`, `SKIP_EXISTING`, `FORCE_IMMINENT`, `FORCE_STALE`, `EVALUATE_ALL`, `NEXT_EVENT_ONLY`) and the never-built `BATCH_API` were **retired in V153**. The six did act at first, on hand-started runs; v2.7.2 (2026-04-06) confined them behind `!triggeredManually` because `SKIP_LOW_RATED` was dropping locations from an explicit Run Forecast, and since every caller passes `manual = true` — the engine's scheduled triggers were retired on 2026-02-27, before the strategies existed — nothing could evaluate them after that. `OptimisationSkipEvaluator` and every mutual-exclusion rule went with them | ⚠️ The per-run `job_run.active_strategies` snapshot is **not written** on any current path — all five endpoints pre-create the run with a null snapshot — so the job_run holds no record of which strategies a run used (the executor does log them at INFO)
...
**Locations**: Multi-location, rendered on the shared `MapView` (Leaflet/OSM — both the Map tab and the Plan-tab overlay mount this one component, below) | Metadata: `SolarEventType`, `TideType` (H/M/L multi-select — the wanted water the Map tab's tide strip and dimmed chip key off, via the served `tideAligned`, `docs/engineering/tide-window-plan.md` §1 #1), `LocationType` (LANDSCAPE/WILDLIFE/SEASCAPE/WATERFALL) | Regions (geographic grouping) | Sunrise/sunset azimuth lines for the selected location — the tab's Heat view draws these too, same as the overlay; only the tab's Pins mode drops them (D-9) | ⚠️ **No marker clustering, anywhere — deleted 2026-09-04, not gated.** `react-leaflet-cluster` is off `package.json`, `MarkerClusterGroup` is out of `MapView`, and `markerUtils.createClusterIcon` is gone with it. **Do not reintroduce it, at any scale.** Clustering is the precise defect the heat field exists to fix: a bubble reading "12" cannot tell you whether tonight is worth driving for, because it averages the gems away — which is the design notes' own argument for why heat became the default. A Pins mode that clusters stops being the honest comparison and becomes a third representation, and the case for heat becomes unfalsifiable because you never see the pile it is solving. Pins already handles density deliberately (dots drawn weakest-first so the best is on top, 26px named over 13px unnamed); clustering would destroy that z-order and replace the best-visible pin with a count. **If the catalogue ever reaches thousands the answer is still not clustering — it is canvas rendering** (`preferCanvas` is already set), because clustering re-introduces the averaging problem at any size. An owner decision, taken with the roster at ~51. Consequences absorbed at the time: the medallions render unclustered on the frozen Plan-tab overlay (a deliberate, stated break of the freeze — the overlay opens focused on one spot and is slated for O-6 convergence anyway), the chip click's `zoomToShowLayer` camera jump went with it, and `excludeFromCluster`/`excludeFromSkyCluster` became dead and were removed — the sky-subject concept they encoded still lives in `heatSpots.js`, which is what the field reads | Star rating + location type filters | Emoji chip UI for metadata | Editable lat/lon

**Map tab (v2)**: `docs/engineering/map-tab-v2-plan.md`, P1–P12 merged. One chronological window control (`WindowControl` — solar EV rows read the served `bestRating`; astro/aurora night rows carry a served night window and a licensed client max, `mapEvents.bestOfNight`) replaces the old date strip, event-type pills and in-map `<select>`. Heat field (bloom, land-clipped, coastline stroke) is the default view, with a Pins mode toggle (`PinsLayer` — one dot per location, the honest comparison) and an expandable `MapLegendPanel` (ramp, whole-star labels, reach-rings toggle, confidence note). Filters live in `FiltersPopover` (min rating, subject chips, drive-from-origin, dark-sky toggle, scope), not an inline drawer. Selection is a greedy-labelled (`MapLabels`) anchored `MapCallout` (ring + card), not a Leaflet popup. ⚠️ Its **three** sheet routes are **not duplicates** (2026-09-05, O-18; the third added by the map-landing increment's L6 — this said "two" until L7's sweep): the clamped prose's `Four days here ›` opens `LocationFourDaySheet` OVER the map, tab unmoved, callout still mounted behind it, so an Escape returns the reader to the selection they pressed it from; the actions row's `Open in Plan` opens the same sheet and moves the tab its label names; and the region panel's location rows and its `Four days at …` action take the peek route for a place that is not the current selection. **One handoff carries all three** — `handleOpenLocationSheet(inPlan, spot)`, where `spot` defaults to the selection — told apart by `inPlan`, and the shell stamps the sheet footer's own map door `inPlace` from the tab in force — a door pressed from a peek must not import the Plan's lens onto the map the reader is already looking at (it would reset their rating floor, reach tier, scope and camera). `RegionsJump` ("◎ Regions ▾") is a nearest-measured-drive-first jump list. ⚠️ **On a phone the Regions/Heat-Pins/Filters bar is retired, not repositioned** — the earlier "phone layout moves them into a bottom bar" description is stale: since the peek-sheet series (`docs/engineering/map-mobile-sheet-plan.md`, M1–M6) `.wf-map-chrome-tr` is not rendered at all on phone (its old bottom-bar CSS rule is now dead code, superseded), and its three controls are mounted inside `MapPeekSheet`'s Layers section instead — see the *Map tab on a phone — the peek sheet* bullet below. **The Plan-tab overlay is a deliberately frozen, separate pre-v2 surface** — same `MapView` component, `overlayMode` gate — keeping the marker popup (`MarkerPopupContent`), medallions and `ForecastTypeSelector` as overlay-exclusive pieces; clustering is gone from BOTH surfaces (see the Locations bullet — an owner decision, and the one place the overlay's freeze was deliberately broken), so the overlay's medallions render unclustered; the selected location's azimuth lines ARE drawn by the tab's Heat view, and only its Pins mode drops them (D-9). Convergence onto the callout is an open owner decision (plan §4.9, O-6). **Two doors from the Plan tab open the Map tab at the window you tapped from** (`docs/engineering/plan-to-map-doors-plan.md`, D1–D4 merged, D5 dropped unbuilt by owner call): `App.openMapTabFromPlan` sends a door on the existing `mapTabHandoff`/`tabRequest` nonce channel, carrying the window, the Plan lens (rating floor, reach tier) and — at home only — the region focus, but never the origin, which is shared state the map already reads. ⚠️ That is the `source: 'plan'` door; the `inPlace` case above rides the same `mapTabHandoff` channel with `source: 'map'` and carries NO lens fields and no `tabRequest` at all — there is no tab to change, and writing the lens would silently undo the floor, tier and scope of the reader who is already on that map. One nonce-keyed effect in `MapView` applies both, and `MapBreadcrumb` mounts above the frame (not over it, so it needs no obstacle seeding) naming what is carried, with a `clear` that resets rating, reach, scope and the app-wide origin together. The tab reads every drive time through `driveMinutesFor` alone (home geography — the HOME marker, reach rings, ring labels, legend toggle — gated on `origin == null`); the frozen Plan-tab overlay keeps its own separate `/drive-times` fetch, gated on `overlayMode`. **A tide strip** (`MapTideStrip`, `docs/engineering/tide-window-plan.md`, T1–T8 merged) mounts as the last child of `.wf-map-chrome-bl` **on desktop and tablet only**. ⚠️ **The phone mount T7 built (a sibling of that chrome at the counts footer's row of the lifted stack, because `left: 8px; right: 8px` had to resolve against the map frame and the left-anchored chrome couldn't stretch a descendant edge to edge) was retired outright at the map-mobile-sheet-plan.md M3 phase**, not repositioned: the phone `MapTideStrip` mount and its `--tsh`-driven phone CSS are gone, `--tsh`/`wf-tide-strip-on` are desktop/tablet-only from here, and the phone reader's tide facts live instead in `MapPeekSheet`'s Tide section (`MapPeekTideSection.jsx`, reusing the strip's own exported chart/footer pieces in a taller vertical layout) — see the *Map tab on a phone — the peek sheet* bullet below. The desktop/tablet strip itself is unchanged: it shows whenever the window is solar and the padded viewport (`bounds.pad(0.12)`) holds a named coastal spot: the representative station's day tide curve with high/mid/low ruled across it, night shaded, the light marked on the curve with its height, and a footer naming the dominant unmet want and jumping to the next window that gives it. A coastal location whose tide misses stays on the map **dimmed, not dropped** — `opacity: .72` on the chip and Pins dot, the wave glyph gaining a shortfall arrow (`↑`/`↓`, null when the location's wanted set straddles the window's state) — never its star, its ramp swatch or its heat contribution. ⚠️ **A match now carries the matched state as a letter** (H/M/L, `tide-window-plan.md` §4 #21, 2026-09-25) — the SAME wide slot the miss arrow occupies, read off the already-served `tideState`/`fact.state` at all four mounts (the chip, the region-panel row, `TideFitBlock`, the heat strip's best-reachable line) with no new backend field: two neighbouring coastal spots can serve one identical tide state and disagree in tier purely because their wanted sets differ, and a bare plain-wave-vs-arrow pair read as a contradiction rather than as two questions correctly answered differently. ⚠️ **The map asks exactly one tide question, and it is the preference one**: the chip, its ring, its label-budget tiebreak (promotes a match, never demotes a miss), its tooltip, the callout/sheet block and the region-panel row all key on the served `tideAligned` (the location's own wanted `TideType` set against the window's served tide state) — never `tideOnTheLight` (bundle rev 2's earlier, type-blind "does an extreme land on the light" fact), which the two-tide-axes rule reserves for a different question and which survives only as the offset clause inside the served fit phrase (`tide-window-plan.md` §1 #6, §4 #11, §5 #3). ⚠️ **The gate is lifted** (2026-09-18, tide gate lift, `tide-window-plan.md` §6 Q1, decided after this increment shipped) — `BriefingGatingPolicy.HARD_CONSTRAINT_REASONS` is now empty, so a mismatched tide no longer withholds a slot from Claude; it reaches Claude and scores through `TideVisitor` exactly as a weather stand-down already did, its star dimmed by the R1 penalty averaged into the combined rating ("the star is the shot"). A gated slot with no star at all is now rare rather than the routine gated-miss shape T4–T8 were built against; the STAR side of `TideFitBlock` needed no change, since it always read the served rating wherever it touched one. `BriefingSlot.skyRating` (added the same day) is new — carried through `TideFitBlock`'s own small addition (the `· sky N★` clause, shown on every miss and on a match only when it differs from the combined star), beside the combined star, so the block can still say *"wrong water, not wrong light · … · sky 4★"* when the two diverge. The app opens on Map at ≥640px and on Plan on a phone (`utils/initialTab.js`, decided once at mount, never persisted — `docs/engineering/default-tab-by-device-plan.md`).

**Map tab — the verdict, the picks and the landing card** (`docs/engineering/map-landing-plan.md`, L1–L7). The window pill no longer says only *which* window: it carries the served `DisplayVerdict` for that window and **the region it names**, and wears the forecast's own `BEST BET`/`ALSO GOOD` as an outline medallion. ⚠️ **The verdict moves with SCOPE and never with a reader filter** — that is the increment's first rule. Rating floor, subject chips, reach and dark-sky change what the map DRAWS; "My area" vs "Everywhere" changes what is being judged, so only that moves the word. There is exactly one verdict channel on the tab (`utils/mapVerdict.js`, read off the scope pool): `WindowFirstMapPane` deliberately does NOT forward the Plan card's own `verdict`, because that one is the whole roster's answer and two answers for one window on one pill would disagree by default. Agreement with the Plan tab is proven by test at whole-catalogue scope, not by shipping the value twice. **The tab opens on an answer** — a two-row landing card (`MapLandingCard`) saying whether tonight or the morning is the one, once per forecast run, keyed on `briefing.generatedAt`; it is a card, not a `Modal`, and a tap on the map does not dismiss it. **One drilldown, two levels**: the pill menu's footer row opens `MapWindowPanel` (this window, region by region, ranked on the served mean with the ceiling beside it), and a row there opens `MapRegionPanel` (one region — its own verdict, its top four locations with the served tide glyph and a leave-by time, its served narrative, and two actions). The second level REPLACES the first rather than stacking, and the terminus is the `LocationFourDaySheet` that already exists, opened with `inPlan: false` over the map (O-18). ⚠️ **Panels persist**: panning, zooming or clicking the ground dismisses none of them (`useOutsideDismiss`) — they close on their own chip, their ✕ or `Escape`, and closing the window control's dropdown closes only that dropdown. ⚠️ **A panel that REPLACES another must move focus**, or the control that was pressed takes it to `<body>` and `MapView`'s pane-scoped `onKeyDown` never fires again — `Escape` then does nothing on the route the product ships. That defect was found and fixed FOUR times across this increment (L4's reopen row, L5's drilldown entry, and both of L6's transitions), and a fifth time in `MapCallout` just before it (#774); a test that fires a key at the panel node instead of at `document.activeElement` passes while it is broken. `map-landing-plan.md` §4 records every deliberate divergence from the design bundle (#1–#37) and §6 the owner decisions, three of them taken and six still open.

**Map tab on a phone — the peek sheet** (`docs/engineering/map-mobile-sheet-plan.md`, M1–M6 merged). On a phone every floating map control — the Regions/Heat-Pins/Filters bar, the tide strip, the landing card's auto-open, the scored-locations toast's standing position — is replaced by one collapsed-by-default bottom sheet (`components/map/MapPeekSheet.jsx`, `data-testid="wf-map-peek"`), 74px tall showing three summary buttons (*Other windows · Tide · Layers*), opening to 356px (clamped `calc(100% - 64px)` so it never covers the pill) on one section at a time. ⚠️ **`BottomSheet.jsx` does not fit and was never used for it** — it has no detents, returns `null` when closed (so it cannot show the collapsed state), portals to `document.body` behind a `.app-scrim` backdrop that would block the map, and is `role="dialog"` with `aria-modal`, which every map surface's Escape/outside-press rule would read as a foreign modal and stand down for; the peek sheet is the opposite on all counts — always mounted, in-frame, backdrop-free, not a dialog — so it is a purpose-built component mounted inside `.wf-map-tab`. **One gate, no threaded flag**: the open section is a value of the existing `openMapMenu` state (`'peek:win' | 'peek:tide' | 'peek:lay'`), which gives it exclusivity with every other map surface and the pane's Escape ladder for free, and any installed map touch or selection (a chip press, a pin press, a Leaflet drag, a handoff) collapses it via a dedicated `useMapEvents` listener — never `useOutsideDismiss`, whose whole rule is the opposite (map-frame presses are ignored so the drilldown panels persist). The Windows section reads its heading off the landing card's own served `landingCardModel().header` (never fixed copy) and lists the pill's full roster including night rows (`{bestRating}★ best` in place of a verdict word), with the drilldown's only phone door (`▤ This window, region by region`) as its last row. The Layers section holds Show (Heat|Pins), Regions, Filters (the scope segment lives here, since scope is what moves the verdict) and a persisted **Tide mode**: Auto/Always/Off (`app_user.map_tide_mode`, V155, `PUT /api/user/settings/map-tide-mode`, copying `map-colours`' shape exactly). ⚠️ **Every tide cue on the map already had exactly one gate, and the mode is one null at that one site**: `MapView`'s spot builder sets `tideTier: tideCuesOn ? tierOf(tide) : null`, and the chip's wave glyph, the pin, both tooltips and the dimming CSS all key off `spot.tideTier` — so Auto (tide shown only when the light is Maybe-or-better and the coast is in view), Always (ignores both) and Off (always null) reach all four consumers with no flag threaded anywhere. The phone's own Tide section (`MapPeekTideSection.jsx`) reuses the desktop strip's exported chart/footer pieces in a taller vertical projection of the same served curve — nothing about the desktop/tablet strip's own rendering changed. The landing card never opens on phone at all (superseded, not merely hidden — the phone never writes `mapLandingSeenRun`, so a reader who later opens the same run on desktop still sees it once), and the scored-locations toast moves to a fixed, centred `top: 62px` that fades 3s after its window key last changes. **The whole phone lifted stack was rewritten off the sheet's 74px floor** — the old bottom-bar CSS rule for the Regions/Heat-Pins/Filters cluster is now dead code (the JSX itself is `!isMobile`-gated so the rule never fires), the counts footer is unconditionally `sr-only`-clipped rather than positioned, and `MapCallout`'s phone placement band reads a fixed `--psh: 74px` rather than a live-measured rect, because every route that opens the callout is a map tap and every map tap collapses the sheet first. Desktop and tablet are provably unchanged throughout (pinned by test in every phase) — the whole series is phone-only, behind `hooks/useIsMobile.js`'s `(max-width: 639px)`.

**Tide runs (Hot Topics)**: ⚠️ **`HotTopic.tideRun` lost its renderer but is NOT write-only** (it was, between the Coming up redesign's P6 and v2.19.9). `KingTideHotTopicStrategy.alignmentInfo` reads `alignedEvent` to build the pill's headline, and `HotTopicEventEnricher.resolveTideEvent` reads it to set `eventType` — which is what decides, per window, whether `PlanWindowProjector.badgeFor` keeps the alignment sentence or substitutes that window's own water. So the run row steers a live, user-visible branch on both Plan cards of every tidal day, without being drawn anywhere. Its only *chart* renderer, `TideRunRow.jsx` (the 24-hour cosine-interpolated curve against the solar events — night shaded, amber solar rules — with the pill's `SPRING RUN n/N` chip), was deleted in the Coming up redesign's P6 (`docs/engineering/coming-up-plan.md` D7) along with the `HotTopicStrip` panel that mounted it; the Coming up tab's own tide-run coverage (`ComingUpTideSparkline`, a fixed-phase cosine unrelated to this field) is a different code path with a different chart, not a replacement reader of this field. The backend derivation below stays — an owner call, the same treatment v1 retirement D2 gave other now-readerless fields — and is recorded here so its removal reads as a decision, not an accident. A spring/king tide is a multi-day **run**, not n unrelated days | `TideRunBuilder` → `TideRunDay` (nullable `HotTopic.tideRun`, no migration — rides the `daily_briefing_cache` JSON) derives per day: range vs the location's own mean, every extreme in the **Europe/London** local day, that day's sunrise/sunset, sea state, and a plain-language alignment verdict | One representative coastal location for the **whole run** (biggest single-day range), named in the footer as the caveat | Chart **replaces** the fact chips; days with no derivable range fall back to them | **The pill's headline is derived from the same run row the chart draws** — `KingTideHotTopicStrategy.alignmentInfo(day, nonExpired, unaligned, passed)` reads `TideRunDay.alignedEvent`, and a null run row yields *silence* rather than a denial. Two rules keep the two lines from arguing, and both were adversarial-review findings against the first cut: `alignedEvent` follows **the water the verdict names**, not `usefulPoint` (verdict rules 2 and 3 name the *other* extremum when the useful water misses the light and that one does not — keying off `usefulPoint` alone printed "no alignment" above `HW 06:18 · 58m after sunrise`), and an alignment with an **already-passed** event gets its own wording (`KING_ALIGNMENT_PASSED` / `SPRING_ALIGNMENT_PASSED`) rather than collapsing onto the denial, because the chart, the verdict and the aligned-day editorial line are all still on that card. `aligned` and `phrase` stay keyed to the useful water — they mark what a reader of *that* run type came for. | **The headline names its own scope, from a roster-wide tally computed off the same geometry** — `TideRunBuilder.rosterAlignment` measures every coastal location at *its own* sunrise/sunset with the same rule the representative's row uses, carried as `TideRunDay.RosterAlignment(sunriseAligned, sunsetAligned, measured)`, so the pill reads `tide aligned with sunrise at 47 of 61 coastal locations` rather than pairing one coastline's geometry with the roster's size. Identical rules are load-bearing: they make the representative a member of its own tally, so an aligned chart can never print above a zero. Costs **no new queries** — `fetchExtremes` already holds every location's extremes; only the per-location solar times are new. ⚠️ **Do not "fix" `forecast_evaluation.tide_aligned` and go back to counting it.** That was considered and rejected twice: it is written only by the *synchronous* engine the batch consolidation is retiring, and it answers a **preference-weighted** question (did the tide *state* match each location's configured `TideType` — production has 42 of 61 accepting `HIGH`) where the badge asks an astronomical one true at all 61. The `countTideAlignedByTargetType` query and both strategies' `ForecastEvaluationRepository` dependency are gone; the drill-down's `TideMetrics` is fed from the same tally | **The denominator is the coastal roster, not the measured subset** — they diverge when a location has no stored extremes, and "of 52" on an aligned card beside "· 61 coastal locations" on the unaligned card of the same run is two totals for one roster. An unmeasured location counts as not aligned: it understates the fraction, which is the safe direction | **`HotTopicEventEnricher.resolveTideEvent` treats the run row as authoritative** and takes `SolarEventFreshness`. Both rules were review findings against the first cut: falling through to the tally when `alignedEvent` is null printed a lead and a window badge above the headline's own denial (harmless only while the tally came from an empty column), and the tally is **clock-free**, so without the freshness check a card saying "tide alignment already passed" still pointed a lead at that morning. A null `eventType` makes `PlanWindowProjector` drop a **window-scoped** topic silently — no badge, no day — which is how all of this surfaced. ⚠️ **It no longer drops a TIDE.** Since v2.19.9 `keysFor` tests `isDayScoped` *before* the `eventType` guard and buckets `KING_TIDE`/`SPRING_TIDE` onto **both** of the date's windows: a spring or king tide is a claim about the day, and a null `eventType` there means only that the alignment has passed or that no water reached the light — neither of which stops the day being tidal. The alignment sentence stays on the aligned window; the other window states its own nearest water, **read from `BriefingWindowTide`** (never re-derived from the run row, whose extremes are clipped to the civil day and whose geometry belongs to a separately-selected representative — both were shipped as bugs in v2.19.9 and reverted) It was a `forecast_evaluation.tide_aligned` count, which asks a different question (did the tide *state* match each location's own `TideType`, in a golden/blue-hour-sized window) and cannot distinguish "nobody aligned" from "no rows written" — so on a king tide, where the useful water is HIGH, every LOW-preference location counted for nothing and the card read "no tide alignments" above a chart saying `HW 04:58 · 39m before sunrise`. The counts still drive the **drill-down** (`TideMetrics` sunrise/sunset counts), which is the question they actually measure | A king run also carries `highWaterRank` — distance to the highest water on record there (`"0.4 m off the record"` / `"highest recorded here"`). Not metres over mean high water: `springTideThreshold = 1.25 × avgHigh`, so that figure is the spring excess plus a per-location constant, and not a percentile either, since the king classification is itself an above-P95 test. Gated on `p95HighMetres != null` (one spring–neap cycle, the same gate the spring chip rides) **and** `dataPoints ≥ 700` (~6 months) — a location added last fortnight has a maximum, not a record

**Lunar eclipse** (`docs/engineering/lunar-eclipse-plan.md`, L0–L7): a `LUNAR_ECLIPSE` hot topic, an
`ALMANAC` `lunar-eclipse` entry and a per-location "dawn race" — the moon racing the brightening or
darkening sky to the horizon — extending the solar `ECLIPSE` topic's slots rather than reworking
them. `LunarEclipseCatalog` is a static, hand-seeded table of NASA Besselian-style contacts (`P1 U1
U2? max U3? U4 P4`, UTC, every umbral eclipse March 2025–2030 visible somewhere in the British Isles
— penumbral eclipses are deliberately not seeded, since a row that can never raise a topic is a row a
future session un-suppresses); `LunarEclipseCalculator.sight(eclipse, lat, lon)` derives each
location's own altitude/bearing at maximum and its set/rise-in-shadow answer from `solar-utils`'
`LunarCalculator`/`MoonriseMoonsetCalculator`, memoised per date (eligibility: ≥3° at max, **or** ≥3°
for a contiguous 30 min somewhere in `[u1, u4]`). ⚠️ **Two types share one channel, not one type with
a `body` field** — this codebase has no `HotTopicType` enum and no shared style catalogue to switch
on, so `LUNAR_ECLIPSE` is its own hot-topic type (rarity rank 2, after solar `ECLIPSE`) that maps
onto the *existing* eclipse channel (colour, glyph family, filter chip) on the client, rather than
threading a `body` discriminator through `HotTopic`/`Badge`/`AlmanacEvent`/`ComingUpEntry` and every
switch keyed on type. `BriefingSlot.eclipse` (nullable `EclipseSight`, no migration — rides
`daily_briefing_cache` JSON like `TideInfo`) carries the per-location, per-window sight: altitude,
bearing, moonset/moonrise, `setsInShadow`/`risesInShadow`, and (only on a window close enough to
twilight) a `race` (`DAWN`/`DUSK`/null) with four served light `stops` for the track's gradient — a
high, leisurely eclipse has a sight with no race, and the popup then shows facts only. The admin hot-
topic simulation template attaches this SAME fixed sight to every slot of its simulated window at
**serve time only** — it is never persisted, which is the one thing that makes the feature
browser-verifiable before the next live eclipse actually occurs. `DawnRace` (`components/DawnRace.jsx`
+ pure geometry in `utils/dawnRace.js`) draws the track in the popup, `aria-hidden`, with one visible
accessible sentence stating the whole answer (the tide-run chart's own rule) — `raceModel`/
`raceSentence`/`eclipseSpotLine` are filter/map/select over already-served instants, not a
derivation (the `comingUpSparkline.js`/`dawnRace.js` precedent the Backend-heavy bullet names below).
The Plan card's chip gets a clocked emphasised pill (`CLOCKED_TOPIC_TYPES`, shared with solar
`ECLIPSE`) reading the eclipse's own maximum time after a divider. ⚠️ **No `clearToDeg` anywhere** —
the solar handoff's "clear to the west N of M" and its west-horizon ranking were dropped for want of
terrain data, and the lunar work does not fake one: the honest per-spot substitute is the served
altitude/bearing/set-or-rise-in-shadow line, mounted as `EclipseSpotLine`
(`components/map/EclipseSpotLine.jsx`) — a sibling block right after `TideFitBlock` in both
`LocationFourDaySheet` and `MapCallout` (L7), fed by `utils/locationSheet.buildEclipseIndex` via the
same `lookupForWindow` join `TideInfo` already uses. **No promoted strip and no T−1 promotion rule**
(§6 Q1's default) — the promoted-strip machinery the solar handoff assumed was retired at plan-matrix
M5 before this feature existed, so the evening-before need is carried by the clocked chip and the
popup alone; a page-level strip is an open owner decision, not a gap in this feature. Coming up
carries a 🌘 glyph, a `next`-from-the-UK accent row and a dashed-top `aside` exposure note ("No
filter needed — bracket, the shadow is ~10 stops under the lit edge"), never `safetyNote` (the design
is explicit the lunar note carries no safety warning, unlike the sun's). The tab badge's rarity gap
(`lunar-eclipse-mean-gap-days: 900.0`) is a documented long-run estimate, not the seeded catalogue's
own mean (~290 days over six gaps, March 2025–2030 — that window is unusually dense); lowering it to
match would be a re-census decision, recorded in-line beside the config value.
...
**Waterfall UI**: Colour forecast AND hourly comfort rows | 💦 marker | ⚠️ Waterfall scores were once excluded from cluster marker averages (waterfall photography ≠ sky colour); that exclusion died with clustering itself. ⚠️ **`heatSpots.js`'s sky-subject filter is NOT its survivor, and this bullet said it was.** `SKY_SUBJECT_TYPES` is LANDSCAPE, SEASCAPE **and WATERFALL**, so a waterfall is a sky-prompt candidate, keeps its scores, and paints in the heat field like any other. What that filter withholds is a WILDLIFE-, WOODLAND- or BLUEBELL-only site (an untagged one counts as a candidate). The cluster-era exclusion has no live successor
...
**User settings**: `UserSettingsService` + `UserSettingsController` — home location (postcode via `PostcodesIoClient` geocoding, lat/lon) and per-user drive times | `DriveTimeResolver` abstraction (replaces per-location `drive_duration_minutes`) | `user_drive_time` table (V67) | ⚠️ **The eight settings columns on `app_user` are `updatable = false`** — home postcode, latitude and longitude, local radius, drive-time stamp, map colour scale, map tide mode (`map-mobile-sheet-plan.md` M4), Coming-up last-seen — and are written only by column-scoped updates on `AppUserRepository`. So `setX()` then `save()` on an existing user writes NONE of them (inserts are unaffected). A whole-entity save wrote every column from whatever copy it held, so overlapping writes lost one another — and the JWT filter's last-active save, made at most hourly from whichever authenticated request comes first, is one such writer. | A drive-time refresh, manual or nightly, stores only while the home is still the one it measured from: `UserDriveTimeWriter.storeIfHomeUnchanged` compare-and-sets the stamp on the measured coordinates, then replaces the rows in the same transaction. Otherwise the manual refresh answers **409** having written nothing (a retry is always allowed, because the move released the cooldown), and the job counts that user as superseded. `saveHome` decides whether the home moved from a `SELECT … FOR UPDATE` read
...
- **Backend-heavy** — all calculations on backend. Frontend is a pure render layer. All four of the Plan-tab exceptions this bullet used to list are now **discharged**: `BriefingDay.peak` has been write-only since the day rail was retired at P2 (the matrix replaced it — `docs/engineering/v1-retirement-plan.md` §8.2), the grid cell reads `BriefingRegion.meanRating` computed at serve time by `PlanWindowProjector`/`enrichWithCachedScores` (Phase 3 of `docs/engineering/plan-verdict-consolidation-plan.md`), "Close to home"'s client-side proximity ranking died with `CloseToHome.jsx` in v1 retirement D2 (the backend endpoint survives with zero callers — §8.3), and the v1 arm's own copies of the roll-up and the cell derivation — kept deliberately byte-identical because v1 was the pilot's frozen comparison control — were retired with it. What is left is a **reach-scoped** class of client-side derivation, licensed rather than exceptional: P8's location sheet, which takes a max over **one location's own** windows for its `◎ best here` tag and counts its own rows for its lead line; the matrix's spread histogram (star-band counts) and its best-reachable line, both over each window's reach-gated pool (plan-matrix A10/A11); the page-level conflict message's ceiling (`planConflicts.ceilingOf` — a max across every window and every location, sanctioned by plan-matrix §5, and the one member that *does* aggregate across locations); and the popup prose slot's "this region's own best window"; **the tide-alignment series adds two more members, and nothing else** (`docs/engineering/tide-plan-card-plan.md` §5 #1) — `card.tideFit` (`{coastal, matched, live, meanQuality}` over the reach pool, `windowFirstCards.js`) and the strip's `tideRun` (which windows are live and which is the best of them, `windowFirstTideRun.js`). None of these re-derive a *server-owned* verdict or Best-pick math — the ban in plan §2.12 is on that, and each of these reads rather than recomputes. ⚠️ **That class is NAMED and has a recorded exit; it is not a precedent for client aggregation.** Reach is per-user (`/api/user/settings/reach`, never ETag'd), so "best *within reach*" has no servable answer on the shared `GET /api/briefing` payload at all — the same reasoning that put "Close to home" on its own per-user contract. They are counts of *places you could drive to*, never "N of M scored" (§6 clause 4), and ⚠️ **none of them may say "within reach" unless a drive time exists to have gated on** — `BriefingWindow`-derived `card.reachMeasured` is the single producer of that answer and six surfaces read it, because an unknown drive passes every tier and a reader with no postcode was being told six times over that a filter had run. The exit is plan-matrix §8's optional **O-4**, a never-cached per-user endpoint; until then it is debt with a name. A third client rule *filters* rather than derives: `windowFirstTopics.js` intersects each topic's served `regions` with the origin scope, exempting whole-sky types by a client type map, because `regions` means something different per strategy (A8). A fourth class lives on the Coming up tab, not Plan (`docs/engineering/coming-up-plan.md` D12): `isNew` per entry, the tab badge's `{band, count}`, and the since-line's highest-bits entry selection — `utils/comingUpArrivals.js` — licensed by the identical reach reasoning, since `comingUpLastSeenAt` is per-user and `GET /api/almanac` is ETag-shared, so "what is new *to you*" has no servable answer on that payload either. Members: those three, nothing else — chip counts, month grouping and the tide sparkline's geometry are the ALREADY-licensed filter/map class (`utils/comingUpFeed.js`), not this one. The exit, if ever needed, is the same shape as O-4: a never-cached per-user `GET`. A fifth class lives on the Map tab, not Plan (`docs/engineering/map-tab-v2-plan.md` §3 P6/P11, adjudicated at P11 and recorded here at P13): `mapEvents.bestOfNight` (the window control's own `N★ best` for an astro/aurora night row — a client max over that night's FULL served roster, never scope/filter-narrowed, mirroring a solar window's served `BriefingWindow.bestRating`) and `regionsJump.buildNightRegionBest` (the identical max, grouped by region via the location→region join `heat.spots` already carries, for the Regions jump list, mirroring `BriefingRegion.bestRating`) — licensed because astro/aurora carries no *rated* per-window or per-region rollup comparable to those solar figures. A star-free per-region aurora summary does exist on the wire (`AuroraRegionSummary`'s GO/STANDDOWN `verdict`, `clearLocationCount`), but nothing served aggregates stars at all, per window or per region, for a night. Members: those two, nothing else. The exit, if one ever ships, is a served RATED per-window/per-region night rollup (map-tab-v2-plan.md's **O-16**) — until then this pair is the licensed stand-in. ⚠️ **A SIXTH class joined that same tab with the map-landing increment** (`docs/engineering/map-landing-plan.md` §3 L1–L6, §5 D-4, adjudicated at L7): four figures the Map tab derives over the reader's own SCOPE segment — "My area" or "Everywhere", which is a per-user planning area and therefore has no servable answer on the ETag-shared `GET /api/briefing` payload, the identical reasoning to the reach class above. **(a)** `mapVerdict`'s tally — the `+N` / "everywhere in your area" count over the in-scope regions sharing the verdict's tier; **(b)** `mapLanding.landingCardModel`'s `allPoor` — a FOLD over two scope-narrowed verdicts producing a sentence no server field states (*"Neither is worth the drive."*); **(c)** `mapLanding.nextWorthIt` — a search forward over the same client-scoped tier for the next window that clears it; **(d)** `mapDrilldown.buildPanelRegionRows`' `atFourPlus`/`placeCount` — the drilldown row's `N of M at 4★+`, and the region's nearest measured drive beside it. ⚠️ **(d)'s population is the places a SKY VERDICT applies to at all, not every place in scope**, and the difference is the whole reason it is honest: `buildHeatSpots` keeps a wildlife hide or a wood as a spot and withholds only its scores (`skySubject` records why — the sky subjects are LANDSCAPE, SEASCAPE and WATERFALL, so it is a WILDLIFE-, WOODLAND- or BLUEBELL-only site that is withheld, and an untagged one counts), so counting it put places in the denominator that can never reach the numerator — a region with five sky locations and four hides read `3 of 9`, understating every wood-bearing region uniformly. That is the mirror image of the "N of M scored" phrasing this bullet already bans: not a denominator of rows-we-scored, but one holding places the question does not apply to. Two adversarial-review lenses found it independently at L6. None of the four re-derives a served verdict or Best-pick maths — the ban is on that — and none says "within reach", because the count is scope-scoped rather than reach-gated and the drive beside it is omitted entirely where none is measured. ⚠️ **Members: those four, nothing else — and `mapDrilldown.buildRegionLocationRows` is deliberately NOT a fifth.** The region panel's top-four *looks* like one and is not: it derives no figure (every star on a row is served), and the per-user input is the drive, which enters as a sort tiebreak and as two fields that are per-user by definition. It is the already-licensed filter/map/select class. `map-landing-plan.md` §4 #32 argues it in full, including the correction to its own first wording — a wrong entry here costs the list the closure it exists for. The exit is the same shape as O-4: a never-cached per-user `GET` carrying a scope-aware tally (§6 Q4, undecided). ⚠️ **A SEVENTH class joined the Map tab with the tide-window increment** (`docs/engineering/tide-window-plan.md` §1 #12, adjudicated at T8): the client's slice of the tide-fit derivation, six members of *filter/map/select over served facts*, never a formula the spec would otherwise have put on the client (the level, the bands and the fit maths all stayed server-side — `TideCurveCalculator`/`TideWording`). **(a)** `mapTideFit.stripModel`'s viewport filter — the strip's visibility and its in-view named-coastal counts, the same shape `MapLabels`' own in-view set already uses; **(b)** `stripModel`'s `dominantWantCount` — a tally of served `TideType` values over the dimmed in-view locations, ties broken HIGH > LOW > MID; **(c)** `nextAlignedRow` — a forward scan over served per-window `tideAligned`/`tideState` for the next fit, a lookup rather than a formula; **(d)** `tierOf` — `tier = aligned ? 'match' : 'miss'`, a served boolean read; **(e)** `wantPhrase` — joining a location's served, static `tideType` set into words ("high water or low water") for the callout/sheet block's jump/denial line, mirroring `windowFirstRows.js`'s own `STATE_WORD` lexical mapping; **(f)** `siblingEventTime` — a lookup over the already-built EV list for the strip chart's sunrise/sunset clock labels, reading the served, already-formatted time off the sibling solar row rather than formatting one itself. **Members: those six, nothing else** — nothing here reads a height, a minute offset or a threshold to decide anything. `mapTideFit.js` also exports `tideTierHeading`/`tideAccessibleClause` (fixed per-tier strings for the tooltip heading and the accessible-name clause), but those are licensed separately, under the pre-existing 'client concatenates a heading to a phrase' rule (`tide-window-plan.md` §5 #5) — a lookup returning a constant is not a derivation over served facts, so they are not a seventh and eighth member here. **Unlike classes four through six, this one names no per-user or per-scope exit, because it answers no per-user or per-scope question** — every fact it filters, tallies or scans is already on the shared `GET /api/briefing` payload, so it is the bullet's OWN opening 'filter/map/select over served facts' licence, not a reach/scope exception to it; it earns a number here only because a coastal-fit lookup sits close enough to a served verdict that a future reviewer could otherwise mistake one for the other. ⚠️ **`utils/dawnRace.js` (the lunar eclipse dawn/dusk race, `docs/engineering/lunar-eclipse-plan.md` §2.7/§3 L7) is the SAME already-licensed filter/map/select class `utils/comingUpFeed.js`/`comingUpSparkline.js` are, not an eighth class of its own** — `raceModel`/`raceSentence`/`eclipseSpotLine` floor a track start, clip a track end, pick which of two already-served booleans draws a hatch, and turn already-served instants into `[0,1]` positions and formatted strings; nothing there computes a moon position, a light boundary or a verdict. ⚠️ **The peek-sheet series' three new client reads (`docs/engineering/map-mobile-sheet-plan.md` §4 #11) are the SAME already-licensed filter/map/select class, not a ninth class of its own** — `otherWindow` scans the already-built EV roster for the next row whose served tier is not `STAND_DOWN` (a lookup, never a verdict computation), `tideSummary` reads the served tide state/direction words plus the strip's own already-computed dimmed count into one string, and the AM/PM word is a lexical map off the served `eventType`; none of the three re-derives a rating, a level or a fit. `docs/engineering/plan-panel-data-contracts.md` records the rest.
...
- **The tide-run and storm-surge charts (`TideRunRow`/`SurgeRunRow`/`components/chart/solarDayGeometry.js`) are gone** — deleted with the Hot topics door in the Coming up redesign's P6 (`docs/engineering/coming-up-plan.md` D7); `solarDayGeometry.js` (their shared pure axis/clock geometry) had no other importer once both charts were removed and was deleted alongside them rather than left orphaned. The design rationale they carried (a tide row plots shape, a boolean high/low; a surge row plots metres through a domain→range scale, so the two were deliberately different functions sharing only axis geometry, never a parameter) is recorded here as history in case a future phase revisits either chart — the underlying backend data (`HotTopic.tideRun`, the surge fields on `forecast_evaluation`) is untouched. The Coming up tab's `ComingUpTideSparkline` is an unrelated, independently-built chart (a fixed-phase cosine with no clock relationship at all, plan P3b) — not a port of either retired component.
...
| V153 | Retires six optimisation strategies unable to act since v2.7.2, plus the never-built `BATCH_API` (`DELETE` by `strategy_type`, the shape of V42) — paired with removing their enum values, so a row left behind would make Hibernate throw on read. Local dev runs no migrations, so `OptimisationStrategyService` deletes the same explicit list at startup (never "every unknown type" — that would eat a newer type's rows on a rollback) and fills any missing default row |
...
|---|---|---|---|
| `cached_evaluation` (briefing evaluation cache) | **no** — overwritten each run | `BriefingEvaluationService.writeFromBatch`, from the batch pipeline | `BriefingService.enrichSlot` — **this is the rating the UI displays**, keyed by location *name* — ⚠️ since the tide gate lift (2026-09-18, `docs/engineering/tide-window-plan.md` §6 Q1) each entry's `results_json` also carries `skyRating` (`BriefingEvaluationResult.skyRating` → `BriefingSlot.skyRating`, no migration): the sky visitor's own component alone, with no tide averaged in, next to the combined `rating` every other reader already used. Null on a row written before the field existed |
...
> `ScheduledForecastService` guards all three, taken on the request thread before the refresh is
> handed to the executor, so there is no accepted-but-not-started gap. Two overlapping refreshes both
> paid WorldTides for the same window, and the slower one's inserts could collide on
> `uq_tide_extreme`. A refused schedule fire or Run Now is only logged — Run Now still shows
> "Triggered ✓", because `triggerNow` queues before the guard is consulted. The backfill is not guarded.
...
### Aurora (PRO_USER/ADMIN; `/api/aurora/admin/*` and `/api/aurora/forecast/preview|run` ADMIN)
`GET /api/aurora/status` | `GET /api/aurora/locations` | `GET /api/aurora/viewline` | `GET /api/aurora/viewline/forecast` (all PRO_USER/ADMIN — `AuroraController` is `@PreAuthorize` PRO_USER/ADMIN, so a LITE account gets 403) | `GET /api/aurora/forecast/results|results/available-dates` (PRO_USER/ADMIN, `AuroraForecastController` — the map's read path) | `GET /api/aurora/forecast/preview` and `POST /api/aurora/forecast/run` (**ADMIN** — see the ⚠️ below) | `POST /api/aurora/admin/enrich-bortle` (ADMIN) | `POST /api/aurora/admin/run` (ADMIN — runs the scheduled polling cycle now and waits for it; 409 while a cycle is running; returns `{status, dark, level, action, trigger, held}`) | `POST /api/aurora/admin/reset` (ADMIN) | `POST /api/aurora/admin/simulate` and `POST /api/aurora/admin/simulate/clear` (ADMIN)

> `/preview` and `/run` shared `PRO_USER/ADMIN` with the two read endpoints above until
> 2026-09-15 — the only frontend caller of either is `AuroraForecastModal`, mounted only inside the
> Admin-only Operations tab, and `/run` spends real Claude cost the way every other forecast-run
> endpoint in this app does (all ADMIN-gated) — the same more-permissive-than-intended shape as
> `POST /api/locations` and the forecast backtesting endpoints, above. ⚠️ **A simulated run never
> reaches `/results` or `/results/available-dates`, for any caller.** `POST
> /api/aurora/admin/simulate` injects fake Kp/storm data into `AuroraStateCache` for admin UI
> testing; before 2026-09-15 nothing downstream checked `isSimulated()`, so the fake reading reached
> signed-in users' Plan cards (`AuroraHotTopicStrategy`), the best-bet advisor's prompt
> (`BriefingRollupBuilder`, `BriefingAuroraSummaryBuilder`), and — via a real Claude call persisted
> with no simulation marker — the map, as if all three were real. Fixed by gating the first two on
> `isSimulated()` and by a durable `aurora_forecast_result.simulated` column (V154) the
> repository's read methods filter on; `deleteByForecastDateIn` stays unfiltered so a real re-run
> still clears an earlier simulated test run for the same night.

### Briefing (Bearer / ADMIN for writes)
`GET /api/briefing` | `GET /api/briefing/digest` (Bearer) | `POST /api/briefing/run` (ADMIN; 409 while a build is running) | `GET /api/briefing/evaluate/scores` (Bearer) | `DELETE /api/briefing/evaluate/cache` (ADMIN) | `POST /api/briefing/compare-models` (ADMIN) | `GET /api/briefing/compare-models/runs` (ADMIN) | `GET /api/briefing/compare-models/results` (ADMIN)
...
interface whose contract is *answer for the whole range or do not exist*; seven implementations cover
tide runs, meteor peaks, supermoons, equinox/solstice, the NLC season, `EclipseAlmanacSource` and
`LunarEclipseAlmanacSource` (the lunar eclipse catalogue, `docs/engineering/lunar-eclipse-plan.md`).
...
reasoning that keeps reach off `GET /api/briefing`). `app_user.coming_up_last_seen_at` (nullable,
no backfill) is served as `comingUpLastSeenDate` on `GET /api/user/settings` — the Europe/London
civil date, derived server-side (`ForecastHorizon.civilDate`) so the client only ever compares two
ISO date strings — and written by its own `PUT /api/user/settings/coming-up-seen`. Every ALMANAC
entry carries a server-computed `enteredWindow` and non-null `bits`/`interim`; the client's
`isNew`/badge/since-line derivation (`utils/comingUpArrivals.js`) is the fourth member of the
Backend-heavy bullet's licensed per-user-join class, above. The band edges (`bands` on this
payload) are **census-set**, not placeholders: `ComingUpAnnualBadgeCensusTest` runs a synthetic
year through the real sources and the real assembler, landing 11 badge arrivals/year with 1
interrupt — the design's own ~10/year, one-or-two-interrupts target.
...
`PUT /api/user/settings/map-colours` | `PUT /api/user/settings/map-tide-mode` (the Map tab's
persisted phone tide mode — Auto/Always/Off, `map-mobile-sheet-plan.md` M4 — copies `map-colours`'
shape exactly; read and written by the phone Layers section's Tide segment since M5) |
`PUT /api/user/settings/coming-up-seen` (Coming up tab badge's last-seen write, plan D3/P5 — see
CLAUDE.md's Almanac section)
