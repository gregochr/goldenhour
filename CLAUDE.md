...
> `@PreAuthorize("hasRole('ADMIN')")`. `POST /admin/run` now runs the same guarded cycle as the
> `aurora_polling` schedule (`AuroraPollingJob.runCycleIfIdle()`), rather than calling the
> orchestrator directly — see the ⚠️ above.
