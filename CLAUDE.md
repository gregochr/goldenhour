...
### Aurora (PRO_USER/ADMIN; `/api/aurora/admin/*` ADMIN)
`GET /api/aurora/status` | `GET /api/aurora/locations` | `GET /api/aurora/viewline` | `GET /api/aurora/viewline/forecast` (all PRO_USER/ADMIN — `AuroraController` is `@PreAuthorize` PRO_USER/ADMIN, so a LITE account gets 403) | `GET /api/aurora/forecast/preview|results|results/available-dates` and `POST /api/aurora/forecast/run` (PRO_USER/ADMIN, `AuroraForecastController` — a PRO account can start a forecast run, one Claude call per viable night) | `POST /api/aurora/admin/enrich-bortle` (ADMIN) | `POST /api/aurora/admin/run` (ADMIN — runs the scheduled polling cycle now and waits for it; 409 while a cycle is running; returns `{status, dark, level, action, trigger, held}`) | `POST /api/aurora/admin/reset` (ADMIN) | `POST /api/aurora/admin/simulate` and `POST /api/aurora/admin/simulate/clear` (ADMIN)
