...
- Single `ClaudeEvaluationStrategy` parameterised by `EvaluationModel` with extracted `PromptBuilder` and `MetricsLoggingDecorator`; `NoOpEvaluationStrategy` for wildlife
...
- **Evaluation strategy** — single `ClaudeEvaluationStrategy` parameterised by `EvaluationModel` (replaces `AbstractEvaluationStrategy` + 3 trivial subclasses). `PromptBuilder` handles prompt construction, `MetricsLoggingDecorator` (GoF Decorator) transparently adds timing/logging/metrics when a `JobRunEntity` is present. `NoOpEvaluationStrategy` for wildlife (no Claude call). `AnthropicApiClient` handles API calls with declarative `@Retryable`. `EvaluationConfig` produces a `Map<EvaluationModel, EvaluationStrategy>` bean; `EvaluationService` does map lookup to delegate to the matching strategy.
...
- 673 backend tests, all passing with ≥80% JaCoCo coverage
