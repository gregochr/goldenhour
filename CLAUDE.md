...
**Waterfall UI**: Colour forecast AND hourly comfort rows | 💦 marker | ⚠️ Waterfall scores were once excluded from cluster marker averages (waterfall photography ≠ sky colour); that exclusion died with clustering itself, and the same subject rule survives where it still matters — `heatSpots.js`'s sky-subject filter, which decides what the heat field paints
