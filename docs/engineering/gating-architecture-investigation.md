...
**Architectural lesson, recorded for posterity:** the UI's `region.displayVerdict`, `region.summary`, and per-location list were all derivable from triage data alone. No code path checked whether Claude had actually evaluated any of the region's locations. The honesty filter is a compensating control at the API read boundary; the deeper fix is the Gate 2 verdict redesign, which should remove the underlying assumption that "briefing verdict" and "evaluated location" are equivalent.

Concretely, in the broken state: the "7 named Worth-it locations" in the drill-down came from `region.slots` filtered to `verdict !== STANDDOWN`, and the "Clear at N of M locations" copy was built by `BriefingVerdictEvaluator.buildRegionSummary` from triage GO/STANDDOWN counts. Neither consulted `cached_evaluation`. Optimistic triage was therefore sufficient to mislead the UI even with zero Claude evaluations — the briefing was honest about *triage*, just dishonest about *coverage*.
