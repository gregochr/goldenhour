...
| A23 | Location result chips read `4 DAYS` | `Windows here` | The sheet it opens derives its own span and prints it — six rendered events fall across **three** days whenever today still has both its windows ahead — so a fixed `4` in the chip would be a number nobody measured sitting beside the same number measured. Rule 5's family. |
