...
**Status: COMPLETE — M1–M5 are all on `main` (M5 as `4627898b`, #599). The flag flip is the owner's call and this series no longer blocks it; §11 states what it still waits on, and §11c is the list to read before flipping.** Written 2026-08-20 against the v3 design bundle at
...
PR lands — a phase cannot name its own hash, since writing it in changes it.

| phase | branch | commit | date | notes |
|---|---|---|---|---|
| M5 — the sweep | `feature/plan-matrix-m5-sweep` | `4627898b` (#599) | 2026-08-21 | The settling commit: the promoted strip deleted, the copy sweep run, the accessibility adjudication made, and everything measured in a **real browser** (headless Chromium via Playwright — the Browser pane's `visibilityState: 'hidden'` suspends RO/IO and rAF, which three phases hit). Seven prosecutor lenses over the whole v2 surface then found more than the build did, and **six of the fourteen things below are theirs**. **Fourteen things a later phase should not rediscover.** (1) ⚠️ **A22 RESOLVED, both ways: `inert` on lower layers, and NO third layer.** Three `aria-modal` dialogs stood open at once and a Tab walk out of the topmost sheet reached the masthead's search button on press **17** and the popup underneath by press **34**. `Modal`'s new `stacked` opt-in fixes the covering (`inert` + no `aria-modal`), and the third layer is now refused outright rather than ordered — it was reachable only through the masthead button, which was bypassing the guard `/` had carried since M3, and it **rendered broken**: every `Modal` is `fixed inset-0 z-50`, so paint order is DOM order and the sheet painted its scrim and its whole card over the search panel. Photographed. (2) ⚠️ **`stacked` is a PROP, so it blurs before any effect in the commit.** It lands in React's mutation phase, which beats `useDialogFocus`'s reading of `document.activeElement` — so opening the location sheet from a field chip recorded `<body>` as the place to go back to and Escape dropped the reader at the top of the document. **Intermittently**, because it races the passive flush: the spot-card route passed three runs and failed under instrumentation. Fixed inside `Modal` (a dialog records the last control focused *inside itself* and restores it when uncovered) — and ⚠️ **from `pointerdown` as well as `focus`, because macOS/iOS Safari does not focus a `<button>` on click**, which would have left the fix Chromium-shaped on the exact gesture it was measured against. (3) ⚠️ **A stacked LAZY dialog left the page with no live layer at all.** `stacked` is synchronous and the covering layer arrives when its chunk does; with the sheet's chunk throttled to 2.5 s the page held **one dialog and zero live layers** for the whole fetch — popup inert, controls and Escape dead, `fallback={null}` rendering nothing. A regression `inert` introduced. Mitigated by warming the two lazy stacked chunks when the popup opens (every route to them starts there); the real fix — gate `stacked` on the covering layer having mounted — is named in §11 rather than smuggled in. (4) ⚠️ **THE REACH CLAIM WAS WRONG ON SIX SURFACES, and the sweep fixed one.** An unknown drive passes every tier (§2.5), so for a reader with no home postcode nothing is gated and every "within reach" names a filter that never ran (§6 clause 7). M5 fixed the popup's footer; three lenses independently found it still standing on the popup header, its two absence sentences, its per-window quiet sentence, the matrix card's own empty word and tooltip, the **lens readout** (§4 A7's "one count statement on the page") and search's figure caption. `card.reachMeasured` is now the single producer and six surfaces read it. (5) ⚠️ **The chip separation rule was applied to boxes that are not targets.** `placed` holds the hint corner, the region labels and the chips, and only the chips are controls; a chip resting `BOX_GAP` above the 24px hint corner is 23.1px away and would have been dropped **unconditionally**. Three lenses charged it. `target: true` is a flag on the box for that reason. (6) ⚠️ **The 2.5.8 case moved from Equivalent to Spacing because Equivalent was measured breaking**: with a region focused the strip filters to that region while the chips only re-order, so **two of six chips named places with no card in the dialog**. `MIN_TARGET_SEPARATION_PX` (24) costs nothing on the seeded roster — one pair moved, five chips still drawn. (7) ⚠️ **`inert` is a silent no-op in this jsdom** (`'inert' in HTMLElement.prototype` is `false`) and **React 19 drops the string form** `inert=""` while rendering the boolean — two silent no-ops stacked. Assert the ATTRIBUTE in jsdom; measure the behaviour in a browser. (8) **Measured redraw, paired in one session.** Twelve-step width storm 1280→840: **7 long tasks / 465 ms** without the throttle, **1–2 / 77–146 ms** with it, **0** with the matrix `display: none`. Origin move 1 task / 75 ms; `document.fonts.ready` 0; popup open (the seventh canvas) 0. Budget 1.41 M device px over 7 canvases at DPR 2. (9) ⚠️ **The `useHeatCanvas` mitigation is a LEADING-edge throttle, not the debounce the design's word suggests** — a plain trailing debounce costs 170 ms of blank on a pane revealed from `display: none` (its retry budget is already spent) and delays every one-shot change for a coalesce that was never going to happen. (10) **`geo` survived the component moves and got better**: it is statically imported by the shared `useHeatCanvas` chunk rather than by the strip's, so the matrix and the popup share one copy; the entry's static graph is `conversions, leaflet, locationTypes, planningArea, prop-types, react, scoreRamp` and holds no `geo` (the `geo-*.js` string in the entry is Vite's `__vite__mapDeps` preload table, not an import). (11) ⚠️ **Stepping a window and picking a region announced NOTHING** (SC 4.1.3): both replace the whole dialog while focus stays put, the dialog is not keyed so its accessible name is never re-read, and the `n/6` counter is `aria-hidden`. An always-mounted `role="status"` now carries the window, its place in the six, its verdict and — on a pick — the region and its count. (12) **Two copy findings that need two surfaces on screen at once** — the prose slot's `N of its locations below` printed the same integer the strip's footer printed 250px lower against a different whole (`9 of its locations` vs `9 of 209`, with the served `across 69 locations` between them), and the footer's `· within 45 min` described a filter that had gated nothing. Both fixed; the first is a recorded deviation from this plan's §5. (13) **§6 clause 3 is RETIRED, not passed** — it is entirely about the promoted strip; struck in `window-first-redesign-plan.md` §6 and §6a rather than left to pass vacuously. (14) ⚠️ **`window.matchMedia`, `ResizeObserver` and `requestAnimationFrame` all work in headless Chromium via Playwright** (`chromium.launch()` from `frontend/`, no `executablePath`) — the honest instrument for every claim the pane cannot make. **Observations recorded rather than fixed**, with reasons in §11: the two 403s a LITE session logs are identical on v1; the reach tier stays operable with no postcode (an inert control, not a false claim); `MastheadLight`'s `sr-only` scope label announces the home place a second time in the tick line's own row; and the lens readout is squeezed to 11px of its 416px across a ~390px band of widths. |
| M4 — the location sheet | `feature/plan-matrix-m4-sheet` | `ea641abd` (#596) | 2026-08-21 | Six lenses + a per-charge pass; the browser found two things the lenses could not and refuted a third. **Seven things a later phase should not rediscover.** (1) ⚠️ **`font: inherit` after `line-height` silently zeroes the line-height** — `font` is a shorthand and the CSS-wide keyword sets every longhand, so `.wf-mchip` inherited `.wf-mapbox`'s `line-height: 0` and the plate would have painted 9px tall behind 9px of text. Two lenses caught it independently and `css: false` means no test ever could; the browser measured 16.15px after the reorder. Order the two declarations, always. (2) ⚠️ **An `aria-hidden` span between two text nodes destroys the space between them.** Splitting `◍ Show on map → Tonight Sunset` around hidden glyph spans left no element boundary, and name-from-contents trims each part and joins with nothing: the name computed as `Show on mapTonight Sunset`. Hide the glyph that is *read as a word* (`◎`, `◍`) and leave the separator inside the text node. (3) ⚠️ **`opacity` cannot be undone by a child, so a de-emphasis is a selector list and every exclusion is an omission from it.** The rating chip and the date box are excluded because each carries its own background: dimming the group lightens the plate while it darkens the ink and they converge (2★ chip 4.83→4.15:1, weekday 7.02→3.20:1). The design's `.62` puts the leave-by line at 3.40:1; `.8` is 4.80 at rest and 4.63 on hover. (4) ⚠️ **Three dialogs can stack on the popup and all three were wired as *the* stacked layer** — same `escapeEnabled`, so any two answered one press twice. `openOverPopup` makes them mutually exclusive rather than ordering them, which is the smaller change and avoids the fourth layer's worth of guards. (5) ⚠️ **A handoff to the map must close the popup, not just the sheet.** `MapOverlay` is `aria-modal` with an unconditional Escape listener and `stackedOverPopup` goes false the instant the sheet unmounts, so the popup re-arms its own. The drill-down sheet had the same hole since M2. (6) ⚠️ **`requestAnimationFrame` NEVER FIRES in the browser pane** — its document reports `visibilityState: 'hidden'`, the same class of limitation M3 recorded for Resize/IntersectionObserver. The `Plan from` focus move is therefore jsdom-only verifiable; do not read `activeElement: BODY` in the pane as a defect. (7) **A shared eligibility test may share the verdict and must not share the words**: `originAction` returns flags only, because "you are already planning from here" is unambiguous on a search box's region row and claims the origin is *Bamburgh* under a heading naming a place. **Deviations recorded rather than silent:** the lead block carries one line, not the design's two (the best row is expanded below it); the date box is 52px above 639px and 34 below; the chips meet WCAG 2.5.8 through the **Equivalent** exception (every chipped place is a full-size card in the same dialog) rather than the 24px arm, with the region-focus gap named. **Left for M5 deliberately:** the third `aria-modal` layer is now reachable rather than merely documented, which is A22's adjudication input; and the drill-down sheet's own cards still open the map, which is the one place a `WindowSpotCard` still means what it says by default. ⚠️ **A scripted splice deleted `toOrigin` and only the suite caught it** — grep the export list after any range-based edit of a util. |
| M3 — the tick line | `feature/plan-matrix-m3-tickline` | `34c22bb9` (#594) | 2026-08-21 | Six lenses + verification in a browser; the browser found the phase's worst defect and the lenses found the rest. **Six things a later phase should not rediscover.** (1) **The stuck sentinel never attached, and no jsdom test could see it.** The lens bar is mounted CONDITIONALLY (on a resolved reach lens), so on the shell's first commit the sentinel does not exist; with a plain `useRef` the effect ran once against nothing and never re-ran, because its only dependency was the measured offset, which had already settled. Every test rendered a shell whose bar was present on the first commit — the one shape the defect does not reach. The fix is a state-carrying callback ref, so the node's arrival IS a dependency; the covering test mounts with `reachLens: null` and flips it. (2) **`--wf-mast-h`'s fallback may not be `0`.** `.wf-mast` is z-45 and `.wf-lens` is 20, so a bar resting at `top: 0` sits *underneath* the masthead — the page's only reach and rating controls, and the only way out of a lens that has emptied the plan, become invisible and unclickable. `0` was the correct resting position before M3 and degrades into a hidden control surface after it. Both fallbacks are now browser-measured literals (128/188 desktop, 134/270 phone). ⚠️ **Refuted, 2026-09-05:** the premise was false. `.wf-mast`'s `position: sticky` never took effect at all — its containing block is the shell's `WRAP_MAX_WIDTH` wrapper, only ~46px taller than the band — so the bar never rested under a pinned masthead and `top: 0` was never the hidden-control-surface hazard this point describes. It was the opposite defect: the bar hung a masthead's height down the viewport with cards scrolling through the band above it. `.wf-mast`'s rule and `--wf-mast-h` are deleted, the bar anchors at `top: var(--safe-t)` (identical to `top: 0` wherever no safe-area inset is reported — a sticky element sticks to its scrollport, so the root's own inset padding cannot reach it), and the reservation is the bar plus the ring again (60/136). (3) **`.wf-hc` and `.wf-tab` were never in the `scroll-margin-top` list** — the Plan tab's primary control and its navigation, reserved against nothing since M1. It cost 53.5px then and ~180px once the masthead started sticking, which is what made it visible. (4) **Two accessible names had stopped containing their visible words** (WCAG 2.5.3), both by inheritance: the nudge's label was `Set postcode` under copy reading `Set a postcode…`, and the away origin's named the base town *before* the region where the button draws the region first. A `toContain` on the `aria-label` attribute cannot see either — the covering test is `getByRole` with an exact name plus an order-preserving containment check. (5) **`ResizeObserver` and `IntersectionObserver` are SUSPENDED in the browser pane**, whose document reports `visibilityState: 'hidden'`: a freshly attached RO never delivers even its initial callback. Measured-height tracking and the stuck transition therefore cannot be verified there beyond a first paint — say so rather than claiming it. Both observers now also watch their subjects directly rather than only the shell, so a late-arriving light row cannot leave the measurement a reflow behind. (6) **`heatStripCards` already carries each window's `bestReach`** — the first cut threaded a second `cards` prop and joined on the window key to reach the identical object. **Deviations from the bundle, both recorded as §4 rows:** A22 (search over the popup — two `aria-modal` dialogs, and the anchoring kept) and A23 (`Windows here` rather than `4 DAYS`, because the sheet derives its own span and prints it). **Costs stated, not hidden:** the age is Plan-only now, dims with the pane, and withdraws when the matrix does. Charges refuted and worth not re-raising: the tick line does NOT need the deleted height-holding placeholder (the origin control sets the row's height in all three light states, browser-checked at 390px and 1280px); and `stackedOverPopup` is already the right operand for M4's stacked sheet, since `sheetSpot` is one of its terms. |
| M2 — the window popup | `feature/plan-matrix-m2-popup` | `ae77b9ec` (#590) | 2026-08-21 | Built on M1's branch while M1 was still unpushed — M2 consumes `windowFirstTopics.js` and the card pool it added — then rebased onto `main` once M1 landed as #588. ⚠️ **A scripted edit left a `useMemo` closing without its semicolon, and only CODEQL saw it** — the arm's ESLint config does not enable `semi`, so `npm run lint` is green on automated semicolon insertion; six review lenses missed it too. If a phase rewrites hook closings with a script, grep for `])$` before pushing. Six lenses + a per-charge refutation pass. **Five things a later phase should not rediscover.** (1) The verdict-badge treatment table was TRANSCRIBED out of the deleted card rather than copied, and drifted in four values — one to `--color-badge-marginal`, which `@theme static` does not define, so every "Maybe" badge rendered in inherited bone under a green suite. Two tests now guard it: the salvaged per-verdict ink assertions, and one that walks every `var(--…)` the component emits against the tokens the stylesheet defines. **jsdom does not resolve `var()`, so no cascade test can ever see an undefined token** — that second test is the only shape that can. (2) The chips are an ORDERING over the gated pool, never a filter by the focused region: built from the region-gated array, a pick erased every other region's names and the picture stopped being a comparison. (3) The popup must not print a confidence *word* where `buildWindowCards` nulls `confidence` on purpose (STAND_DOWN/AWAITING) — `resolveConfidence` infers from the horizon, so it will happily produce one, beside a badge the same line refuses to decay. (4) `applyConflictAction`'s focus move needs `button[data-testid="wf-heat-card"]`: an away day keeps its matrix cell as a non-focusable `<div>` and `querySelector` returns DOM order. (5) The dialog is NOT gated on the heat catalogue — only its field map is; gating the dialog made every matrix cell a control with no visible effect while `/api/locations` was in flight. **Deviations from the bundle, both measured:** the focused region's own label is no longer drawn (rail + prose already name it, and the space is what the chips need — 0 chips before, 3 after on a 390px phone), and the popup's single-column breakpoint is 639px rather than the bundle's 767, so the layout and the map's phone aspect band switch together. **Deferred with reasons, not silently:** the chips' `title` (region · drive · leave-by) lands at M4 with the click that makes it reachable — a `title` on a `pointer-events: none` span inside an `aria-hidden` subtree reaches nobody; `escapeEnabled`'s `searchSeed` arm is dormant until M3 puts search in the masthead, since `/` is presently refused while any dialog is open; and `buildPaneItems`' away payload (`label`/`note`/`windowCount`) now has no reader — M5 owns it, since the whole module is down to a `length === 0` check once the promoted strip goes. Charges refuted and worth not re-raising: the shared `Modal` does focus-in and restore rather than containment **by settled decision** (`useDialogFocus` records why), so "no focus trap" is not a gap; and `strip.adjacent` survives as a derivation because §7 deletes `windowFirstPromoted.js` wholesale at M5. |
| M1 — the matrix | `feature/plan-matrix-m1-matrix` | `5273a5d5` (#588) | 2026-08-20 | Six lenses + a per-charge refutation pass; the survivors are folded in and the CHANGELOG entry names them. **Five things a later phase should not rediscover.** (1) The card face is `overflow: visible` for the pick legend, so nothing inside it clips itself any more — the canvas well and the topic labels each carry their own clip, and anything new with a `nowrap` or an inline width needs one too. (2) `.wf-hc-pls > span` is a `(0,1,1)` selector: an override on a value-grid child must be written `.wf-hc-pls > .x`, or it silently loses. (3) `button.wf-hc:hover` is `(0,2,1)` and out-specifies every `.wf-hc.x` state rule — the pick and open-state borders each carry their own `:hover` arm, and a new state rule must too. jsdom cannot see any of this: it computes a selector list's specificity as the max over the list, so a sliced-cascade test here answers the *opposite* and passes. (4) The topic scope filter fails **open**, so `windowFirstTopics.js`'s two sets must together cover `TopicRarity.RANK_BY_TYPE`'s sixteen types — the test holds its own literal copy, and both have to move together. (5) Measured redraw: a full matrix repaint is 60–210 ms (median ~97 ms) over six canvases at DPR 2, against zero long tasks with the matrix hidden. The mitigation is a debounce on `useHeatCanvas`'s observer nonce, which the Map pane shares — M5 task 5. **Deviations from the bundle, both for measured contrast:** the best-reach rating is a filled `spotBadgeStyle` chip rather than bare ramp-coloured text, and the histogram sits in a `--color-heat-sea` well. **One deviation for SC 1.4.1:** today's column says the word "Today" in the weekday slot, because the bundle's reason for omitting it (the cards carry the day in their own labels) stopped being true when the card face changed. Charge refuted and worth not re-raising: the A8 scope filter DOES run at home — the bundle's own `HOME` origin is "every region in your area" at the same 180-minute rule — and `.wf-shell`'s `max-width: 1080px` bounds the widest a card can ever get, so there is no runaway-card case at 2560px. |
...
| A26 | *(added 2026-08-30, after v2.19.9 — this row amends A8 rather than sitting beside it)* Tide topics bucket by **type** onto **both** windows of their date, ahead of the `eventType` switch | `PlanWindowProjector.keysFor` tests `isDayScoped(topic.type())` first; the client mirrors it with `DAY_SCOPED_TOPIC_TYPES`, pinned to the backend predicate by a parsing drift canary in `windowFirstTopics.test.js`. The badge is built **per window key** (`badgeFor`), so the non-aligned window carries its own nearest water — read from the served `BriefingWindowTide`, never re-derived | ⚠️ **This is a deliberate departure from A8 on two counts, recorded because A8's value depends on being the complete list.** (1) A8 says "key on the served `eventType` field, **never a client list of types**" — this adds exactly such a list. It is debt with a named exit: `isDayScoped` should be a **served field** on `HotTopic`, written by `HotTopicEventEnricher` (which already owns this decision and already special-cases these two types), following `TopicRarity`'s precedent that "the client reads it off the wire". That would delete the client set and the canary together. (2) The design bundle is **not** authority for the two-window placement — its `WTOPICS` mock puts `ktide` on one window per day. The bundle *is* authority for the per-window **detail** (`{t, d}`), which is what `badgeFor` restores. Reason for the change: placing a day-level claim by a window-level fact left evening cards on a spring run showing no tide, and blanked a tidal day entirely once its alignment had passed |
...
| A22 | (M3's review, on A14) The search dropdown replaces the tick line | **Kept anchored. ⚠️ RESOLVED AT M5 — read the last paragraph of this cell first, because the two before it record the state M5 replaced.** The panel is positioned exactly over `.wf-tick` and covers it — `Modal` is `z-50` and `.wf-mast` is 45, so the covering is what the stacking order already does and nothing has to be hidden underneath. Two consequences are accepted rather than fixed: over an open window popup there are **two `aria-modal` dialogs**, and the lower one is not marked inert (the shared `Modal` has no containment by settled decision, and M5 owns that adjudication); and the panel's square top corners meet the light rule's 2px-radius bottom corners rather than a straight border, leaving a 2px sliver of gradient at each junction. What IS fixed is the focus consequence — the three controls the panel covers leave the tab order while it is open (`searchOpen`), because WCAG 2.4.11 is a real failure where the centred box this replaced only dimmed them. **⚠️ M5's ruling, and it went both ways.** The two-modal state is no longer accepted: `Modal` takes a `stacked` opt-in, and a covered layer goes `inert` and drops `aria-modal`, so exactly one element on the page claims to be the modal (measured before: a Tab walk out of the topmost sheet reached the masthead's search button on press 17 and was inside the popup underneath by press 34). And the THIRD layer is refused outright rather than ordered — search over a sheet over the popup was reachable only through the masthead button, which was bypassing the guard the `/` shortcut had carried since M3, and it rendered broken: every `Modal` is `fixed inset-0 z-50`, so paint order is DOM order and the sheet painted its scrim and its whole card over the search panel. The supported stack is two deep: search over the popup, or a sheet over the popup, never both. The `escapeEnabled` predicate and the `stacked` one are the same predicate for that reason, derived once in each dialog so they cannot come apart | A14 required M3's review to decide this rather than let it pass silently. Anchoring is what the design asks for and what makes the box read as chrome rather than as a card floating over a dialog; the alternative (a centred box over the popup) loses that for no accessibility gain, since the two-modal problem is identical either way. |
| A24 | (M4) The sheet's lead block is a kicker over a paragraph of the best window's prose | **The kicker alone.** The gold wash and the mono uppercase treatment are built; the second line is not | The best row arrives EXPANDED directly beneath it, so the paragraph would be the same sentence twice about 200px apart — and quality-said-four-times is the disease §9.3 names as what this redesign cures. P8 recorded the same refusal when it built the line. |
| A25 | (M4) Location-sheet rows at 2★ or below get `opacity: .62` on the row | `opacity: .8` on the row's TEXT children only, and never on the row the sheet leads with | Measured. `.62` puts the row's own 10px departure line at 3.40:1, a straight 1.4.3 failure on the line a reader acts on; `.8` is 4.80 at rest and 4.63 under `:hover`. The rating chip and the date box are excluded because each carries its own background, so dimming the group converges ink onto plate (2★ chip 4.15:1, weekday 3.20:1) — and the chip is the QUALITY signal, which CLAUDE.md is explicit is never dimmed by another channel. The best row is excluded because it arrives expanded under a gold border and a `◎ best here` tag: three treatments arguing on one row is not a signal, and it is also this rule's tightest measured point (4.38:1). |
| A23 | Location result chips read `4 DAYS` | `Next few days` | The sheet it opens derives its own span and prints it — six rendered events fall across **three** days whenever today still has both its windows ahead — so a fixed `4` in the chip would be a number nobody measured sitting beside the same number measured. Rule 5's family. ⚠️ **This cell said `Windows here` until 2026-09-08, and the code has read `Next few days` since the row was written** — the reasoning survived into `planSearch.js`'s comment verbatim while the string moved on, so a reader checking the chip against this row found neither the bundle's answer nor the code's. Corrected by the audit of `docs/design/plan-matrix/README.md`; the same audit found the window chip reads `Open`, not the bundle's `OPEN WINDOW`. |
...
3. Accessibility pass: axe on the four surfaces, screen-reader walk of matrix → popup →
   ...
   traversal, focus-visible on the new controls. **Three things M4 hands this task by
   name.** (a) The third `aria-modal` layer is now *reachable* rather than merely
   documented — search over the location sheet over the popup, via Tab out of a
   non-trapping dialog onto the masthead's search button — which is A22's adjudication
   input and the one place M4 widened the surface it was told not to deepen. (b) The field
   chips put six or eight duplicate tab stops into the tree, named as a group but in rating
   order over a picture the reader cannot see; their 2.5.8 case rests on the **Equivalent**
   exception, whose one gap is a live region focus (it filters the strip but only orders
   the chips). (c) The drill-down sheet's own cards still open the map while the popup's
   open the sheet — one component, two destinations, and only the wording tells them
   apart.
...
| M2 | Card list **rendering only** (`WindowFirstWindowCard` list role, `WindowAwayRow` — the `buildPaneItems` derivation survives as matrix + promoted-strip input), accordion + `WindowRowRegionLayer` inline mount, Order control (`usePlanOrder`, `windowFirstOrder.js`, lens segment, order note), per-card `lensEmpty` (`windowLensEmpty.js`), the promoted strip's scroll-specific `adjacent` suppression | Popup dialog; page-level conflicts + per-window quiet sentence |
| M2 (as built, beyond the row above) | `WindowRegionBand` — it was the inline mount's third piece and became the prose slot (`WindowProseSlot`); the **snow row promotion** in `buildWindowRows` and the second badge list it existed to reduce (`card.badges`, `badgeKey`, `MAX_ROWS`, `WindowAttributeRow`'s snow arm, `.wf-frow-snow`); the focused region's field label and its `[data-hot]` ring | `WindowTopicRows` states each topic once, facts included; the rail cell and the prose heading name the focused region |
| M2 (kept, on a review call) | `WindowSpotPeek`/`useSpotPeek` — the popup shows this window's prose but not a spot's, so the hover peek still answers a question nothing else does. It is suppressed only by a layer stacked OVER the popup, never by the popup itself | — |
| M3 | Rail footer: `PlanOriginChip`, home-not-set line, Edit-reach link, forecast-age line | Tick line; age beside change line |
| M4 (as built — nothing dies) | Two things are *replaced in place*: the popup's spot-card and field-chip handoffs to the map (both now open the location sheet, and the map moved into that sheet's footer), and `originAction`'s reason strings (the shared thing is the verdict; each surface words its own) | The location sheet, stacked over the popup |
| M5 | (D-1, resolved) `WindowFirstPromotedStrip` + `windowFirstPromoted.js` + strip wiring in the shell | Topics on cards + popup rows |
| M5 (as built, beyond the row above) | The dead weight the strip was the last reader of: `buildPaneItems`' away payload (`label`, `note`, `windowCount`) with `awayDateLabel`, `noteFor` and the `travelRanges` parameter behind them, and `card.topRarityRank` — `BriefingWindow` still sends it and every surface that orders topics ranks the BADGES on their own `rarityRank`, so a window-level minimum kept alive beside them is a second answer waiting to disagree. `windowFirstAway.js` itself SURVIVES: one item per rendered thing in date order is the empty-state line's denominator, and a travel day has no card, so the card list cannot answer it. Also the prose slot's `N of its locations below` chip — a **deviation from §5**, measured and argued in `WindowProseSlot`'s header | The matrix cell's own away treatment (a div, never a button); the strip footer's `N of M` plus its active-filter clause, which names the region |
...
follows is the honest residue — stated so the owner is choosing with it in view rather than
discovering it after the flip.

⚠️ **"The suite is green" needed a change to stay true under load, and the change is worth knowing.**
An adversarial review reproduced three failures on a full parallel run that passed in isolation —
all of them `await screen.findByTestId('wf-heat-strip')`, a wait on a real `React.lazy` boundary
rather than on a fetch a test could gate, timing out at Testing Library's 1000 ms default. Twenty-five
tests across six files begin that way. `src/test/setup.js` now sets `asyncUtilTimeout: 4000`, which is
a ceiling rather than a delay (a resolved boundary still returns immediately). It is the honest fix
...
| register | what it covers here |
|---|---|
| **Measured in a real browser** (headless Chromium, `visibilityState: 'visible'`) | The dialog stack (`inert`, `aria-modal`, the Tab walk, focus restore); the redraw storm and the origin move; the chip geometry; axe on four surfaces × two widths; the phone transpose and full-screen popup at 390px; the LITE and no-postcode sessions; the `geo` chunk boundary |
| **Asserted in jsdom** | Every attribute-level consequence of the above — `inert` and `aria-modal` presence, the throttle's leading/trailing/coalescing behaviour under fake timers, the chip separation rule, the copy deletions. `inert` is a silent no-op in this jsdom, so its BEHAVIOUR is browser-only by construction |
| **Adversarially reviewed** | Seven prosecutor lenses over the whole v2 surface — runtime, CSS/tokens, test quality, accessibility, plan compliance, deletion completeness and copy — read-only, each with §3–§6 pasted in. Their surviving charges are folded into the phase row above and were each re-measured before being acted on; three were refuted by measurement (the search panel's *pointer* occlusion, which `inert` already prevents; the `useHeatCanvas` timer leak, which the cleanup does prevent; the `capSpots` fixture change, which was legitimate) |
| **⚠️ NOT established** | **A screen reader was never run.** The accessibility work here is the computed accessibility tree, real focus order and axe in a real browser — which is what an AT reads *from*, and is strictly more than any previous phase managed — but it is not VoiceOver, NVDA or JAWS, and browse-mode behaviour across two `aria-modal` siblings is exactly the kind of thing implementations differ on. The remedy chosen removes the ambiguity rather than relying on how it is resolved, which is why it is defensible without the walk; a real-AT pass remains the one accessibility claim this series cannot make |

### 11b. Not covered by the local verification, and why

The seeded local database has **no hot topics and no served picks**, so three card surfaces were
exercised only by their unit suites: the matrix cards' topic chips and the popup's topic rows (the
A8 join), the `BEST BET`/`ALSO GOOD` legends, and the tide row. `HotTopicStrip` behind the doors is
in the same position. These are the richest states on the page and the ones a pilot user is most
likely to meet on an interesting night.

### 11c. Standing items the flip inherits, none of them this series'

- **Two 403s per LITE session** (`GET /api/nlc/sighting`, `GET /api/aurora/status`), logged to the
  console on **both** arms — measured identically under `v1`, so this predates the series. Nothing
  visible fails; the surfaces degrade to silence.
- **The reach tier stays operable with no home postcode**, gating nothing (an unmeasured drive time
  passes every tier by design — §2.5, absence means unknown). M5 fixed the false *claim* that came
  with it (the popup's footer no longer names a tier that gated nothing) and deliberately did not
  disable the control: §6 clause 7's first half ("no control gates on data that does not exist")
  is a real question, the tier is day-stamped and starts working the moment a postcode is saved,
  and the tick line's nudge is the designed route to saving one. An owner call, not a defect.
- **`MastheadLight`'s `sr-only` scope label** announces the home place a second time inside the tick
  line's own row (once in the origin button's name, once as the times' scope). Left alone: dropping
  it would leave the times unscoped, which is worse.
- **The two reach-scoped client derivations** (spread histogram, best-reachable line) remain
  per-user debt with a named exit — §8's optional **O-4**. Recorded in CLAUDE.md's backend-heavy
  bullet as a class rather than a precedent.
- **⚠️ M5 task 3(b)'s SECOND half — the chips' duplicate tab stops — is adjudicated ACCEPTED, and
  the reason is a conflict rather than a cost.** Six to eight chips sit in the tab order naming
  places the ranked strip below names again with more detail, and the spatial meaning that justifies
  them (the canvas, the region labels) is `aria-hidden`, so a screen reader gets the same places
  twice and no way past them. The remedy that fits is a roving tabindex — one tab stop, arrows
  within — and the popup **already binds `←`/`→` to step the window**, which is the reader's primary
  navigation inside that dialog. Taking those keys for the chip layer would trade a traversal cost
  for a collision on the more important control. The proportionate alternatives, both for the owner:
  a skip control before the layer, or making the chips non-focusable again and accepting that the
  map is pointer-only (which M4's D-3 deliberately reversed). The 2.5.8 half of that hand-off IS
  resolved — see the phase row.
- **M5 task 3(c) — one component, two destinations — is adjudicated ACCEPTED.** The popup's spot
  cards open a location sheet; the same `WindowSpotCard` inside the "See all" drill-down opens the
  map. The difference is on the control itself, not inferred: `◇ The next few days here →` against
  `◍ Open on map →`, different glyph, different verb, different noun, drawn on every card. Unifying
  them would either destroy the browsing position (`openOverPopup` makes the stacked layers mutually
  exclusive, so a location sheet would REPLACE the "See all" list with no route back) or need a
  fourth `aria-modal` layer — which M4 was told not to add and which M5 has just spent its whole
  accessibility budget reducing.
- **Two accessibility residuals recorded rather than fixed, both with reasons.** (a) The spread
  histogram's per-band counts live only in a `title` on a span inside an `aria-hidden` subtree, so
  they reach nobody — the card's hidden sentence carries the pool total and the unrated remainder,
  which are the two figures a reader acts on, and adding five band counts to six accessible names
  costs every AT reader on every tab. (b) `PlanSearch`'s `role="status"` fires on every keystroke
  with no debounce, so typing eight characters queues eight counts over the option announcements;
  the always-mounted decision is right and the cadence is not.
- **⚠️ The lens readout is squeezed to illegibility across a ~390px band of widths, and M5 measured
  it rather than changing it.** `.wf-lens-res` carries `flex: 1 1 0; min-width: 0` so that the
  READOUT, never a control, is what gives way when the bar runs out of room — a deliberate,
  documented M2 trade ("a control the reader cannot see or reach is worse than a bar that is 92px
  tall for a while"). What that note does not record is how far it gives way. Measured at M5 in a
  browser, resting state, admin session: the string wants **416px**, and it gets 391px at 1080,
  211px at 900, 145px at 834, 111px at 800, 79px at 768 and **11px at 700** — a bare ellipsis at
  iPad-portrait-ish widths, recovering to 365px at 660 once the bar wraps. So on a tablet the page's
  ONE count statement (§4 A7) is effectively absent for sighted readers. It is not a *false* claim —
  the whole string stays in the DOM and a screen reader reads all of it — which is why this is
  reported rather than fixed: the remedy is to switch to the phone's count-only variant well above
  the phone breakpoint, and re-litigating a measured flex decision at the final gate, with no room
  left to browser-verify the alternative, is the riskier move. An owner call before the flip.
- **⚠️ The "exactly one modal" property is held route by route, and a new route can break it.** M5
  ...
  the popup live under an `aria-modal` `MapOverlay`). ⚠️ **"The three that existed" was three of
  four, and the cog's close was not whole either** (found 2026-09-16): the tick line's postcode
  ...
  `searchSeed` and which the cog is Tab-reachable from. Both now run the shell's
  `yieldToForeignDialog`, and so does the Map tab's ⌂, a later route O-18 made
  (`map-tab-v2-plan.md` O-20). The ⌂ is reached through `App` reporting the dialog to the shell
  (`settingsOpen`), which covers any route into settings; beyond that one dialog, nothing enforces
  it structurally. The
  ...
  topmost dialog, which is a behaviour change to every v2 dialog and a reversal of
  `useDialogFocus`'s settled app-wide decision — hence not at the last moment.
- **Residual from that**: from an open dialog a keyboard reader can still Tab onto the page and
  activate a matrix card, which mutates a popup two layers down that is `inert` and invisible to
  their AT — plan §3 rule 14's own ban, arrived at through the non-trap rather than through a
  component. Pre-M5 it left a reachable popup; now it leaves nothing perceivable.
- **Residual from the lazy-chunk fix**: warming removes the no-live-layer window on every route that
  exists, because a chip, a spot card, a pick badge and `/` are all reachable only from an open
  popup. A reader who opens a window and clicks a chip inside the same few hundred milliseconds on a
...
- ✅ **The v1 deletion** is the next piece of work and is not part of this series. **D0–D4 done
  ...
  deletion would sweep them. Note also that the v2 kill switch dies with v1: `PlanLayoutErrorBoundary`
  recovers by setting `PLAN_V1`, and `usePlanLayout` falls back to it in two more places, so §10's
  "the flag is a page-level kill switch" stops being true at that point. It takes with it the
  stale `DailyBriefing.jsx` aurora-grid comment, `computeAuroraCellTier`, and v1's byte-identical
  copies of the roll-up and the cell derivation.
