...
**Status: ✅ CODE-COMPLETE — all six phases (M1–M6) landed.** Phase log
...
|---|---|---|---|---|
| M0 | `docs/map-mobile-sheet-plan` | [#923](https://github.com/gregochr/goldenhour/pull/923) | 2026-09-25 | Vendored the bundle; this plan; `map-mobile-sheet-prompts.md`. No code. |
| M1 | `feature/map-mobile-sheet-m1-quiet-start` | #924 | 2026-09-26 | Landing card never opens on phone (`landingOpen` gated `!isMobile`, `mapLandingSeenRun` never written, `↺ Back to …` row withheld); the tab's scored-locations toast moves to a fixed `top: 62px` and fades (`wf-map-scored-legend-gone`) 3 s after its `\|date\|eventType\|` key last changes, reduced-motion-safe; retired the conflicting `.wf-tide-strip-on` `bottom` override for the toast that M1's fixed-`top` rule would otherwise have fought for the cascade. **Adversarial review (five read-only lenses, run by the orchestrator after the session's own review stalled at one): three confirmed findings, all fixed before the commit landed** — (1) `openDrilldown` calls `dismissLanding` unconditionally and the pill menu's drilldown row renders on every viewport, so the phone COULD write `mapLandingSeenRun` (§5 D-3 broken by a route the session's tests never pressed); `dismissLanding` now returns at once on the phone and a test presses `wf-win-more`; (2) the fade timer was keyed on the window alone, so a toast whose gate opened > 3 s after mount (a slow briefing load) was born with the class applied — the effect now keys on a single `toastShown` gate shared with the render, and a rerender-after-5-s test pins it; (3) an unmount-before-3-s cleanup test was missing. **Browser (390 × 844 and 1280 × 900, §9 fixture with a Worth-it tonight and a Poor tomorrow morning, SEEN AND MEASURED)**: phone — no `.wf-land`, `mapLandingSeenRun` null after the tab mount, toast at `top` 62 px in-frame, centred to the pixel, one line 26.5 px tall (it had wrapped to three lines, 43 px — fixed with the design's own `white-space: nowrap`, pinned in the cascade test), opacity 0 with `aria-hidden` and `pointer-events: none` after 3 s; desktop — landing card open (`Tonight, or tomorrow?`), toast standing bottom-right (8 px / 54 px gaps), no class, no fade. |
| M2 | `feature/map-mobile-sheet-m2-sheet` | #927 (opened after #926 was auto-closed when its stacked base branch was deleted — see §8's orchestration note) | 2026-09-26 | New `components/map/MapPeekSheet.jsx` (in-frame, backdrop-free, `data-testid="wf-map-peek"`, z-index 1120) replaces the phone's floating pill dropdown, Regions/Heat-Pins/Filters bar and tide strip with one collapsed (74px) / open (356px, clamped `calc(100% - 64px)`) sheet. `WindowControl` gains `pillOverride` (drops `aria-haspopup="listbox"`, points `aria-controls` at `wf-map-peek-body`) so the phone pill opens the Windows section instead of the listbox; `KindChip` exported for reuse. Windows section: heading `landingCardModel().header` (never fixed copy), the pill's own roster with night rows showing `{bestRating}★ best`, the drilldown row last (collapses the sheet by `openMapMenu` exclusivity). Layers section: Show/Regions/Filters/Legend; `RegionsJump`/`FiltersPopover` gain `chipHidden` + `restoreFallback` (threaded to `BottomSheet`/`useDialogFocus`) so their sheet stays reachable and closing one returns focus to the Layers button. `SheetDismissOnMapTouch` (a `useMapEvents` child, not `useOutsideDismiss`) collapses on any map touch; a second effect keyed on `selectedLocationName` collapses on any installed selection (chip/pin/handoff all write it directly); a peek press clears the selection FIRST (D-7). Lifted stack rebuilt off the sheet's 74px floor (attribution `padding-bottom: 74px`, chrome-bl `calc(74px + 27px + 8px)`); counts footer unconditionally `sr-only`-clipped (no phone home, §4 #10); `MapCallout`'s phone band reads a fixed `--psh: 74px` custom property rather than measuring a live rect. `mapPhoneChromeCascade.test.jsx` rewritten for the two-row stack; `mapChromeZLadderCascade.test.jsx` gains the 1120 rung. **Adversarial review (four read-only lenses): five real findings, all fixed before the commit landed** — a stale doc comment claiming the phone tide strip sat at z-index 1120 (it is 1100, chrome-tier, unchanged by the phone media query) corrected; a `MapPeekWindowsSection` test missing the sibling-row negative assertion; no test proving the Tide button is absent in M2 (task 2's "never an empty panel" rule); no test proving `aria-expanded` tracks the same pill through a real open→close cycle; no test exercising D-7's reverse-order clause — all closed. Accessibility lens measured the peek row's 8.5px key labels at **≈7.1:1** contrast against the sheet's background (`--color-plex-text-secondary` over the `.97`-opaque sheet bg over the app bg) — clears WCAG AA for small text (4.5:1) comfortably; §6 Q5 closed. **Browser (390 × 844 and desktop-wide, SEEN AND MEASURED)**: phone — sheet 74px collapsed / 356px open exactly; pill `aria-haspopup` absent, `aria-controls="wf-map-peek-body"`; Windows section shows the served header and per-tier verdict colours; Layers → Regions opens the real `BottomSheet` with the peek sheet gone, both a real Escape and the ✕ button return focus to the Layers peek button (verified with real clicks/keypresses, not synthetic `.click()` calls, which behaved differently); a real mouse drag on the map collapses an open section; the "Other windows" verdict span computed `flex: none`. Desktop — landing card, the Regions/Heat-Pins/Filters bar, Legend chip and counts footer render exactly as before, no `.wf-map-peek` anywhere. |
| M4 | `feature/map-mobile-sheet-m4-tide-mode-setting` | #925 (follow-up for two Codex retrospective findings: #929) | 2026-09-26 | Tide mode persisted setting: `V155__user_map_tide_mode.sql`, `MapTideModeRequest`, `UserSettingsService.saveMapTideMode`, `PUT /api/user/settings/map-tide-mode`, `useReaderSettings`' `mapTideMode`/`saveTideMode` (its own save line, reusing `colourSaveQueue.js`'s mechanics unchanged), threaded `App` → `WindowFirstMapPane` → `MapView` as unread props. No UI. Migration proven only in CI (no local Docker). **Adversarial review (three read-only lenses run by the orchestrator plus the session's own late reviewer): backend clean; two findings fixed before landing** — (1) `saveTideMode` claimed the hook's answer-order slot at PRESS time, the one participant that did (every other save is numbered when it lands), so a press during a slow `getSettings()` made that whole mount read fail its own `claim` and be dropped — home, colour and last-seen date with it; the claim now happens in `onSaved`, the landed mode is shown again in case a read landed in between, and a held-read test pins it; (2) `App.test.jsx`'s null → `'auto'` case waited on a call already made and asserted the pre-load default, so it passed with the mapping deleted — it now gates on the same read changing `mapColourScale` off its bootstrap. Migration pending CI. **Codex retrospective on #925 (after the limit reset), two findings fixed in a follow-up**: a tide save landing BEFORE the mount read still dropped the read whole (the landing-time claim was in the shared order), and an older choice landing or failing after a newer press put the older mode back — the tide now has an order of its own (`tideOrder`: presses and landings), never touching the answers' order, and only the newest press moves what is shown. |
| M3 | `feature/map-mobile-sheet-m3-tide-section` | #928 | 2026-09-26 | `MapTideStrip.jsx` split into exported `TideStripHeader`/`TideDayChart`/`TideStripFooter` the desktop strip keeps composing unchanged, pinned by a one-off `outerHTML` golden-string test (captured pre-split, to be deleted at M6); `TideDayChart` gains a `tall` viewBox (92 vs 32, a provable `× (32/32)` no-op at the default) the new `components/map/MapPeekTideSection.jsx` reuses, alongside `TideStripFooter` unchanged, for the sheet's own vertical layout. `MapPeekSheet` mounts the Tide button (fixed key `TIDE AT THIS LIGHT`, never `CLOSE`) gated on `stripModel.visible`, value from new `utils/mapPeek.js#tideSummary` (served state word, an arrow only on `MID`, `· N dim` omitted at zero). `MapView` owns the close-and-rescue: because the component's early "no forecast data" return sits between where hooks must run and where `tideStripModel` is computed, a `useRef` hand-off (`tideCloseGateRef`, written fresh every render by a plain assignment after the return, read by a no-dep effect declared before it) closes the section and rescues focus to Other windows when the Tide button's own gate goes false while open — M5 reuses this unchanged, swapping only the gate's source. Phone `MapTideStrip` mount and its `--tsh`-driven phone CSS retired outright; `--tsh`/`wf-tide-strip-on` are desktop/tablet-only from here. **Adversarial review (three read-only lenses): no findings** — one lens re-derived the `yForAt` floating-point identity by hand and confirmed the desktop strip's DOM/CSS/focus-order are provably unchanged; one lens checked every new computation against CLAUDE.md's Backend-heavy rule and found each licensed; one lens verified the ref-hand-off effect is race/loop-safe, every `MapPeekSheet` prop matches its PropTypes, the chart stays wholly `aria-hidden`, and `eslint --max-warnings 0` passes clean. **Browser (390 × 844 and 1280 wide, seeded with real coastal `tide_extreme` rows for four locations across two wanted-water groups, SEEN AND MEASURED)**: phone — Tide button reads `~Mid ↑ · 4 dim` on tonight's Worth-it sunset; the section matches screenshot 03's structure; stepping to tomorrow's Poor sunrise updates the section without hiding the button (M5's own rule); a map touch collapses it. Desktop at 1280 — strip unchanged, no `.wf-map-peek`. The seeded tide's phase never produced a "jump" case (only "beyond") within its 4-day horizon in this session's fixture — that path is TESTED at the unit level, not seen live. |
| M5 | `feature/map-mobile-sheet-m5-tide-rule` | #930 | 2026-09-26 | `utils/mapPeek.js#tideVisible({mode, tideAvailable, hasCoastalInView, tier})` — the pure Auto/Always/Off rule, exhaustively tested (3×2×2×5). `mapTideFit.js#coastalInView(spots, bounds)` lifted out of `stripModel` into its own export, called a second time on the map's UNDECORATED spot list (before `tideTier` exists) to avoid the `labelSpots`/`tideCuesOn` cycle a Codex finding on the plan's M0 named — the block computing `mapEvents`/`evVerdicts`/`activeMapEvent` moved earlier in `MapView`'s render for exactly this reason. The single `tideTier` null (§1 #6) now reads `tideCuesOn ? tierOf(tide) : null`; `tideCuesOn = !isMobile \|\| tideVisible(...)`, so desktop/tablet are provably untouched. A one-shot pulse (`.wf-map-peek-btn-pulse`, box-shadow sweep, 1.2s, cleared on `animationend`) plays on a false → true transition, armed only after the first bounds-backed evaluation. The Layers section gains an Auto/Always/Off `.wf-seg` row wired to M4's `mapTideMode`/`saveTideMode`, announcing a save failure through the pane's one `role="status"` region. **Codex retrospective findings on #927/#928, fixed here** (both land in code this phase already edits, per the coordinator's decision): (1) #927 — the Layers Show (Heat\|Pins) row rendered unconditionally on phone; now gated on `heatOffered`, matching the desktop toolbar cluster. (2) #928 — the close-and-rescue effect compared `document.activeElement` from inside a `useEffect`, which runs strictly after the commit that unmounts the focused Tide button, so `document.activeElement` had already reset to `<body>` and the ref had already nulled — the rescue could never fire (proven with a plain jsdom `removeChild`); fixed by tracking focus via the Tide button's own `focus`/`blur` events into a `tideButtonHadFocusRef` (no `blur` fires on removal, so `true` survives the unmount), read by the rescue effect instead of the timing-losing comparison. **A third finding from this phase's own adversarial review** (not Codex): Always mode can open the Tide section with nothing coastal in the padded viewport, which fell through to `footerModel`'s shared "no coastal spot here has its water on this light" sentence — ambiguous with a different, desktop-only meaning of the same text; `MapPeekTideSection` now checks `model.visible` itself and states the distinct, honest reason. **Adversarial review (four read-only lenses)**: one real finding — the coastalInView/stripModel divergence above (fixed); one CSS-comment citation error (`CLAUDE.md's Backend-heavy §4 #9` doesn't exist; corrected to `map-mobile-sheet-plan.md §4 #9`, the rule this file already cites elsewhere); one stale-doc note (two comments still said "declared below the early return" after the block moved above it — corrected); one test-comment inconsistency (a since-removed `MapPeekSheet.test.jsx` animationend test was still cited by name — corrected to describe why it was removed). **Browser (390 × 844, the §9 fixture's real Worth-it sunset / Poor sunrise / coastal tide data, SEEN AND MEASURED)**: stepping to the Poor window removes the Tide button and every `data-tide` glyph on the coastal chips; zooming to an inland view removes the button on the still-Worth-it window (the dim count visibly narrows 4 → 2 → 0 as the padded viewport shrinks) and zooming back restores it; the Layers Tide row renders Auto-selected with the exact hint copy. Desktop is unchanged: landing card, standing tide strip and its footer render as before, no `.wf-map-peek`. NOT seen live: the pulse's mid-flight frame (tool round-trip latency exceeds 1.2s; TESTED, not photographed) and the Always/Off segment's live persisted effect — the shared local backend on :8083 answered `PUT /api/user/settings/map-tide-mode` with `500` (H2 introspection: that running instance's `app_user` table has no `map_tide_mode` column despite the source's V155 migration and entity both declaring it — the process needs restarting to pick up schema changes made since it last started; this session's rules withheld permission to restart or alter the shared backend). The FAILURE path was genuinely exercised live and worked: the segment stayed on Auto and the pane's status region read "Could not save tide mode — kept the last saved choice." — TESTED but not SEEN: the success/persistence path. |
| M6 | `feature/map-mobile-sheet-m6-sweep` | — | 2026-09-26 | Docs-only sweep. §1's line-number citations re-verified against the tree (`MapView.jsx` is 6,729 lines now, not the ~6,100 §1 opened with) and corrected item by item; four items (§1 #1, #3, #4, #6) had not just moved but been semantically overtaken by M1/M2/M3/M5 themselves (the phone chrome-tr bar is now `!isMobile`-gated JSX rather than CSS-lifted, its old media-query rule is dead CSS, the landing card's own doc comment now states the opposite of what §1 quoted, the toast already carries the fixed-`top`/fade rule, and `tideTier`'s null already reads `tideCuesOn` rather than a bare `tierOf(tide)`) — each flagged in place rather than silently renumbered, so §1 does not assert something the code no longer does. M3's one-off `outerHTML` golden-string pin on `MapTideStrip.test.jsx` (added to prove the header/chart/footer split changed nothing about the desktop strip's DOM) deleted as planned, with its three in-source doc-comment cross-references (`MapTideStrip.jsx` lines 44, 64, 176) reworded from present to past tense rather than left dangling on a test that no longer exists. CLAUDE.md gained a **Map tab on a phone — the peek sheet** bullet under *What's Built*; the Backend-heavy bullet's three §4 #11 client reads (`otherWindow`, `tideSummary`, the `eventType`→AM/PM word) were added as members of the *already-licensed* filter/map/select class rather than an eighth numbered one; the Map tab (v2) bullet's stale "Phone layout moves Regions/Heat-Pins/Filters into a bottom bar" sentence and the tide-window paragraph's phone-mount description were both rewritten to say the phone strip is retired and why; the `PUT /api/user/settings/map-tide-mode` API-doc parenthetical ("no frontend caller yet, wired but unread until M5") — found stale in the same sweep, since M5 wired it to the Layers Tide segment — was corrected too. `map-tab-v2-plan.md` §6 O-20, `tide-window-plan.md` §0 and `map-landing-plan.md` §0 each gained the one cross-reference note §3 M6 task 4 named. §6 Q1–Q6 answered against what shipped (below). **Adversarial review (two read-only lenses, hunting specifically for false statements in the newly-written docs — the map-landing L7 lesson that most defects in a sweep phase are in the docs just written)**: findings and fixes recorded in §6/below. **Browser (one continuous session, §9 fixture — a Worth-it sunset tonight / Poor sunrise tomorrow, four coastal spots across two wanted-water groups — measured at both 390 × 844 and 1280 × 900, plus a spot-check at 768 × 1024, SEEN AND MEASURED)**: check 1 — collapsed sheet `getBoundingClientRect().height` is exactly **74px** on first Map-tab mount, no `.wf-land`, no `.wf-map-chrome-tr`, no `.wf-map-tide-strip`, no `[data-testid=wf-map-peek-body]`; the map's own visible height above the collapsed sheet measured **591px** (`map-container` height 665px − 74px), well past the README's ~440px estimate — the sheet leaves the reader *more* map than the spec assumed, not less. Check 2 — on the seeded Worth-it coastal sunset the Tide button reads `Mid ↑ · 4 dim`; stepping to the Poor sunrise removes the Tide button and every `[data-tide]` node (0 found) while `Other windows` correctly reoffers the next non-STAND_DOWN row; stepping back restores the button (pulse's mid-flight frame not caught live — tool round-trip latency exceeds the 1.2s animation, as M5's own row already noted; TESTED at the unit level, not re-photographed here). Check 3 — Layers → Tide **Off** on the Worth-it window removed every `[data-tide]` node and the Tide button in one press; **Always** on the Poor window restored both the button (`Low · 2 dim`) and the chip wave/shortfall glyphs; **Auto** persisted server-side (`GET /api/user/settings` → `mapTideMode: "auto"` after a press and a full page reload) — the previously-blocked persistence path (M5's row: local backend 500'd on a stale schema) is now live, since this session's backend was restarted after M4 per the kickoff brief. Check 4 — a real `left_click_drag` on the map (not a dispatched event — a synthetic `mousedown` alone did not reach Leaflet's native listener) collapsed an open 356px section back to 74px. Check 5 — the "Other windows" verdict span's computed style is `flex: 0 0 auto` with `scrollWidth === clientWidth` (48 === 48, no clipping) and the day span carries `text-overflow: ellipsis`. Check 6 — at both 1280 × 900 and 768 × 1024, `.wf-map-peek` is absent and `.wf-map-chrome-tr`/`.wf-map-tide-strip`/the landing card all render exactly as before. |

---

## §1 Corrections to the spec — where the codebase has already moved

Every path is under `frontend/src/` unless stated. Line numbers were re-verified against the tree at
M6 (2026-09-26) and **must be re-verified again before editing** — `MapView.jsx` is 6,729 lines (not
the 6,100 this section originally cited) and moves weekly. ⚠️ **M1, M2, M3 and M5 have since landed**,
so several items below describe the *pre-M1* baseline this whole document was written against; where
a phase has since replaced or deleted what an item names, a bracketed note says so rather than leaving
the corrected line number stand in for a claim that is no longer true.
...
phone". They do, but there is no bar component: `.wf-map-chrome-tr` (`MapView.jsx` **5970–6034**,
...
`index.css` (`@media (max-width: 639px)`, **3847–4080**: `bottom: 8px; left/right: 8px;
flex-direction: row`). So "move the controls into the sheet" means: **on phone, do not render
`.wf-map-chrome-tr` at all**, and mount its three children inside the sheet's Layers section. ⚠️ **M2
already did exactly this** — the bar's JSX is now itself `{!isMobile && (...)}`-gated (so the phone
media-query rule that used to lift it to the bottom, `index.css` ~3873–3881, is dead CSS: the element
it targets is never rendered there any more), and the whole phone **lifted stack** it used to key off
has been rewritten to the sheet's own floor: attribution `padding-bottom` is now `74px` (not `76px`,
lines 4046–4048), the counts footer is unconditionally `sr-only`-clipped rather than carrying a
`bottom` literal at all (lines 3962–3971), the scored toast moved off the lifted stack entirely onto a
fixed `top: 62px` with a 3 s fade (lines 4011–4025, M1), `.wf-map-chrome-bl` reads
`calc(74px + 27px + 8px)` rather than a bare `192` (lines 3996–3998), and the phone tide strip mount
plus its `wf-tide-strip-on`/`--tsh` re-anchors are **retired outright** (comment at 4061–4070, M3) —
`--tsh`/`wf-tide-strip-on` are desktop/tablet-only from here. `test/mapPhoneChromeCascade.test.jsx`
was rewritten against this new arithmetic at M2, as this item originally called for.
...
at `z-index: 9999` that blocks the map (`useOutsideDismiss.js` **27–31** states it in prose — "the map cannot be panned at all while one is open" — and **73, 85–86, 110** are the `MAP_FRAME_SELECTOR`/`isInsideMapFrame` symbols and the press check; the file records that the map
cannot pan while one is open, via the exported `MAP_FRAME_SELECTOR`/`isInsideMapFrame`), locks body
scroll, and is `role="dialog"` with `aria-modal` by default —
...
⚠️ **M1 already built this item** — `landingOpen` (`MapView.jsx` **1961–1962**) now reads
`!isMobile && Boolean(landingRunStamp) && (landingSeenRun !== landingRunStamp || landingReopenedRun
=== landingRunStamp)`, and `MapLandingCard.jsx`'s header comment (`components/map/MapLandingCard.jsx`
— note the full path; it has never lived directly under `components/` — lines **36–53**) has been
rewritten to say the *opposite* of what it said when this item was drafted: it now states plainly
that the card never opens on the phone, superseded by the peek sheet series. On phone the pill menu's
`↺ Back to …` reopen row is withheld (`landingLabel = ''`), and **the phone never writes
`mapLandingSeenRun`** — a reader who later opens the same run on a desktop has not seen the card and
should (§5 D-3). The card's *model* (`utils/mapLanding.js#landingCardModel` → `{rows, header, allPoor,
lead, nextUp, picks}`) is what the Windows section reads its heading from (§4 #2).

### 4. The toast has no timer, no dismiss, and its phone position is a lifted-stack literal

⚠️ **M1 already built this item.** `★ PhotoCast-scored locations shown` (`MapView.jsx` **6309–6351**
on the tab, now built from a memoised `toastShown` gate at lines 3036–3044 carrying the same
predicate; a separate overlay copy at **5817–5832** which is **untouched**) is gated on
`!aurora && !astro && briefingScores.size > 0 && solarWindowOnScreen()` and keyed on
`|date|eventType|`. On phone it now sits at a fixed `top: 62px`, centred, fading after 3,000 ms
(`wf-map-scored-legend-gone`, lines 4011–4026) rather than the old lifted-stack `bottom: 152px`.
Desktop keeps the standing legend, unchanged (§5 D-4).
...
**707–719**) calls `setTideViewBounds` on `moveend`/`zoomend` (mounted at line 5459), and `stripModel`
is rebuilt on every render (**3990**). The spec's Auto rule is **that gate AND
`verdict.tier !== 'STAND_DOWN'`**, and the mode
...
⚠️ **M5 already built this item's rule.** `MapView.jsx`'s spot builder (**3641–3679**) sets
`tideTier: tideCuesOn ? tierOf(tide) : null` (**3669** — already the post-M5 form, gated on the
Auto/Always/Off `tideCuesOn` flag rather than the bare `tierOf(tide)` this item originally described;
`tideShortfall`/`tideFitPhrase`/`coastal` sit alongside it at 3660–3679). The chip's wave glyph
(`MapLabels.jsx` **615** `data-tide`, **634–639** `<TideWave shortfall state>` — it now also carries
the served H/M/L `state` letter, tide-window-plan.md §4 #21, 2026-09-25), the pin (`PinsLayer.jsx`
**396**), both tooltips (`tideTierHeading — tideFitPhrase`) and the dimming CSS (`index.css`
**5912–5914** `.wf-maplab-chip[data-tide='miss'] { opacity: .72 }`, **6220–6222** `.wf-pin[...]`) all
key off `spot.tideTier`. So "Off hides every tide cue" and "Auto hides them on a Poor window" was
**one null at that one site**, never a flag threaded to four consumers, exactly as planned.
...
(`'window' | 'jump' | 'filters' | 'legend' | 'window-panel'`), and `handleMapPaneKeyDown` (**4864**)
...
`#8FC0C7` has no token; `index.css` **3451** and **6008–6010** (rule at 6019–6021) record that the
tide bundle's `#8FC0C7` was
...
All six answered against what shipped (M6):

- ~~Q1 — Persistence.~~ **Taken: the server column.** `V155__user_map_tide_mode.sql` +
  `PUT /api/user/settings/map-tide-mode` shipped at M4 (#925), read and written by the Layers Tide
  segment at M5 (#930). Not the localStorage fallback.
- ~~Q2 — Does Off reach the callout/sheet `TideFitBlock`?~~ **No, as planned (§4 #13).** Off hides
  only the map-surface cues (button, section, chip glyph, pin dimming, tooltip clause); the
  callout's and location sheet's `TideFitBlock` were not touched and read no mode. Still open, if
  ever revisited: whether the location sheet on the Plan tab should read the mode too.
- ~~Q3 — The counts footer has no phone home.~~ **Left hidden, as the plan defaulted.** It is
  unconditionally `sr-only`-clipped on the phone (M2, §4 #10) — no last row on Layers, no reach
  line under the Windows heading. Still open if a phone home is ever wanted.
- ~~Q4 — Does the mode belong in the desktop settings dialog too?~~ **No.** The mode is phone-only
  (`tideCuesOn = !isMobile || tideVisible(...)`, M5); the desktop settings dialog does not surface
  it, and a saved Off/Always on the phone has no desktop effect, exactly as this spec always said.
  Still open if the owner later wants it recorded or extended to desktop.
- ~~Q5 — The 8.5 px key labels.~~ **Closed at M2.** The accessibility lens measured the peek row's
  8.5px key labels at **≈7.1:1** contrast against the sheet's `.97`-opaque background — clears WCAG
  AA for small text (4.5:1) comfortably. No 9px fix was needed.
...
6. **A stacked branch a GitHub auto-close can take out from under you.** M2's first PR (#926) was
   opened against a base branch that a later step deleted (`--delete-branch` on an earlier merge),
   which GitHub reads as "the base is gone" and auto-closes every PR stacked on it — #926 included,
   with no code lost, just the PR wrapper. The fix was mechanical (re-open as #927 off `main`
   directly), but the lesson is the orchestration one: **do not stack a phase's PR on a branch that
   a later `--delete-branch` merge will remove.** Branch every phase off the current `main` tip, not
   off a sibling phase's still-open branch, even when the plan calls them "parallel with" each
   other (§8's table above already keeps M1/M2/M3 parallel with M4 for exactly this reason — the
   miss was doing it for two phases that were not supposed to be independent).
