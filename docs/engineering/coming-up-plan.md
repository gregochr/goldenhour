  ...
  "entries": [                           // P1 ships AlmanacEvent fields + enteredWindow; P2 the rest
    {
      ...
      "interim": false,                 // P2 (added during implementation, not in the original brief —
                                         // see the P2 phase-log row) — true unless bits rests on a
                                         // mature (≥60-observation) empirical magnitude. Every type but
                                         // a mature tide run stays interim for its whole life. This is
                                         // the entry-level carrier D4's "never badge from a bucketed
                                         // magnitude" rule needs: a client badge reader (P5) must
                                         // exclude interim entries from clearing a band regardless of
                                         // how high bits reads, rather than distorting the printed
