...
| P3a | 2026-08-29 | — (not yet pushed) | Shipped: `utils/comingUpFeed.js` rewritten from the pre-P2 client-side derivation (facts/attribution/degrade from raw `meta`) to the thin presentation layer P2's server-computed payload now licenses — `buildDateRail` (single day / same-month range / month-crossing range, per design §4), `groupEntriesByMonth`, `chipCounts`+`FILTER_CHIPS` (D6), `footerCopy`+`FOOTER_LEAD`. New `WindowComingUpEntry.jsx` replaces `WindowComingUpRow.jsx` (deleted, with its test): the date-rail/card grid, unconditional `kindTag`, three-tone facts, the threshold line, and the single action. `WindowFirstComingUp.jsx` rewritten: legend (unconditional), filter chips (served counts, never recomputed on filter), month-grouped `role="list"` sections. Seven `--color-topic-*` tokens added to `index.css`'s `@theme static` (D6): `coastal`/`night-sky`/`eclipse` alias existing tide/nlc/eclipse hexes, `aurora`/`sun-moon` value-alias the verdict-go/close-to-home hexes, `air`/`dust` are genuinely new. **Seam decision for P3b (D8), as §6 asks to be recorded here:** only `action.kind === 'plan'` renders as an interactive `<button>`, dispatched by reading `entry.action.kind` itself inside the click handler (not by branching on `interactive`) — P3b adds its `coastal-spots`/`dark-sky-spots` branches to that same switch when the `kind:'coming-up'` map channel exists, rather than only widening `interactive` and risking a silent fall-through to `onGoToPlan`. Until then those two kinds render inert (`<div>`, no pointer, no hover change, no button role) with their served label still visible as plain text. **Not built in this phase, on the wire but unrendered:** `entry.coincidence`/`joinNote` (D10's two-topic card) and `entry.tide` (the sparkline) — both explicitly P3b (§6b); a merged entry still renders correctly on its winning topic's own facts. `entry.scoreNote` is also unrendered — §13 says "since-line + card read it" but §6's P3a card inventory never lists it and the design bundle has no visual slot for it separate from the threshold line, so it is left for whichever phase builds the since-line (P5) to place, the same "recorded gap, not silently dropped" treatment §11.21 gives the lone-tide-run threshold. The NEW-flag slot §6 asks for is marked with a comment at its DOM position (between the title and the kind tag) but not built — nothing to build yet. One deviation from the design bundle recorded under §11.22: month abbreviations use the house `en-GB` short form (`Sept`), not the design's three-letter `SEP`. ⚠️ **One accessibility defect found and fixed during adversarial review, worth flagging for any later phase that touches this button:** the first draft set `aria-label={entry.action.label}` on the interactive card so a screen reader got the concise destination rather than the whole card as one run — this was wrong, because `button` is an ARIA role with `childrenPresentational: true`, so an explicit `aria-label` override does not just set the name, it throws away every fact, the prose and the threshold line too, leaving a screen-reader user with only the action sentence and none of the card's actual content on exactly the entries that ARE clickable. Fixed by leaving the name to compute from content (matching `WindowFirstComingUpHandoff`'s own button), with bare `{' '}` text-node siblings between every section so nothing glues — verified empirically via `dom-accessibility-api`'s `computeAccessibleName` rather than assumed. Four adversarial review lenses (correctness vs spec, CSS/tokens/accessibility, test quality, conventions/forward-compatibility) found this plus: a `.wf-cu-card-inert:hover` rule losing the topic-colour left border to a later shorthand rule (fixed by restating the longhand); `.wf-cu-month:first-child` never matching because each month section is its own sibling wrapper (fixed with an explicit `data-first` marker); AA contrast failures on the kind tag and action link from carrying the design's literal opacity values on top of an already-translucent card background (fixed by dropping the opacity, matching this file's established `--color-plex-text-secondary`-over-muted precedent); the footer stating a specific served count beneath "Looking ahead…"/"Could not load…" (fixed by splitting `footerCopy` into an always-true `FOOTER_LEAD` and the full counted paragraph, gated on `status === 'ready'`); a zero-count filter chip (e.g. Air & dust, D9's near-unreachable-at-first-ship families) silently blanking the pane with no explanation (fixed with a named `coming-up-filter-empty` message, distinct from the feed-is-genuinely-empty state); and the D13 "no count on the tab" pane-side comment, which a full-file rewrite had dropped rather than preserved for P5 to rewrite as the plan asks — restored. Frontend gate green (175 files / 4295 tests, lint clean, `npm audit` 0 vulnerabilities, build clean). Browser verification: see the PR description for exactly what was seen at each viewport vs. only asserted from source — the environment's outbound network policy blocks JitPack (`solar-utils`), which the backend depends on, so the backend could only be started after installing the vendored jar into the local Maven cache under its correct coordinate as a session-local workaround (not committed; a fresh checkout still needs real JitPack access or the same workaround). |
| P7 | 2026-08-29 | — (not yet pushed) | Shipped as planned, out of phase-map order (after P1, before P3–P6 land) — deliberately invisible: `topic_daily_log` (V151, next-free-number re-verified against the tree on this branch AND origin/main at write time, not the plan's stale guess) + `TopicDailyLogJob`, a nightly job seeded at 04:40 (re-verified free against every migration that seeds or updates `scheduler_job_config`, not just the ones the brief named — the brief's own list turned out incomplete but its conclusion held). Logs presence/intensity/`landed_on_window` for the eight candidate topics per region for the UK-civil "yesterday", each from the least-triage-biased store available (documented per topic in the job's own Javadoc, per the round-3 "denominator lesson"): DUST and STORM_SURGE from the complete `forecast_evaluation` population (both columns are written pre-triage from `AtmosphericData`, confirmed by tracing all seven `ForecastService.buildEntity` call sites — none skip it, including both triage branches); INVERSION from `forecast_score` (survivor-only, the best available — the persisted score is Claude's own output per `InversionDetails`'s Javadoc, so it's null on anything that never reached Claude), restricted to SUNRISE rows only (`InversionHotTopicStrategy`'s own Javadoc: a SUNSET row's score is "physically meaningless"); SNOW from `survivor_atmosphere` via `SnowFreshHotTopicStrategy.isFreshSnow` — the codebase actually has two live snow strategies (`SnowFreshHotTopicStrategy` and `SnowTopsHotTopicStrategy`) against the plan's one candidate name, and the job's Javadoc now says explicitly why the lying-depth one was chosen over the fell-summit derived one, not just which; SPRING_TIDE/KING_TIDE by replaying `TideSizeIndex`'s per-day-per-location height logic (same London-day bucketing, same threshold via `TideService.getTideStats`) for *which dates and how big*, but the label (which of the two topic types the date logs) comes from the moon alone — `LunarPhaseService.nearestSyzygyIsPerigean`, asked once per date — never from `TideStats.p95HighMetres`, per the two-axis rule (`backend/AGENTS.md` §2); `landed_on_window` is left null for both, a recorded gap in the spirit of §11.21 rather than replicating `TideRunDay.alignedEvent`; AURORA from `aurora_forecast_result`, silent (no row at all) for a region with nothing stored that night rather than a false `present: false`, since the table is written only on a manual admin trigger; NLC as `NlcClarityService.isNlcSeason` — re-confirmed `NlcSightingClient`/`NlcSightingService` persist nothing and free-text-report a location that doesn't map to the region roster, so there is genuinely no other per-night signal anywhere in the tree — logged identically across every region since the season boundary isn't region-specific. AURORA and NLC always log `intensity: null` per the brief's own instruction. Adversarial review (one read-only pass, cross-checked against `TideSizeIndex`, `ForecastService`, `DynamicSchedulerService`, both snow strategies, and every `scheduler_job_config`-touching migration directly rather than trusting this diff's own comments) found one real defect, fixed: `persist()`'s check-then-act (`existsBy…` then `save()`) isn't transactional, so a scheduled fire racing an admin `triggerNow` could hit the table's unique constraint — uncaught, that would have aborted the rest of that topic's regions (and, for the shared DUST/STORM_SURGE call, silently dropped STORM_SURGE too) with no backfill path on an append-only table. Now caught per row (`DataIntegrityViolationException`, logged at DEBUG, loop continues), with a dedicated collision test. Review also flagged one test that didn't exercise what its name claimed (`tides_outsideDayOrNoStats_producesNothing` never actually constructed a boundary extreme) — split into a real adjacent-local-day boundary test and a separate no-usable-stats test. Backend gate green (7756 tests, JaCoCo/SpotBugs/Checkstyle clean); no frontend diff, so no browser pass. **Revisit the interim rarity/magnitude constants (dust and inversion config fallbacks, tide cold-start bucketing) after ~90 days of accrued rows**, per the brief. The optional admin-triggered dust/surge backfill from historical `forecast_evaluation` was skipped — time was not tight, but the phase's own scope reads as complete without it and a backfill is easy to add later without touching this commit's shape. **Post-push external review (Codex, on PR #686) found one real, significant defect, fixed before merge**: the original `logTides` gated `KING_TIDE` on `TideStats.p95HighMetres` independently of `SPRING_TIDE`'s spring-threshold gate — precisely the "OR the height test into the king label" pattern `backend/AGENTS.md` §2 and CLAUDE.md's tide-axes doctrine name as forbidden, and precisely the bug `KingTideHotTopicStrategy.isPerigeanSpring`'s own history already fixed once (a big spring tide clearing P95 without being perigean, August 2026, used to print "king" with no perigee in sight). Because both gates were independent, a single non-perigean day with big-enough water would have logged **both** types, permanently inflating both arrival rates in the future rarity model — exactly the corruption the reviewer named. Fixed by removing the P95 read entirely: presence is now gated on the spring threshold alone (*which dates and how big*), and the resulting date logs `KING_TIDE` when `LunarPhaseService.nearestSyzygyIsPerigean` is true and `SPRING_TIDE` otherwise (*what kind*), asked once per date so the two types are mutually exclusive by construction — matching how `KingTideHotTopicStrategy`/`SpringTideHotTopicStrategy` already partition the Plan tab and confirming there is no third, disagreeing tide rule. Two of the new tests were rewritten and a third (`tides_hugeButNonPerigean_staysSpringNeverKing`) added specifically to pin the regression the reviewer described. Backend gate re-run green (7758 tests) after the fix. |
| P4 | 2026-08-29 | — (not yet pushed) | Shipped: `service/comingup/ComingUpConditionsBuilder` (new) builds `conditions[]` in the assembly pass — Coastal tides, Saharan dust, Valley inversions, always present even with zero occurrences (D11). `AlmanacService.assemble` now keeps the RAW (unfiltered) per-build event merge alongside the eligible subset, passing both into the new builder alongside the already-assembled `entries[]`, so a run wholly inside Plan's window (never eligible for the chronology) still gets a strip occurrence without a second detection pass — every tide-run occurrence stages through the SAME `TideRunBuilder`/`TideRunPeakHistory`/`SurpriseScore` calls `ComingUpAssembler.enrichTide` already uses (P2), never a second scoring formula. Dust rarity upgrades to an observed rate (bursts over the trailing 60 days, replayed against the complete `forecast_evaluation` population via a new `findByTargetDateBetweenAndTargetTypeIn` query — never survivor stores, D4's evidentiary-bar rule) once ≥5 arrivals exist; inversion rarity never upgrades (no unbiased population until P7's log matures) and dust below the bar falls back the same way; magnitude for both stays a fixed config threshold forever (`ComingUpScoringProperties.RecurrentTopics`/`Dust`/`Inversion`, documented in `application-example.yml`) — never a real distribution, so the quant line never claims `median`/`p90` for either, using the "promoted above X (interim)" phrasing D4 asks for instead. `DustHotTopicStrategy.isDustEnhanced`/`InversionHotTopicStrategy.STRONG_SCORE_INCLUSIVE` widened to `public` for reuse. Frontend: `WindowComingUpConditions` (new) renders the strip between the filter chips and the handoff row — the design of record's own DOM order (`Coming Up.html`: header → since-line → chips → conditions → handoff → chronology), which moves the handoff row one place down from where P1 first put it (kept deliberately un-gated on the almanac fetch status, since it reads only `hotTopics`). Condition rows collapse at 820px, occurrence rows at 760px (the design of record's own figure, not the README's 820 — §11.17); expansion panels carry both the `hidden` attribute and a display class (this codebase's Tailwind v4 preflight already makes the attribute alone sufficient — the class is defence in depth, not the mechanism, mirroring `WindowFirstShell`'s own tab-panel comment). A `promoted`/`insidePlan` occurrence renders as a real `<button>` wrapping every cell rather than only its status span, so the 760px breakpoint (which hides only the status/reason TEXT) can never also remove the click target. `in the list →` scrolls to and highlights the matching chronology card via a new `data-entry-id` anchor on `WindowComingUpEntry`, using `--wf-mast-h` (not `--wf-lens-reserve`, absent on this tab). Header sub-line gains a quiet "scores provisional" suffix while any visible condition is interim. `TideSurfaceAgreementTest` gained a third assertion: given the same height/threshold, `TideAlmanacSource` — which the strip stages verbatim rather than re-detecting — agrees with the other two surfaces; its own "what this does NOT pin" section now says explicitly that this assertion doesn't run `ComingUpConditionsBuilder` itself, that's `ComingUpConditionsBuilderTest`'s job. Three adversarial review lenses (backend scoring, frontend/CSS, conventions/phase-seams) found four real defects before landing, all fixed: (1) a `Map.merge` NPE on a legitimate null-AOD-but-elevated-surface-dust row (plan §1: "values can be null when the aerosol fetch degraded") would silently truncate the trailing-window loop partway through, since `findByTargetDateBetweenAndTargetTypeIn` returns rows "in no guaranteed order" — fixed with a null-tolerant presence recorder that still lets a later non-null reading for the same day upgrade a null placeholder (`Map.merge`'s own null-tolerant semantics for an existing null value, not a second special case); (2) coastal-tide run scoring had no per-run failure isolation unlike dust/inversion — one bad tide row propagating uncaught through `build()` would 500 the WHOLE `/api/almanac` response and, because coastal tides builds first, take the otherwise-healthy dust/inversion conditions down with it too — fixed with the same catch-and-degrade-to-cold-start pattern the other two conditions already used; (3) the `reason` tag (D10's max rule) could never fire in production: it matched a run to its chronology entry by the run's OWN id, but a run that LOST a coincidence merge surfaces in `entries[]` under the WINNER's id instead (`ComingUpAssembler.mergeEntries` keys the surviving entry by whichever event won) — so the one case `reason` exists to name was exactly the case the lookup missed, and the two tests written to cover it both fabricated an impossible entry shape (keyed by the losing run's own id) that passed without ever exercising a reachable code path; fixed with a fallback match by date overlap plus a coincidence line carrying the run's own family (`"coastal"`), which also retired the `promoted`-status gap the first draft of this row recorded as an accepted simplification — closing one gap closed the other for free. A second logic slip surfaced while fixing this: naming the winner from `matchedEntry.coincidence().getFirst().name()` would have named the LOSER (the coincidence line always describes who lost, and when the fallback fires that loser is this very run) — corrected to `matchedEntry.title()`. (4) a React `key={occurrence.date}` on the frontend's occurrence list was not guaranteed unique (two occurrences can legitimately share a calendar day) and reproduced a live console warning in this phase's own tests, unnoticed by the author — fixed with a `${date}:${index}` composite. Also fixed: `.wf-cond-kind` stacked `opacity:0.72` on a per-topic accent colour, repeating the exact AA-contrast mistake `.wf-cu-kindtag` (P3a) already found and fixed in this same file — measured 3.70–4.31:1 against all three shipped topic colours, corrected to full-strength accent with no opacity. Backend gate green (7776 tests, JaCoCo/SpotBugs/Checkstyle clean); frontend gate green (179 files, lint clean, `npm audit` 0 vulnerabilities, build clean). Browser-verified live against real local data (not a fixture): the strip renders all three conditions with genuine `ComingUpConditionsBuilder` output, expansion panels open/close with `[hidden]` toggling `display:flex`↔`none` as designed, `in the list →` scrolls to and highlights the correct chronology card (confirmed via `getBoundingClientRect`, landing well clear of the sticky masthead), and both 820px/760px collapse breakpoints were confirmed at true mobile width (375px) including that a promoted occurrence stays a real `<button>` with its status text hidden. One state seen rather than asserted: no `insidePlan` coastal-tide occurrence existed in the local dataset's actual run mix, so that status's live rendering was not visually confirmed (it is covered by both `ComingUpConditionsBuilderTest` and `WindowComingUpConditions.test.jsx`). Local tide `valueLabel`s render empty (no seeded `tide_extreme` history) — an honest degrade, not a defect.

**Pre-merge conformance review found one blocking defect plus five departures from §2/§7/§13, all fixed under the deviation protocol (none left as unrecorded drift):** (1) **Blocking.** `tideOccurrence`'s insidePlan check additionally required `!event.startDate().isBefore(builtFor)`, which D11 never asks for — eligibility already guarantees the complement (no entry) implies `endDate ≤ lastPlanDate`, full stop. `TideAlmanacSource`'s backward walk (P2's own phase-log row) routinely produces an in-progress run whose start already lags "today"; that run was misread as `heldBack` — a live-on-the-Plan-tab run reported as neither promoted nor upcoming. Fixed by dropping the clause; the unreachable `heldBackDefault` fixture (a run ending 18 days ago cannot be served — every event's `endDate` is on/after `builtFor`) was replaced with a real in-progress-run boundary test. **`heldBack` is now structurally unreachable for coastal tides until a future phase gates the chronology on bands rather than dates alone — P5 needs to know this before it assumes the status is exercised in production.** (2) **D10's `reason` is now bidirectional**, matching its literal text ("mandatory wherever the max was taken") rather than the design README's narrower "only where the score is not the topic's own magnitude" reading: it now fires whenever a merge occurred, regardless of which side carried the score, naming the OTHER topic either way. Which field supplies that name depends on which side won — the run's own coincidence line (when it won) or the surviving entry's own title (when it lost) both happen to name the correct party, which is not a coincidence but is easy to get backwards; a second slip caught while fixing this used the coincidence line's name in the fallback branch too, which names the LOSER (this very run) rather than the winner. `matchingEntry`'s Javadoc now states the four invariants its date-overlap-plus-family heuristic depends on (runs maximal/disjoint; a merge keeps the winner's own span; merges only ever pair tide×supermoon; a merge only ever involves already-eligible events) so a future widening of the merge machinery is warned rather than silently miscounted. (3) The cadence words (`"deterministic"`/`"recurrent"`/`"persistent"`) were Java string literals; moved into `ComingUpScoringProperties.Cadence` (documented in `application-example.yml`) per §7/§11.16's "authored in config, not derived, until P7". (4) The "scores provisional" marker rendered on the CHRONOLOGY pane's own sub-line; §7 asks for it on the STRIP's own header sub-line ("frequent · never announced · always one click away · scores provisional") — moved into `WindowComingUpConditions`, which now derives `anyConditionInterim` itself rather than `WindowFirstComingUp` computing it and threading a suffix into an unrelated sub-line. (5) `PEAK_LIGHT_WINDOW_MINUTES` was a builder-local constant; moved to `ComingUpScoringProperties.peakLightWindowMinutes` (default 90) and `passesPeakGate` became an instance method that reads it — genuinely read (a non-positive configured value closes the gate, pinned by a new test) rather than merely relocated for appearances, though it is still not read arithmetically: no forward candidate yet carries a clock time to compare against a light window's edge, so SUNRISE/SUNSET typing remains the documented v1 proxy. (6) **Schema addition under the deviation protocol, in §13, this commit:** `ComingUpConditionOccurrence` gains `label` (nullable string) — which run kind an occurrence is, read straight from the source event's own type (`"Spring tide"`/`"King tide"`), never re-derived; null for dust/inversion, which have only one kind per row. Needed because D11 puts spring and king runs in ONE strip row with nothing else to tell them apart — a new headline test pins one window holding both kinds landing in the single `COASTAL_TIDES` condition with distinct labels. **The field is populated and tested but NOT yet rendered by `WindowComingUpConditions`** — matching P3a's own precedent for `scoreNote`/`tide`/`coincidence` shipping on the wire ahead of their renderer — left for whichever phase next touches the occurrence row, recorded here rather than silently dropped. One more phase-log note, no code: `TideSurfaceAgreementTest`'s third assertion pins that `TideAlmanacSource` (the shared SOURCE the strip stages verbatim) agrees with the other two surfaces — it does not exercise `ComingUpConditionsBuilder` itself, which is `ComingUpConditionsBuilderTest`'s job; its own "what this does NOT pin" section says so explicitly now. Both gates re-verified green after all six fixes (backend: 7776+ tests, JaCoCo/SpotBugs/Checkstyle clean; frontend: lint/test/audit/build all clean) and the PR description and this row both record the post-review-fix state.

**A Codex review pass on the pushed PR found one further real defect, fixed:** `buildDust`'s trailing-window `catch` left `maxAodByPresentDate` empty on a transient DB failure with no record that the read itself had failed, so the zero-arrivals branch reported `"none in the last 60 days"` — a false claim of absence rather than the honest "we don't know" the failure actually is. Because `AlmanacService` caches the built response for the whole civil day, that false claim would have been served and RETAINED for hours after the database recovered — the exact silent-corruption shape D4's "unbiased population" concern exists to prevent, just triggered by infrastructure instead of triage bias. Fixed by tracking the read's own success and reporting `"history unavailable right now"` instead of a fabricated zero, with a dedicated test. |
| P3b | 2026-08-29 | — (not yet pushed) | Shipped, frontend-only: `ComingUpTideSparkline` (sparkline, `utils/comingUpSparkline.js` for the pure geometry) and the coincidence-card rendering in `WindowComingUpEntry` — the two on-the-wire-since-P2 fields P3a's phase-log row named as gaps — plus the `coastal-spots`/`dark-sky-spots` map channel (D8). **Trigger-kind decision (for P6's delete list, §6b):** a new `kind:'coming-up'` branch in `mapOverlay.js`, modelled on `topic` but never reusing it — `App.jsx`'s dispatch (extracted into a testable `normalizeMapTrigger`, adversarial-review finding) checks `kind === 'coming-up'` before the generic `filterAction` check, so it can never fall into `kind:'topic'`, the branch P6 deletes along with `HotTopicStrip`. **Date decision:** the trigger carries `date` — the coming-up branch, like `topic`, never calls `ratingFor`/`solarTimeFor`, and its markers are selected via `focus.names` (an exact-match override bypassing the rating-filter pipeline), so a T+90 date can never dress "no data" as "stand down" the way `kind:'event'` would. **`solarDayGeometry.js` reuse decision (for P6's `TideRunRow` deletion call, §6b):** NOT reused — that module's axis is a Europe/London clock day (1440 minutes, sampled by clock time); this sparkline is a fixed-phase cosine over a 62-unit period with no relationship to a clock at all, so there was nothing to share. Four adversarial review lenses (runtime correctness vs spec, CSS/tokens/accessibility, test quality, conventions/forward-compatibility) found real defects, all fixed: the sparkline amplitude's floor was documented but never clamped (`Math.max` added; a below-average delta would otherwise go negative and silently invert the wave, dropping a marked HIGH water's marker below the axis) — caught independently by all four lenses, along with the coincidence card's mutual-exclusivity-with-prose bug (`ComingUpAssembler.assemble` runs `markFirstOfType` AFTER `mergeCoincidences`, so a merged winner that is also first-of-type legitimately carries both `prose` and `coincidence`; an exclusive-or silently dropped the prose — fixed by rendering both, frontend-only, no backend ordering change); the coincidence card's synthesized "self" line duplicated the card's own title verbatim (the design's combined name for a merged entry, e.g. "Spring tide run, on a supermoon", is not what `entry.title` actually is — fixed by rendering only the served line(s), never a synthesized self-line); the `handoffDarkSky` effect fired unconditionally on every map handoff sharing the app's one `handoffNonce` counter, so an unrelated handoff silently cleared a reader's manually-set dark-sky filter on the persistent Map-tab pane — fixed with a `handoffDarkSky != null` guard, safe specifically because the coming-up channel is the only producer of that prop and always sends an explicit boolean; a `<div>` nested inside a `<button>` (invalid content model, matching the file's own established `<span>`-with-`display:block` fix elsewhere); a CSS specificity collision silently dropping the sparkline's own `gap`; a missing `align-items:center` leaving the sparkline's label pinned to the top of its 24px row instead of centred on the wave; and a hardcoded `#E0A542` where `--color-verdict-marginal` already names it. **A Codex review pass on the pushed PR found one further real P1 defect, fixed:** the combined `handoffDarkSky` effect set the dark-sky toggle but never touched `activeTypeFilters`, so a coastal-spots handoff followed by a dark-sky-spots one left the stale SEASCAPE filter ANDed with the new dark-sky one — "dark-sky coastal spots only", not "all dark-sky spots" — on the persistent Map-tab pane; fixed by folding `activeTypeFilters` into the SAME guarded, nonce-keyed effect (both flags always move together, since the `coming-up` channel always sends both explicitly), with a new test proving a non-SEASCAPE dark-sky location becomes visible after the second handoff. **§11.23 records the gap that remains**: the standalone `handoffFilterAction` effect topic pills use still has no clearing branch at all, so a `coming-up` filter can still leak into a LATER, unrelated trigger (a location drilldown, a region row) — not new, but its weight changes once P6 deletes `kind:'topic'` and `coming-up` becomes the principal producer of `filterAction`-bearing handoffs. **§6b's "label from served strings" instruction was satisfied as display-formatting of §13's numbers, not as a served string** — §13 declares `tide.range`/`tide.delta` as plain numbers (`round1`-ed server-side), with no separate formatted-label field; the sparkline's `5.2 m` / `+1.9 vs avg` text is `toFixed(1)` + a sign prefix over those already-served, already-rounded numbers, which is presentation arithmetic over served facts (the same licensed class the date rail already uses), not a new fact composed client-side. Frontend gate green (179 files, 4372 tests, lint clean, `npm audit` 0 vulnerabilities, build clean). Browser-verified live (Playwright headless from Bash — the interactive Browser pane wedged mid-session, the documented recovery path) against real seeded `tide_extreme` data (synthesized locally, phase-locked to the real 2026-08-12 new moon so spring/king runs land on the same dates the live server's own ephemeris-only fallback already reported, confirmed by comparing `GET /api/almanac` before/after seeding): the sparkline renders correctly on every tide entry at desktop and 390px with the label centred beside the wave; the coincidence card renders BOTH merge directions (a tide run absorbing a supermoon, and a supermoon absorbing a king tide run) with no duplicated title and the correct per-line swatch colour; `Show coastal spots` opens the map overlay with the SEASCAPE filter active; `Show dark-sky spots` opens it with Dark sky active; no console errors either way. **Two states seen only via unit test, per the user's own choice when asked** (a live check needed a paid Anthropic evaluation run to seed a Map tab locally — declined): the LW (low-water) wave inversion (the seeded data happened to mark every run HW; covered by the pure-function tests and a component-level marker-position test) and the exact dark-sky-then-coastal clearing sequence on the PERSISTENT Map tab specifically (the overlay itself, which remounts fresh per open, WAS browser-verified for both actions) — `MapViewDarkSkyHandoff.test.jsx` exercises the identical prop sequence directly against `MapView`. |
| P6 | 2026-08-29 | — (not yet pushed) | Executed D7, frontend + docs only. Deleted `HotTopicStrip.jsx` (1310 lines), `components/shared/CertaintyChip.jsx`, `utils/topicCertainty.js`, `TideRunRow.jsx`, `SurgeRunRow.jsx` and their tests; `WindowFirstDoors.jsx` rewritten down to its one surviving door (Regional planner — `.wf-doors`/`.wf-door`'s existing flex rules already go full-width with no CSS change, so none was made). **One orphan found beyond D7's own list, deleted under the same "verify no live caller remains" discipline**: `components/chart/solarDayGeometry.js` (+ `test/solarDayGeometry.test.js`) had no importer left once both `TideRunRow`/`SurgeRunRow` were gone — confirmed by grepping every one of its exported constants — so it was deleted rather than left as a second, quieter orphan chain; the plan's own D7/§9 text never named it because P3b's phase-log row only recorded that it was *not reused*, not that it would become fully dead. `mapOverlay.js`: removed the `kind:'topic'` branches from both `normalizeMapTrigger` and `buildMapOverlay` (sole remaining producer of a bare `filterAction` handoff was `WindowFirstDoors.jsx:224`, now gone) — the `kind:'region'`/`locationNames` "qualifying spots" machinery one function down was **deliberately left in place**, even though its only real-world producer (`HotTopicStrip`'s region-drilldown taps) died in the same diff: `test/mapOverlay.test.js` exercises it directly against `buildMapOverlay`, `kind:'event'` (HeatmapGrid's live caller) shares the same code path below the `region`/`event` fork, and removing it touches shared control flow for a class of trigger the plan never asked P6 to remove — recorded here as a residual orphan for a future phase rather than silently expanded into. `planDoors.js`: `DOOR_IDS` down to `['regional']`, one-line change, no compat shim (the existing unknown-id filter already covers a stored `'topics'` key). `index.css`: deleted the `.hot-topic-*` block and the `.tide-run`/`.tide-row`/`.runchip`/`.tr-*`/`.sr-*` block plus their media queries (272 lines total, two contiguous blocks with one blank-line gap, collapsed to one on deletion); the `.wf-door-panel :is(…) { scroll-margin-top }` rule was left untouched and its comment's "every hot-topic pill" clause corrected (the rule's real subject was always `HeatmapGrid`'s re-parented focusables, which live under the surviving door); grepped for a `.wf-cu-kind` comment citing `topicCertainty` by name (per §9) and found none — superseded by a later merge before this phase started, so no action was needed there. Fixed prose in `windowFirstRows.js` (the "reconvergence with `HotTopicStrip` is DUE" instruction is now stated as discharged by removal, not deferred — recorded in the CHANGELOG too, per the brief), `confidenceUtils.js`, `mapDates.js`, `WindowFirstLensBar.jsx`, and `windowFirstTopics.js` (an unlisted fifth file — its `topicCertainty` precedent citation was a comment only, never an import, but went stale the same way). Fixed `test/instantsAbroad.test.jsx` (dropped its two `HotTopicStrip`-rendering `describe` blocks and the now-unused import) and rewrote `WindowFirstDoors.test.jsx`/`test/mapOverlay.test.js`/`test/planDoors.test.js` for the single-door, `kind:'topic'`-free world; `WindowFirstShellSticky.test.jsx` needed nothing, confirmed. **Ledger duties, both recorded rather than silently dropped:** `auroraTonight`/`auroraTomorrow` are now write-only (their only reader was the deleted `HotTopicStrip` panel) — backend fields kept per the brief's owner-call instruction, CLAUDE.md updated in the v1-retirement §8 style; `HotTopic.tideRun` likewise loses its only renderer (`TideRunRow`) — called out loudly in CLAUDE.md's "Tide runs (Hot Topics)" bullet and in the CHANGELOG, backend derivation untouched. **§11.23 decision, recorded per the brief's instruction to choose rather than default:** left as a documented gap, NOT closed in this phase. Closing it would mean redefining `MapView`'s standalone `handoffFilterAction` effect to clear `activeTypeFilters` on every handoff nonce regardless of kind (matching the `handoffDarkSky` effect's bidirectional shape) — but that changes user-facing behaviour for every OTHER trigger kind too (a manually-set Advanced-panel filter would now be silently cleared by an unrelated location drilldown or region-row tap), which is a UX call with no test coverage plan established for the right semantics, not a mechanical follow-on to a deletion phase. The ownership note is updated instead: with `kind:'topic'` gone, `coming-up` is now the sole live producer of `handoffFilterAction`, so the gap's blast radius is now fully attributable to that one channel rather than shared with a dead one. CHANGELOG states this discharge/non-closure split explicitly. **Two read-only adversarial review passes** (one on runtime correctness, one on test quality/accessibility — both independently ran the full frontend gate themselves rather than trusting this diff's own claims) found the deletion complete and correct against §9/D7/D8, and surfaced one real orphan neither the plan's own list nor the author had named: `WindowFirstBriefingContext.jsx`'s `isLiteUser` field had exactly one production reader (`WindowFirstDoors`'s `HotTopicStrip` panel) and was left computing and publishing a value nothing consumed — fixed by deleting it (the field, its context-default, its memo/deps entries, and the stale "two components behind the doors" comment) under the same discipline the phase already applied to `auroraTonight`/`HotTopic.tideRun`/`solarDayGeometry.js`; confirmed via grep that no test asserts on the real context's `isLiteUser` shape (only unrelated mock fixtures reference the name, harmlessly). Also fixed: two `index.css` comments (an AA-contrast justification and a phone-breakpoint rationale, both outside the deleted blocks) that cited `.tr-foot`/`.hot-topic-facts .opt` in the present tense after this diff deleted them. Frontend gate green (173 files / 4068 tests, lint clean, `npm audit --audit-level=high` 0 vulnerabilities, build clean) both before and after the review fixes. **Browser verification, and one gap in it stated rather than implied away**: the Coming up tab was verified live end-to-end — the strip, the chronology, and both `Show coastal spots →`/`Show dark-sky spots →` actions were clicked and each confirmed via DOM inspection to open the map overlay with the correct filter active (`Seascape`/`Dark sky only` chips shown selected), proving the `kind:'coming-up'` channel survived the `kind:'topic'` deletion untouched. The Plan tab's door itself was **not** visually confirmed at full width: the local H2 database has no `daily_briefing_cache`/forecast data (`GET /api/briefing` returns 204), and seeding it for real would mean a paid Anthropic evaluation run — declined, matching the precedent P3b's own phase-log row already recorded for the same constraint. The full-width claim instead rests on: the CSS diff shows `.wf-doors`/`.wf-door` were not touched (its `flex: 1` rule mechanically fills the container with one child regardless of data), the component was confirmed to mount and unmount cleanly with zero console errors against the live (empty) app, and `WindowFirstDoors.test.jsx`'s rewritten suite exercises every branch of the one-door render. 390px was checked only via the test suite's own viewport-emulation tests for the same reason. |
| P5 | 2026-08-29 | — (not yet pushed) | **The series is complete — P1 through P7 have all shipped.** Executed the whole brief: the census (D4/§11.13), the migration + settings endpoint (D3), the eager fetch (D13), the `isNew`/badge/since-line derivation (D12), the bootstrap write, `Mark seen`, NEW flags, and the CLAUDE.md amendment. **Census run first, as instructed**: `ComingUpAnnualBadgeCensusTest` assembles a synthetic year (1 Sep 2026 – 31 Aug 2027) through the real ephemeris sources and the real `ComingUpAssembler`. Under the placeholder edges every annual-rate topic (5 showers + the NLC season boundary, all landing exactly at 9.5) interrupted — **11 badges/year, 7 of them interrupts**, ~4× the design's "one or two". Moved ONE edge, `interrupt: 9.5 → 10.0` — chosen because it sits above every annual-rate topic's score (9.5) and below the eclipse's (11.6), the only rung left on this v1 inventory's discrete score ladder (supermoon 6.9 · solar turning point 8.5 · shower/NLC 9.5 · eclipse 11.6, every magnitude at the D4 default of 1.0). Result: **11 badges/year, 1 interrupt** — the design's target hit exactly. `announce` stayed at 7.5 (the 6.9→8.5 rung gap gives no reason to move within it). No topic special-cased; the fixture is a permanent regression test. Two deterministic rarity gaps (`supermoonMeanGapDays`, `eclipseMeanGapDays`) were measured against their catalogue/ephemeris-derivable alternatives and deliberately KEPT as the documented estimates rather than swapped: the ephemeris-true supermoon rate (~3–4/year) would push supermoons to the announce contour's knife edge and the badge rate to ~14/year; the catalogue-true eclipse rate (5 events, 347-day mean gap) would drop eclipses BELOW the annual-rate topics and leave interrupt structurally unreachable by anything. Each is recorded as a P5-census finding in `ComingUpScoringProperties`' own Javadoc, not a silent placeholder any more. ⚠️ **§11.13 names TWO problems and this census resolves only one.** The over-fire half (showers/equinox landing on the interrupt contour) is what the edge move above fixes. The design's own spring-run contradiction ("entering at day 90 is silence" vs. its own 8.2-bit worked example) is a SEPARATE half, left deferred rather than resolved: `TideAlmanacSource` is never wired into this census at all, so it says nothing about a real tide-run magnitude. What keeps that contradictory 8.2 from badging today is the unrelated `interim` cold-start exclusion (P2), which holds only because no port has yet crossed `coldStartMinObservations = 60` (D4: ~2.5 years out, not the 90-day horizon the recurrent-topic constants below are revisited on) — there is no tripwire watching for that, only this note and the matching one now in `ComingUpAnnualBadgeCensusTest`'s own class Javadoc and in §11.13 itself. Revisit by wiring the source in and re-running the census once any port matures. A corrected figure while here: a mature tide-run magnitude clearing `interrupt` needs a `n ≥ 69`-observation run-peak history (`SurpriseScore.magnitudeFromHistory`'s `log2(n+1)` for a record run) — 9 past the cold-start floor, roughly 4–5 months past first maturity — not the "once-in-200-runs" figure an earlier draft of this row and the surrounding Javadoc gave, which conflated the exact-percentile mechanism with the bucketed cold-start one. **Backend**: `app_user.coming_up_last_seen_at` (V152 — read the tree fresh, V151 was `topic_daily_log`'s from P7, confirmed still the latest on both this branch and origin/main), nullable/no-default/no-backfill per the V136/V147 pattern; `ForecastHorizon.civilDate(Instant)` (new) derives the Europe/London civil date from a stored instant, the serve-time counterpart to `today`'s own "now" rule; `UserSettingsService.markComingUpSeen` (its own `PUT /api/user/settings/coming-up-seen`, matching `map-colours`' own reasoning for not folding into `saveHome`) takes an injected `Clock` — already a single bean in the context (`AppConfig`), no collision. `HttpCachingConfigTest`'s `@ValueSource` gained the new path in the same commit. **Frontend**: `useComingUpFeed`'s class doc is rewritten to state the D13 reversal explicitly (both halves the plan named — the hook's own "why eager" section and the pane's "still no ROW count" section, which now explains precisely why the fetch going eager didn't also earn a plain count); the shell calls it with `enabled` hardcoded `true`. `utils/comingUpArrivals.js` (new) is D12's whole membership — `isNewEntry`, `qualifyingArrivals`, `deriveBadge`, `selectSinceEntry` — imported into `comingUpFeed.js`'s `buildEntryView` (which threads `isNew` through, per-entry) and into `WindowFirstShell`/`WindowFirstComingUp` (which independently call the SAME pure functions off the SAME `comingUp.events`/`comingUpLastSeenDate` values, so the tab badge and the since-line can never disagree — verified, not just argued). `WindowComingUpSinceLine.jsx` (new) places `{bits, title, dates, scoreNote}` verbatim — including a recorded correction to the design bundle's own demo copy, which prints a shower's PEAK date next to "entered the window" (that can't be literal: an event 90 days out enters the window ~90 days before it peaks, not on the peak); this component reports `entry.enteredWindow` instead, the date the field name and the badge's own arrival test actually mean. CLAUDE.md's Backend-heavy bullet gained a fourth class (D12) in the same commit, per the instruction that the bullet's enumeration must stay exhaustive. **Comment sweep**, all four items in the brief re-verified against the tree first (none had been touched by P6's own separate sweep, which fixed a DIFFERENT stale `topicCertainty` citation at `.wf-cu-kind` — not one of this list's four): `index.css`'s `--color-topic-marker` comment (past-tensed the `TideRunRow.jsx` citation, P6 deleted it), `index.css`'s doors comment (rewritten for the one surviving door, past-tensing "two buttons"), `mapOverlay.js`'s two `locationNames`/"hot-topic click" comments (past-tensed to name the machinery producer-less today, deliberately kept for `test/mapOverlay.test.js` and `kind:'event'`'s shared code path — matching P6's own reasoning for not touching that machinery), and `v1-retirement-plan.md`'s stale `auroraTonight`/`auroraTomorrow` "keep a reader" claim (past-tensed with a pointer at P6's actual deletion). **Adversarial review** (four parallel read-only lenses: backend correctness, frontend runtime correctness, CSS/tokens/accessibility, test quality/conventions) found two real defects, both fixed: (1) the first cut matched the design bundle's literal `.tab{display:inline-flex;gap:8px}` container rule, which — applied to `.wf-tab` — doubled the Plan/Map tabs' own glyph-to-label spacing (they already bake a trailing space into the glyph span's text, so a container `gap` on top widened both tabs by the space's width PLUS the new 8px, a real regression on tabs this phase never meant to touch); fixed by margining the badge span itself (`margin-left:8px`) instead of touching `.wf-tab`'s layout at all — identical visual result on Coming up, zero blast radius on its siblings. (2) `.wf-cu-since-bits` used `--color-plex-text-muted`, which composites to ≈3.48:1 against the since-line's own coral-tinted background — under AA's 4.5:1 floor at 10.5px, the exact mistake this file has already made and corrected twice before (`--color-plex-text-secondary` fixes it, ≈6.5:1+); switched to match. Also fixed from the same review round, all minor: `Mark seen` now reconciles with the server's own echoed `comingUpLastSeenDate` on a successful write rather than trusting only its own optimistic `todayStr` guess (closes a narrow but real clock-skew/midnight-race inconsistency window the bootstrap write's own reconciliation didn't have a matching gap for); the census fixture now sorts its merged event list to match `AlmanacService.build()`'s own `.sorted()` call exactly (confirmed inconsequential to the measured counts — it only affects `markFirstOfType`'s prose selection, not scoring — but a genuine, if small, fidelity gap against "assembled the way production assembles it" otherwise); `ForecastHorizon.civilDate` gained its own direct unit tests in `ForecastHorizonTest` (previously exercised only indirectly via `UserSettingsServiceTest`, which does correctly pin the true BST boundary). Backend gate green (7794 tests, JaCoCo/SpotBugs/Checkstyle clean); frontend gate green (176 files / 4122 tests, lint clean via the ACTUAL `npm run lint` script — `eslint src --max-warnings 0` — not a bare `npx eslint .`, which also sweeps the gitignored `frontend/dist/` build output and reports over a thousand irrelevant errors from the generated service-worker bundle; `npm audit --audit-level=high` 0 vulnerabilities; build clean). **Browser-verified live against the real local backend** (H2, `-Plocal-dev` on 8083, `admin`/`golden2026`, `solar-utils` already cached at the correct coordinate on this machine so no JitPack workaround was needed this time): the eager fetch, confirmed via network trace — `/api/almanac?days=90` fires on first paint while the Plan tab is the one on screen, before Coming up is ever opened, proving D13's whole point; the announce badge state, driven by directly editing `app_user.coming_up_last_seen_at` in the local H2 file (stop → `RunScript` → restart, this session's own working recipe since `AUTO_SERVER=TRUE`'s TCP handshake would not authenticate for a live edit) against a REAL served entry (`Orionids`, 9.5 bits) rather than a fixture — the tab's `aria-label` read exactly `"Coming up, 1 new announced event"`, the since-line rendered the server's own `scoreNote` verbatim, and the chronology card carried both `wf-cu-card-fresh` and the NEW flag; the bootstrap write, confirmed by reading `GET /api/user/settings` before (`comingUpLastSeenDate: null`, no badge) and after opening the tab (`comingUpLastSeenDate` set to the server's own today, still no badge — the quiet bias) with no manual date edit in between; `Mark seen`'s full round trip, confirmed by the tab badge, the since-line AND the entry's NEW flag/fresh box-shadow all clearing together, and the server's response confirming the persisted date. Two things seen only via the test suite, both stated rather than implied away: the interrupt (solid ◆) badge state has no naturally-occurring entry in this local dataset (the highest real score is the Orionids' 9.5, just under the 10.0 edge) — matching the exact precedent P3b's own phase-log row already recorded for an analogous local-data gap; and the admin four-tab 320px overflow figure could not be re-measured with a real Map tab present (this sandbox's local DB has no seeded briefing/forecast data, so `mapPane` never mounts) — measured instead at the three reachable tabs (Plan/Coming up/Operations): 0px overflow with no badge, **9px** with a single-digit announce badge showing, smaller than the badge's own 26px growth because ordinary flex-shrink on the neighbouring tabs absorbs most of it before anything spills into the pre-existing `overflow-x:auto` scroller — recorded in `index.css`'s own comment beside the pre-existing 22px (no-badge, four-tab) figure it sits next to. **What remains open, by design**: §11's still-recorded gaps (the lone-tide-run threshold line, §11.21; the standalone `handoffFilterAction` clearing gap, §11.23, whose weight P6 already noted grows once `coming-up` became the sole producer); hysteresis, deliberately absent from v1 (D4 — no prior-band store until P7's `topic_daily_log` accrues one); and the plan's own instruction to revisit BOTH kinds of interim constant — the recurrent-topic rarity fallbacks (dust, inversion) AND the two deterministic rarity gaps this phase measured and deliberately kept (supermoon, eclipse) — after roughly 90 days of `topic_daily_log` rows exist to derive real figures from. |
...
13. **The band edges will not survive contact with the inventory** — the design's own placeholder
    ...
    while its own arithmetic scores its 12 Sept run 8.2 = Announced). The over-fire half is
    resolved at P5's census — edges move (`interrupt` 9.5→10.0); no topic is special-cased. ⚠️
    **The spring-run contradiction is a SEPARATE half, and P5 did not resolve it — it stays
    deferred, with a stated tripwire.** The census never wires in `TideAlmanacSource` at all (see
    `ComingUpAnnualBadgeCensusTest`'s own class Javadoc), so it says nothing about what a real
    tide-run magnitude would score. What actually keeps the design's own contradictory 8.2-bit
    example from badging today is the unrelated `interim` exclusion (P2's addition): every port is
    still cold-started (D4: maturity is ~60 stored run-peak observations, ~2.5 years out — the
    12-month backfill yields only ~25), so every tide-run entry ships `interim: true` and is
    excluded from badging regardless of which band it would otherwise clear. That protection has
    an expiry nothing currently watches: once any port's history matures past cold start, its
    tide-run entries stop being `interim` and start competing for a band on their own real
    magnitude — a scenario never measured here. Revisit by wiring `TideAlmanacSource` into the
    census with a matured port's real history once that happens; there is no automated tripwire
    for it today, only this note.
...
21. **A lone tide run in the window ships `threshold: null`, not silently — this is a recorded gap,
    ...
    intentional.
22. **Month abbreviations are the house `en-GB` short form (`Sept`), not the design bundle's
    three-letter `SEP`.** The design spells every month at three letters, including September;
    this app's existing date columns (`HeatmapGrid`, `conversions.js`, and the pre-P3a "Coming up"
    derivation itself) all use `Intl`'s `en-GB` short month, which renders September as `Sept`.
    Matching the established house convention was judged more important than matching the design
    bundle's one-off spelling on this single point of type; both are otherwise pixel-identical.
23. **`MapView`'s `handoffFilterAction` effect still only ever turns a type filter ON — the P3b fix
    is narrower than it reads.** P3b's own combined `handoffDarkSky`/`handoffFilterAction` effect
    (Codex's P1 finding on the pushed PR) covers exactly one sequence: two `coming-up` handoffs in a
    row, on the persistent Map-tab pane. It fixed that by having `coming-up` set BOTH flags together
    on every dispatch, guarded on `handoffDarkSky != null` so it can never fire for an unrelated
    trigger. The STANDALONE `handoffFilterAction` effect above it — the one topic pills use — is
    untouched and still has no clearing branch at all: a `coming-up` coastal-spots handoff followed
    by ANY trigger that carries no `filterAction` and no `darkSky` (a location drilldown, a region
    row, an event trigger) leaves the SEASCAPE filter latched into a view about something else
    entirely. This is the same shape of bug the topic-pill path already had before this phase — not
    new — but **its weight changes after P6**: `kind:'topic'`/`HotTopicStrip` is `handoffFilterAction`'s
    only other producer today, and P6 deletes it, so once P6 lands the `coming-up` channel becomes
    the PRINCIPAL (and for a while, only) producer of `filterAction`-bearing map handoffs — a stale
    filter surviving into an unrelated later view stops being a narrow edge case and becomes the
    default risk on every Map-tab visit that follows a coastal-spots tap. Left as a recorded, scoped
    follow-up rather than widened under review-fix time pressure (the same treatment §11.21 gives
    the lone-tide-run threshold gap) — but whoever executes P6 should read this entry FIRST and
    decide whether closing it is now in scope, since P6 is exactly the change that makes it matter
    more.

    **P6 decision (2026-08-29): kept as a recorded gap, ownership note updated, NOT closed.**
    `kind:'topic'`/`HotTopicStrip` are now deleted, so `coming-up` is `handoffFilterAction`'s SOLE
    producer — the "only other producer" language above is stale and the risk this entry warned
    about is now live on every Map-tab visit that follows a coastal-spots tap, exactly as predicted.
    Closing it was considered and declined: the mechanical fix (fold the standalone effect into the
    nonce-keyed, bidirectional shape `handoffDarkSky` already uses, clearing `activeTypeFilters`
    unconditionally on every handoff) changes behaviour for every trigger kind, not just
    `coming-up` — a reader who sets a type filter by hand in the Advanced panel and then taps an
    unrelated location drilldown or region row would find it silently cleared, which is a new UX
    claim ("every navigation clears manual filters") that P6's own brief never asked for and has no
    test coverage plan behind it. That is a design decision for whoever next touches `MapView`'s
    filter effects, not a mechanical follow-on to a deletion phase.
...
// GET /api/almanac?days=90 → ComingUpResponse
{
  ...
  "bands": { "list": 5.0, "announce": 7.5, "interrupt": 10.0 },  // P2 shape; P5's census set interrupt 9.5→10.0; lower-inclusive
  ...
  "conditions": [                        // P4 (empty [] from P1)
    {
      ...
      "occurrences": [
        { "date": "2026-08-30", "dateLabel": "30 Aug", "valueLabel": "4.8 m",
          "label": "Spring tide",               // P4, added under the deviation protocol — which
                                                 // run kind this occurrence is, read from the
                                                 // source event's own type, never re-derived; null
                                                 // where a condition has only one kind (dust,
                                                 // inversion). Needed because D11 puts spring AND
                                                 // king runs in one row, so nothing else in the
                                                 // occurrence answers "which is this one"
          "bits": 5.4, "reason": null,          // reason: "max w/ supermoon" etc. — mandatory
                                                 // wherever the max was taken (D10), BOTH
                                                 // directions, naming whichever topic actually
                                                 // carried the score even when it was this one
