...
against a seeded four-region catalogue with real drive times. Its row records what it decided,
including **five defects that a green suite, a clean lint and a successful build had all passed
over** — a row printing another location's clock beside a departure derived from its own, a
provisional mark reporting a different region's confidence, a rating join on the display name that
the provider had *already written a note against naming this phase*, a lead line counting our own
database rows, and a dialog whose entire accessible name asserted a window count the payload does
not guarantee.

**⚠️ THIS IS THE LAST PHASE OF THE SERIES — nothing follows it.** The heat field is complete: the
kernel, the strip, the open row, the Map tab, leave-by, movement, the origin and now the sheet. What
remains on this arm is not a heat-field phase but the window-first arm's own **flag flip** and the
v1 deletion that follows it (`window-first-p15`), plus the still-open §9 questions below. Do not
read a P9 into this document.
...
- Radius in real distance: `radiusFor(map, 8500, 34, 240)`; `grid: 6`, blur 4, `conf` from the
  ...
  live in the same zoom handler). ⚠️ **Base opacity superseded 0.9 → 0.92, 2026-09-03** — the
  map-tab-v2 bundle moved the map surface's own figure and `map-tab-v2-plan.md` §4 item 16
  conforms `MapHeatLayer.jsx` to it (radius/fade/floor here were separately re-tuned by that
  plan's §3 P4 and are current as written there).
...
| **P8 — four-day location sheet** | ~~Per-location six-window timeline (ratings + `summary` why from the scores rows, leave-by), location-card entry (+ search entry if P7 landed)~~ **DONE 2026-08-20** (`feature/heat-p8-sheet`). **§9.9 is resolved — the sheet hangs off the SEARCH only** (owner, 2026-08-20): a spot card's click and the peek keep today's map-open behaviour byte-for-byte, and `WindowSpotStrip.test.jsx` / `WindowSpotPeek.test.jsx` passing **unedited** is the proof (verified in the browser too — a card click still opens the map overlay). **Eight decisions worth not re-deriving:** (1) **The departure names its day, and the wrap is now real.** P7's warning in `leaveBy.js` named this exact surface as the one that would reopen the wrapped-`HH:mm` ambiguity, because search matches the whole roster while every drive a spot card sees is inside a measured ceiling. It is closed with a **second function** — `leaveByParts` returns `{time, date, sameDay}` and the row prints `↰ leave Thu 23:50` — rather than by widening `leaveBy`, whose shape is unchanged: it runs once per rendered card and per peek, and `formatInstantUk` builds an `Intl` formatter per call. The two share one `departure()` helper, and a test table pins them equal across a wrap. **Nothing is conditioned on a drive-length threshold** — that ceiling has already moved twice. Seen in the browser at a 5h50 drive to an 06:00 sunrise. (2) **The drive is measured from the page's ORIGIN**, not the home map. At home the two are the same object, so this is the prototype's "home minutes"; away it is base-measured, which is what the lens bar above it already says, and the header **names the base** ("3h 10min from Keswick"). Home minutes under an away origin would put two journeys on one screen. (3) **The outside marker NAMES its scope.** The prototype's bare "outside your plan" means the home planning area at home and region membership away, and only one of those is about distance — a near spot in a neighbouring region wore the badge directly over its own short drive and read as a broken filter. It now says `outside The Lake District` / `outside your 3h area`. `planOrigin.scopeRegions` was extracted for it (behaviour-identical; `planOrigin.test.js` passes unedited) because asking `scopeSpots` a one-spot question computes the planning area *from that one spot*, and matching by object identity breaks across a poll. **An unknown OR EMPTY scope marks nothing.** (4) ⚠️ **The rating and the "why" come from ONE score row, joined ID-FIRST.** The first cut read the provider's name-keyed `scoreIndex` — against a comment the provider had already written *naming P8* — so the sheet timed its rows id-first and rated them name-only: a renamed location showed a correct departure under "Not scored yet" while the heat field behind it still painted its star, and two locations sharing a display name swapped ratings *and* prose. `scoreRows` is now published on the context and `buildScoreIndex` joins it properly. (5) ⚠️ **The confidence is the LOCATION'S OWN region's**, never `card.confidence`, which `buildWindowCards` documents as the *top region's* — right for a roster-wide thumbnail, wrong for one place in another region, and silently wrong in both directions. It rides `BriefingRegion.confidence` off the same `regions[]` the event-time index already walks. Null on an unrated row: the channel qualifies a forecast, and "Not forecast · provisional" qualifies an absence. (6) ⚠️ **The row's clock is the LOCATION'S own**, not `card.time`. The window header's time is the roster-wide earliest (or an order-dependent first slot on the fallback path), so the first cut printed somebody else's sunset one line above a departure derived from the right one — four minutes apart in its own fixture, with nothing on screen to explain the gap. `card.time` survives only as the fallback for a window with no slot. (7) **The lead line states NO denominator.** "N of the scored windows" was built and removed: §6's sweep bans counts of our own data, and "3 scored windows" counts evaluation rows. `The next 4 days here · 2 windows at 4★+`, or `none at 4★+`, which is the sheet's only "don't bother" signal. It says **nothing** while `scoresLoaded` is false — an unfetched request is not evidence that nothing was rated. (8) **"best here" needs more than one rated window** (a max over one is not a comparison — the tide runs' own rule), the map handoff never picks an away window while a forecast one exists and names the window in the strip's vocabulary, and the accessible name **counts** the rows it renders rather than asserting six. **Deliberately NOT built:** the prototype's `◎ Plan from <region> →` footer — moving the origin from inside an open sheet would swap the drive, the base named beside it, the outside badge and every departure on every row while the reader watches, and search is one keystroke away. **Layout:** the lead line is a CHILD of the row scroller, not a fourth pinned band — with four bands a 320×256 viewport clipped the map action with nothing able to scroll (verified fixed in the browser). | `utils/locationSheet.js`, `components/LocationFourDaySheet.jsx`, `utils/leaveBy.js` (`leaveByParts`), `utils/planOrigin.js` (`scopeRegions`), `context/WindowFirstBriefingContext.jsx` (publishes `scoreRows`), `WindowFirstShell.jsx`, `index.css`, 5 test files | ✅ Owner accepted the search-only entry (§9.9); peek and map-open behaviours preserved and proven by unedited suites; frontend gate green (lint, 4412 tests, audit, build) |
...
9. ~~**P8 scope**~~ **RESOLVED — the search only (owner, 2026-08-20)**: the four-day sheet hangs
   off a search location result; a spot-card click and the hover peek keep today's map-open
   behaviour byte-for-byte, proven by their suites passing unedited. Built in P8.
