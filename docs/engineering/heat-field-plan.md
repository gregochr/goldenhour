...
recipe needs a backend restart (§7.3)). **P0 is BUILT** on `feature/heat-p0-kernel` (2026-08-18,
not yet merged) — kernel, ramp, vendored coastline and §7.1 tests, adversarially reviewed (six
lenses) with the surviving findings applied; see the P0 row for what it decided and what it
deliberately left to later phases. **D1 is CONFIRMED (owner,
