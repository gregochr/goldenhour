...
| **Aurora real-time poll** | `AuroraPollingJob.poll()` → `AuroraOrchestrator.runForecastLookahead()` + `.run()` — [AuroraOrchestrator.java:108](backend/src/main/java/com/gregochr/goldenhour/service/aurora/AuroraOrchestrator.java:108). *Since 2026-09-14: `runForecastLookahead()` in daylight or `runNightPoll()` after dark — `run()` is gone.* | Updates `AuroraStateCache` (in-memory FSM). *Corrected 2026-09-14: this said it "triggers Pushover/email"; it never did — NOTIFY only scores.* |
