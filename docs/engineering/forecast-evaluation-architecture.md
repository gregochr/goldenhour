...
> **Since 2026-09-14** the entry points are `AuroraOrchestrator.runForecastLookahead(tonight, now)` (daylight) and `runNightPoll(tonight, now)` (after dark), both called by `AuroraPollingJob`; `run()` is gone. The admin route is `AuroraAdminController.triggerRun()` (`POST /api/aurora/admin/run`), which runs the scheduled cycle itself through `AuroraPollingJob.runCycleIfIdle()` (409 while one is running). The Claude call has gone through `EvaluationService.evaluateNow` since Pass 3.2, which also added the job_run and api_call_log rows this table says are missing.
