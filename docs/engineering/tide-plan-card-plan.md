...
**Status: COMPLETE — C0–C4 shipped.** Five phases (C0–C4; C0 was added on the plan
...
9. **`heightAtWindow` was not added as a fifth popup fact.** §1 #7 named it as "the natural companion
   fact if the phase wants the height the spec's tooltip names"; C3 declined it — a judgement call,
   not an oversight, recorded in its own phase-log row (`WindowSheetDialog.jsx`'s `tideFitFacts` memo
   carries only the reach-scoped count). Left open at §6 Q7 for an owner to reconsider; C4 did not add
   it, since adding a served fact belongs to `windowFirstRows.js#tideFacts` and is a code change this
   sweep does not make.
...
- **Q6 — C1's two advisory review notes.** C1's phase log records "two low-confidence, explicitly
  ...
  one of these two.
- **Q7 — `heightAtWindow` as a fifth popup fact (§4 #9).** C3 declined it for file-scope and
  marginal-benefit reasons (its own phase-log row), not because it is wrong to add. Still open: an
  owner who wants the exact metre figure on the popup's tide row, rather than the implicit range ·
  anomaly fact already there, can add `heightAtWindow` to `windowFirstRows.js#tideFacts` (it is
  served, so it belongs in that module, unlike the reach-scoped count — §5 #6) as its own small
  change.
