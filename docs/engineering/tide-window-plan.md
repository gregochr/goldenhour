...
**Status: T1, T2, T3, T5 and T6 BUILT (T5 merged to main as #879); T4 pending PR #880; T7, T8 not
started.** T5 and T6 were built in parallel with T4 (each in its own worktree, per §8) and none
touches a file either of the others claims — confirmed mechanically: merging `origin/main` (after
T5 landed) into T6's branch conflicted only in `windowFirstRows.js`, `mapTideFit.test.js` and this
document's own phase log, all three exactly where two phases were each independently extending the
same shared, small surface (`STATE_WORD`'s export, `mapTideFit.test.js`'s import list, this table),
never a case of one phase's own feature work colliding with another's. T1's
`TideCurveCalculator` lift and T2's window
...
resolution carries it out. Owner decisions this plan needs are listed in §6; **none blocks T3**,
...
12. **Collapse persists in `MapView` state, not storage.** The pane is never unmounted
    ...
    and its try/catch.
13. **The sheet's denial states its own numeral horizon, not the design's literal "four days"** —
