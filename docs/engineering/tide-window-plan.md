...
**Status: T1–T8 BUILT — the series is complete.** All eight phases have merged to main: T1 #877,
T2 #876, T3 #878 (plus a same-branch Codex follow-up, `1507aa3c`, folded into the squash before
merge), T4 #880, T5 #879, T6 #881, T7 #882, T8 this docs sweep. T1's `TideCurveCalculator` lift and
T2's window facts were built in parallel sessions against the same `75ff1f90` base and merged
independently (T2 first, as #876; T1's merge commit folds the two together) — exactly the
parallel-phase shape §8 describes for T1/T2. Post-merge, `WindowTideRollupBuilder.rollup` calls
`TideCurveCalculator` for every one of T2's new facts (`positionOf`, `clockMinutesFrom`,
...
|---|---|---|---|---|
| T2 | `feature/tide-t2-window-facts` | `3bed6281` (#876) | 2026-09-17 | Adds `sunrisePosition`/`sunsetPosition`, `extremes` (`record Extreme(kind, position, time)`) and `heightAtWindow` to `BriefingWindowTide`, computed inside `WindowTideRollupBuilder.rollup` from data it already fetches — both solar events now called unconditionally rather than only the window's own (T1 had not merged; computed from the private statics in place per T2's instruction 2). New legacy twelve-field constructor mirrors `BriefingSlot.TideInfo`'s own, keeping the three existing direct-construction call sites (`BriefingEventSummaryWindowSerializationTest`, `PlanWindowProjectorTest`, `BriefingServiceTest`) compiling unchanged. ⚠️ Adversarial review (3 read-only lenses: correctness/wiring, test quality, docs/contract/conventions) found one real test gap and two documentation overclaims, all fixed pre-commit. Test gap: both `heightAtWindow` tests were degenerate — one landed exactly on a stored extreme (f=1, collapsing interpolation to the raw height), the other used a flat day (every extreme equal) — so the cosine arithmetic itself was unverified; added a hand-computed interior-point case (11:00 between an 08:35 high and a 14:45 low, height = 3.8996 m → `"3.9 m"`) plus the symmetric sunset-null guard. Documentation: the `sunrisePosition`/`sunsetPosition` Javadoc claimed the sun-does-not-rise-or-set case is a live path "for a high-latitude anchor near midsummer" — empirically false of the vendored solar-utils 2.1.0 (`SolarCalculator.sunrise`/`sunset` at 78°N returns a degenerate midnight instant, never null, confirmed by direct invocation); reworded to state the real justification — `SolarService`'s contract carries no non-null guarantee, and `NlcTwilightWindowCalculator` already treats the identical call as nullable for the same reason. And `extremes`/`heightAtWindow`'s Javadoc claimed nullability was for "a payload cached before this field existed" — false: `BriefingHierarchyBuilder.buildSummary` always attaches `window = null` on the build path `persistBriefing` serialises, so `BriefingWindow`/`BriefingWindowTide` is never itself part of `daily_briefing_cache`, old shape or new; corrected to name the true reason, the legacy twelve-field constructor's existing call sites. Gate green: `./mvnw checkstyle:check` 0 violations, `./mvnw clean verify -Dtest='!**/integration/**'` 8156 tests, 0 failures, 0 errors, BUILD SUCCESS. No browser-visible change — backend only. |
| T1 — per-slot tide facts (backend) | `feature/tide-t1-slot-facts` | `f52a20ac` (#877) | 2026-09-17 | Re-verified every file:line the plan cited against the tree before editing — all current, no drift since 2026-09-17. **Task 5's stop condition did not fire**: `TideFactDeriver.java:96–97` still computes `tideAligned` from the *tight* `tideData` and `widenedAligned` from `dualMaybe.get().widened()` separately, `BriefingSlotBuilder` still serves the tight one, and the new tide-fit fields read that same served field — confirmed by a dedicated test (`tideFitFieldsTrackTightAlignmentNotWidened`) that gives `calculateTideAligned` two distinguishable `TideData` objects and proves the miss form prints when tight says no even though widened says yes. **The lift is behaviourally identical**: `WindowTideRollupBuilderTest` (43 tests) passes with a literal `git diff --stat` of zero on that file, both before and after a follow-on cleanup that also moved the day-axis position formula (`positionOf`) and the rounding helper into `TideCurveCalculator`, deleting the two duplicate copies the first cut had left behind in `WindowTideRollupBuilder`. ⚠️ **Adversarial review (5 lenses: correctness, the lift's behavioural identity, test quality, Checkstyle/Javadoc, what it makes harder for T2/T3) found one crash the local gate's own green run had not exercised**: `curveFacts` guarded on the fetched rows being non-empty, not on the *date-filtered* series `TideCurveCalculator.seriesAround` narrows them to — a location whose stored extremes sit right at the ingestion horizon, briefed two days past it, hit a non-empty fetch and an empty series, and `heightAt` indexed off the end of it. Fixed with a second guard and a regression test that reproduces the exact `IndexOutOfBoundsException` (proven by removing the guard and watching the new test fail on cue, then restoring it). A second, narrower finding — the miss phrase's height could print a fraction above the "of X m" day-high figure beside it, since one is an unclamped interpolation and the other a 30-minute sample max — is fixed by flooring the day-high at the light's own height. Test suite grew from the review pass too: eq()-pinned mock windows in place of `any()` where the value was knowable, a tight-vs-widened distinguishing test, a BST case for the date/clock conversions, boundary tests at `bracket`'s exact-midnight edges and the flat-span threshold, and an exact-string assertion for the match-tier fit phrase that had been asserted only by its component parts before. **Post-T2-merge**: resolved the merge against `origin/main` (T2 had landed as #876 while T1 was in review) by keeping T1's `TideCurveCalculator` delegation and re-pointing T2's `sunrisePosition`/`sunsetPosition`/`extremes`/`heightAtWindow` computations onto it — `WindowTideRollupBuilder` now calls `TideCurveCalculator.heightAt`/`clockMinutesFrom`/`positionOf` exclusively, with `heightAt` for the event hoisted into one local (`heightAtEvent`) so `windowLevel` and `heightAtWindow` share the one call rather than each computing it separately. `DailyBriefingResponseJsonTest` keeps both phases' test sections, unmodified, one after the other. Nothing in T1 is browser-visible. |
| T3 — plumbing and `mapTideFit` (frontend) | `feature/tide-t3-plumbing` | `0f9273ca` (#878) — plus a same-branch Codex fix, `1507aa3c`, folded into the squash | 2026-09-17 | Re-verified every symbol T3 names against the tree before editing — all field names (`BriefingSlot.TideInfo`'s `tideLevel`/`tideDirection`/`tideHeight`/`tideShortfall`/`tideFitPhrase`, `BriefingWindowTide`'s `sunrisePosition`/`sunsetPosition`/`extremes`/`heightAtWindow`) matched T1/T2's merged shape exactly, no drift. All five T3 tasks built as specified: `buildTideAlignmentIndex` now skips only on a missing `tideState` (not `tideOnTheLight`) and carries the full `{aligned, onTheLight, phrase, level, direction, height, shortfall, fitPhrase, gated}` family; `spotOf` copies `tideTypes`/`coastal`/`tideTier`/`tideShortfall`/`tideGated` onto `labelSpots`; `mapEvents.solarRow` forwards `tide: served.tide ?? null` on the served branch and `null` on the filler branch, the D-13 interleave untouched; the new `utils/mapTideFit.js` holds `tierOf`/`nextAlignedRow`/`stripModel` exactly to the tie-break (HIGH > LOW > MID) and `bounds.pad(0.12)` spec, with `stripModel` taking three params beyond the plan's own illustrative `{row, spots, bounds}` (`evRows`/`evIndex`/`idx`) because `nextFitRow` cannot be answered without them — recorded as a deliberate, documented deviation rather than a silent one. ⚠️ **Adversarial review (4 read-only lenses: runtime behaviour, project conventions including the Backend-heavy licence and the two-tide-axes rule, test quality, what it makes harder for T4–T6) found and fixed six things, two of them independently by separate lenses**: `spotOf` never set an `id` on label spots, so `nextAlignedRow`'s documented-and-tested id-first lookup was unreachable in production — a silent trap for the first two coastal locations ever to share a display name (found by both the runtime-behaviour and the T4–T6-impact lens independently); `spotOf` was also missing `tideFitPhrase`, which T4's own tooltip requirement (plan text, not inferred) needs for both tiers — added rather than left for T4 to discover, since T3's stated goal is that every fact reaches its future reader; `mapTideFit.js` named its tide-index parameter `index`, sitting beside the unrelated array-position `evIndex` in the same call and inviting exactly the kind of misread CLAUDE.md's Jackson-mapper story warns about — renamed to `idx`, matching `locationSheet.js#lookupForWindow`'s own parameter name; the `bounds.pad(0.12)` boundary tests bounded the ratio to a loose `(0.05, 1.0)` range rather than pinning the documented `0.12` figure itself — tightened to bracket the true edge (1.10 in, 1.14 out) so a mutation to any other common ratio fails at least one of the pair; a "does not disturb the interleave" test duplicated an existing ordering test's exact fixture and re-asserted tide values a sibling test already pinned, violating one-behaviour-per-test for no added mutation coverage — trimmed to a smaller, distinct fixture asserting order alone; `buildTideAlignmentIndex`'s now much-larger doc comment sat above `buildEvaluationGateIndex`'s declaration rather than its own (a pre-existing ordering issue T3 tripled in size) — reordered so each doc sits directly above the function it describes. One further, out-of-file fix: `MapCallout.jsx`'s existing `tideOnLight` prop doc, untouched by T3's own task list, asserted "never `tideAligned`, a different question" — literally true of what that component currently renders, but now false as a description of the object's *shape*, which T3 changed; corrected to state both axes are present and why reading the preference one is safe here, so a T5 author reading that file first is not misled into building a duplicate lookup path. **Declined, and documented rather than fixed**: `stripModel` computes the viewport-dependent and window-dependent halves of its answer in one function, which prevents the exact `[evIndex, idx]` vs `mapBounds` two-memo split T3's own task text describes for `MapView` (T6) — flagged by the project-conventions lens as a real but *inert* tension, since T3 wires no call site into `MapView` at all (confirmed by grep) and restructuring a module against a consumer that does not exist yet would be speculative complexity against CLAUDE.md's "extend, never fork" spirit; a `⚠️` doc block records the tension and the concrete split T6 should make if the whole-function recompute on every pan ever proves measurably expensive. Gate green: `npm run lint && npm test && npm audit --audit-level=high && npm run build` — 6302 tests, 0 failures, 0 vulnerabilities, build succeeded. **Browser-verified, not merely asserted**: backend + frontend run locally per §9, three coastal locations seeded with real `tide_extreme` rows (one wanting LOW, one HIGH, one both — reaching the null-shortfall arrow case), a briefing built and served — `GET /api/briefing` carried every T1/T2/T3 field correctly, including `tideFitPhrase: "wants high water or low water · mid tide, falling…"` with `tideShortfall: null` for the two-value want exactly as designed. The Map tab, its callout, its four-day location sheet and Pins mode all rendered with the seeded data, zero console errors, zero failed network requests, across a `computer` screenshot at each step. Not seen in the browser: an aligned (`match`-tier) chip on screen — the seeded synthetic tide cycles happened to miss every window in the fixture window, so that state is tested by the automated suite (`tierOf`, `buildTideAlignmentIndex`'s "reads every tide-fit field" case) rather than seen. |
| T4 — the chip: dimmed, not dropped (frontend) | `feature/tide-t4-chip` | `d13075f7` (#880) | 2026-09-17 | Re-verified every file:line the plan and T3's own merged shape cited against the tree before editing — `spotOf`'s `tideTier`/`tideShortfall`/`tideFitPhrase`/`onTheLight` fields, `buildTideAlignmentIndex`'s full family and `buildRegionLocationRows`'s prior `tideOnLight` boolean all matched exactly, no drift since T3 merged. All seven tasks built as specified: `visibleLocations` lets a served tide fact past the rating stage via `if (getTideOnLightForLocation(loc)) return true;` before the wildlife/`showUnrated` branch, with `hasUnrated` excluding the same locations from the admin "unknown" toggle's own count; the chip's `data-tide` moved to `'match'`\|`'miss'`, `.wf-maplab-chip[data-tide='miss']` dims to `opacity:.72` (restored on hover/selection), `TideWave` grew a `shortfall` prop drawing the bundle's 21×8 arrow (up for `HIGHER`, down for `LOWER`, the plain wave for null) behind a new `data-wide` attribute the CSS widens on; the label-budget tiebreak in `utils/mapLabels.js` promotes a match and never demotes a miss; the tooltip's third line reads the served `fitPhrase` for both tiers behind two new shared helpers (`mapTideFit.tideTierHeading`/`tideAccessibleClause`); Pins mode (`PinsLayer.jsx`) gained tide data for the first time (`data-tide`, the same `.72` dim, the same third tooltip line); the region panel's row and `mapDrilldown.buildRegionLocationRows` moved from `tideOnLight` onto `tideTier`/`tideShortfall`. ⚠️ **Adversarial review (6 read-only lenses: runtime behaviour, CSS/tokens, test quality, accessibility, project conventions, impact on T5/T6) found and fixed five things, confirmed three deliberate choices as sound rather than silently accepting them**: `PinsLayer`'s pin carried no accessible-name statement of the tide fact at all — only a mouse-hover tooltip, unlike the chip's aria-label and the region panel's `sr-only` span — so a keyboard/screen-reader user in Pins mode had no textual equivalent of the visual dim; fixed by extending the pin's aria-label with the same `tideAccessibleClause` the chip uses (the pre-existing, file-wide lack of a keyboard trigger for either tooltip was left alone as out of this phase's scope, since it is identical for a match and predates T4). A CSS comment overclaimed `--color-plex-text-secondary` (.66) as "the mapped token for" the design's bespoke `.72` miss-tooltip ink; corrected to name it as the closest existing token with an honest ~0.06 gap stated. `toBeTruthy()` on one new chip-glyph assertion replaced with `toBeInTheDocument()` (the banned pattern per the frontend test standards). Two test gaps closed: a missing swatch-fill-unchanged assertion for a miss chip (§7 check 12 has two halves — the star ink was pinned, the swatch fill was not) and a missing gated-miss (no rating + `tideTier:'miss'`) combination test at the chip level, since `hasRating` and `tideTier` branch independently and every other miss test paired a miss with a real rating. Two §7 checks strengthened to match their own literal wording: check 1 (heat untouched) compared only HIGH vs LOW, now also MID — the design's own "straddling want" case and the most plausible target for a "dim the field too" mistake; check 2 (nothing dropped) flipped only one location between fixtures, now every coastal slot in the pool at once, with an added premise assertion that the tide index actually reached every rendered spot. **Confirmed sound, not changed**: the miss tooltip's canonical heading ("Wrong water, not wrong light") diverges from the plan text's literal quoted example ("Wants high water —", the design bundle's own want-formatted tooltip line) — recorded as a deliberate reuse of the callout/sheet block's own heading in `tideTierHeading`'s doc, since formatting the want client-side would violate "the client concatenates headings to phrases and nothing else" and would restate what the served `fitPhrase` already says; the rating-stage bypass admits ANY served tide fact rather than only a served-`gated` one, which is §5 #9's own literal worked example (`if (tideFact) return true`) even though its prose says "tide-gated…and nothing else" — narrowing it would reopen the "coast disappears" defect for a miss rated null for an unrelated reason, so the breadth is deliberate and now commented at the call site; `.wf-reg-tide`'s ink stays `--color-tide` for both tiers (no opacity split on that row), matching the design bundle's own region-row rule, which likewise has none. No confirmed runtime-behaviour or project-convention defects; `OBSTACLE_SELECTOR` untouched in both files as required. Gate green: `npm run lint` (0 warnings), `npm test` (249 files, 6324 tests, 0 failures — the full suite, re-run after every review fix), `npm audit --audit-level=high` (0 vulnerabilities), `npm run build` (succeeded). **Follow-up, once the shared machine's load eased (`uptime` load averages back to single digits): browser-verified against a real seeded backend, not merely asserted.** Backend on 8093 (local-dev profile), frontend on 5173 per §9; `scripts/dev-seed-locations.sh` for the catalogue; `location_tide_type` set directly (Bamburgh Beach → LOW, Dunstanburgh Castle → HIGH, Tynemouth Priory → HIGH+LOW — the null-shortfall case); three real `tide_extreme` cycles inserted (745-minute alternation, Bamburgh/Dunstanburgh phase-locked so a LOW extreme lands exactly on today's sunset, Tynemouth phase-shifted so its sunset falls at the cosine's midpoint — MID, more than the ~41-minute tight window from either extreme); `cached_evaluation` seeded for two non-coastal locations so `BriefingHonestyFilter` does not wipe the region's slots, then for Bamburgh (5★) and Dunstanburgh (2★) too, each requiring a restart for `rehydrateCacheOnStartup`. `GET /api/briefing` served exactly the designed triple: Bamburgh `tideState:"LOW", tideAligned:true` (match); Dunstanburgh `tideState:"LOW", tideAligned:false, tideShortfall:"HIGHER"` (miss, wants higher); Tynemouth `tideState:"MID", tideAligned:false, tideShortfall:null, evaluationGate` set (miss, gated, straddling want) — all three tiers and the null-shortfall case in one fixture, unplanned but welcome. On the running Map tab (Today sunset, `getComputedStyle` read directly off the live DOM rather than eyeballed): Bamburgh's chip and pin both `data-tide="match"`, `opacity:"1"`, plain-wave glyph (one `<path>`, no `data-wide`), aria-label `"Bamburgh Beach, 5 star, tide right here"`; Dunstanburgh's chip `data-tide="miss"`, `opacity:"0.72"`, glyph `data-wide="true"` with a SECOND path `"M17.5 6.8V1.2M15.4 3.3L17.5 1.2L19.6 3.3"` — the design bundle's up-arrow, byte-identical — aria-label `"Dunstanburgh Castle, 2 star, wants the water higher"`, swatch `background-color` unchanged from `rampHex(2)` (§7 check 12, live); Tynemouth's pin `data-tide="miss"`, `opacity:"0.72"`, plain wave (no `data-wide`), aria-label `"Tynemouth Priory, wrong water"`. Pins mode showed the same triple with the same `data-tide`/opacity pair. The chip tooltip (hovered live, screenshotted) read `"Tide lands on the light — low water, rising · LW 19:21 · at sunset · 0.9 m"` for Bamburgh, teal-inked, matching the served `fitPhrase` and canonical heading exactly. **Not seen, and not chased further**: the pin's own hover tooltip (a synthetic `dispatchEvent('mouseenter')` did not trigger React's handler, and the browser pane went "not displayed" mid-session before a real pointer hover could be tried) — the underlying render path is identical to the chip's own (already seen working), and both read the same `tideTierHeading`/served `fitPhrase`, so this is inference from a shared code path, not a repeated observation. **The `.72` vs `.8` chip-ink measurement (item 2) is answered analytically, not by a live pixel sample**: the Esri basemap tile is cross-origin, so a canvas read of the actual rendered pixel throws a tainted-canvas `SecurityError` (confirmed live); the browser pane became unavailable for a screenshot-based sample before a workaround could be tried. Computed instead from the real CSS values (`rgba(14,11,9,.84)` plate, `#F2E7D3` text, WCAG relative-luminance formula) composited over two representative basemaps — this app's own "darkest sampled tile" figure (`rgb(20,15,12)`) and an eyeballed light-land tone from a seeded screenshot (`rgb(170,165,150)`) — giving ≈8.5:1 and ≈6.2:1 respectively at `.72`, both clearing AA (4.5:1) with room to spare; `.8` computes higher still but bought nothing `.72` did not already clear, because the chip's own near-opaque plate (unlike `.wf-loc-row`'s translucent card, which genuinely needed `.8`) absorbs almost all of the outer opacity's effect on the LOCAL text-vs-plate contrast. `.72` (the design's value) is the one that shipped; see index.css's own comment on `.wf-maplab-chip[data-tide='miss']` for the full working. State plainly: the tide facts, the served fields, the rendered `data-tide`/opacity/glyph-path/aria-label state and one tooltip are BROWSER-SEEN via live DOM inspection; the pin's own tooltip and the exact contrast ratio are INFERRED/CALCULATED, not photographed. |
| T5 — the callout and the location sheet: one block, two tiers, and the jump (frontend) | `feature/tide-t5-block` | `96f9263a` (#879) | 2026-09-17 | Re-verified every symbol T5 names against the tree before editing — T1/T2/T3's merged shape matched exactly, no drift; `gh pr list --state open` showed no T4/T6 PRs yet (both were still mid-build in their own parallel worktrees). New `components/map/TideFitBlock.jsx` renders both served tiers (match/miss — the spec's "near" tier stays unbuilt per §4 #3) with the exact headings, the served `fitPhrase` as body, and on a miss either the text-button jump or the italic denial. ⚠️ **Deliberate, documented deviation from the plan's illustrative prop list**: the component takes `{fact, want, nextFitRow, onSelectEv, horizonWord}`, not `{fact, evRow, evRows, onSelectEv, horizonWord}` — each CALLER (`MapCallout`, `LocationFourDaySheet`) resolves the next-fit row once via T3's tested `nextAlignedRow` (any-want reading) and hands the component the resolved row object plus the location's own wanted set, so the "pure render" component reads no index, no height, no offset, no threshold at all (T3's own precedent for `stripModel`'s extra params). `MapCallout.jsx`'s `.wf-callout-tide` row becomes the block, keyed on `tideOnLight != null` with a `fitPhrase` rather than `onTheLight`; the gate row (`.wf-callout-gate`) is kept, and both a new `tideAlignmentIndex` prop (forwarded one line from `MapView.jsx`'s existing state) and the block itself join the repaint-dependency list the gate-index fix already established. `LocationFourDaySheet.jsx` gains a `tideAlignmentIndex` prop (threaded from a new memo in `WindowFirstShell.jsx`, gated on `sheetSpot` like its siblings) and renders the block on every solar row between the light-times line and the gate row; the next-fit jump calls a new `focusRow(key)` that opens and focuses the TARGET row on the same sheet (never the map underneath it, per `plan-to-map-doors-plan.md` §5 #4) — built via a `scanRows` adapter that reuses T3's `nextAlignedRow` rather than a second forward-scan. A client-side `mapTideFit.wantPhrase` (new) joins a location's wanted `TideType` set into words for the jump/denial's `<want>` clause, mirroring the backend's `TideWording#joinOr` exactly and reusing `windowFirstRows.js`'s newly-exported `STATE_WORD` rather than a second vocabulary; §1 #12's licensed-derivation list grows from four members to five to name it. `.wf-callout-tide*` CSS is renamed to a shared `.wf-tide-fit*` namespace (one component, two hosts) with a new miss tier built via `color-mix()` off `--color-plex-text` (§1 #9's tokens-only rule) — the match tier's existing `rgba(111,168,176,…)` numbers are `--color-tide`'s own RGB and are left untouched (confirmed byte-identical via `git diff`). §4 gains #13, recording that the sheet's denial states its own numeral horizon (`"3 days"`/`"4 days"`, `leadLine`'s own convention) rather than the design's literal "four days" — which the callout's default parameter still carries unmodified, since it has no computed horizon of its own — qualifying #8's blanket "verbatim" claim for this one line. ⚠️ **Adversarial review (5 read-only lenses: runtime behaviour, CSS/tokens, test quality, accessibility, project conventions/T4-T6-T8 impact) found and fixed seven things, two of them P1**: (1, P1, accessibility) the callout's jump had no focus rescue — activating it resolves to a MATCH window by `nextAlignedRow`'s own contract, so the jump/denial line (and the button under the reader's finger) stops rendering on the very next commit, and with no `key` remounting `MapCallout` React dropped focus to `<body>` exactly as `MapView.jsx`'s own orphaned-focus note describes for two other controls — fixed by focusing the card's own always-rendered close button first, mirroring the reason button's self-focus above it; (2, P1, runtime) `TideFitBlock`'s own `want`-derived jump/denial text reads the LIVE roster (`location.tideType`) while `fact.fitPhrase`'s "wants" clause is FROZEN inside a cached forecast row, so an admin editing a coastal location's tide preference between pipeline cycles could make the two clauses on one card disagree — closing it needs a backend field (the want the pipeline built the phrase against) outside this frontend-only phase's scope, so it is recorded in `TideFitBlock.jsx`'s own doc as an accepted, narrow risk rather than fixed; (3, P1, CSS) `.wf-tide-fit-denial` reused `--color-plex-text-muted` at 9.5px, a contrast failure (≈3.5:1, under AA's 4.5:1) this file has already corrected four-plus times elsewhere for the identical token/size combination — switched to `--color-plex-text-secondary`; (4, P2, runtime) the sheet's next-fit SCAN had no away-day guard even though the current row's own display does — the pipeline still collects (never evaluates) a travel day's tide extremes, so `buildTideAlignmentIndex` can carry a real aligned entry for an away date, and an unguarded scan could jump a reader to a row reading "Nothing was forecast for this day — away"; fixed by giving an away row's scan-adapter a non-solar `kind`, reusing `nextAlignedRow`'s own existing kind guard rather than adding a second, away-aware one to that shared function; (5, P2, accessibility) `focusRow`'s `.focus()` ran synchronously alongside `setOpen`, landing before React's commit flipped `aria-expanded` — deferred one `requestAnimationFrame`, `WindowFirstShell.jsx`'s own pattern for the identical shape; (6, P2, test quality) the "renders none on the away row" test was a tautology — the fixture simply had no index entry for that date, so deleting the `row.away` guard would not have failed it — fixed by adding a genuinely-aligned entry for the away date so the guard, not the data, is what the test now pins; (7, P2, test quality) `wantPhrase`'s three-item Oxford-comma branch had zero coverage across the whole diff (every fixture wanted 0, 1 or 2 tide types) — added direct unit tests in `mapTideFit.test.js`. Also fixed on the same pass: a misleading CSS comment claiming the miss tier's glyph draws a distinct arrow SHAPE via `TideWave`'s `shortfall` prop, when `TideWave` does not read that prop yet (T4's own addition) and both tiers draw an identical wave, differentiated only by ink; four `toBeTruthy()`-on-a-query-result and test-id-only interactive-control queries (the jump button) switched to `getByRole`/`toBeInTheDocument`, per `frontend-test-standards.md`; and §1 #12 updated to count `wantPhrase` as the licensed derivation class's fifth member (was undercounted at four), so T8's later "copy the four members, and only those" sweep will not silently omit it. Gate green: `npm run lint && npm test && npm audit --audit-level=high && npm run build` — 6330 tests, 0 failures, 0 vulnerabilities, build succeeded (`TideFitBlock` splits into its own ~6 KB chunk). **Browser-verified against a locally seeded briefing** (three coastal locations wanting HIGH/LOW/HIGH-or-LOW respectively, `GET /api/briefing/run` against the seeded tide extremes): **seen** — the gated-miss callout (Bamburgh Beach, a Friday sunrise with `tideAligned=false` and a served `evaluationGate`) showed the tide-fit block and the gate row together, screenshotted, with the offset clause (`HW 07:30 · 46m after sunrise`) present once, confirmed via `get_page_text`; clicking the jump moved the window control to "Tomorrow · 19:16" and re-anchored the callout, now showing the MATCH tier, screenshotted; the location sheet (`Open in Plan`) rendered a tide-fit block on both the miss+gate row and the adjacent match row (two blocks, two solar rows, confirmed via `get_page_text`), in the position between the light-times line and the gate row exactly as designed; clicking the sheet's own jump flipped the target row's `aria-expanded` to `"true"` (confirmed via a direct DOM attribute check), matching the map's jump moving to no map at all. **Not seen live**: the sheet jump's actual focus landing — the Browser pane went `document.hidden = true` mid-session (a session/tool constraint, confirmed via `document.visibilityState`), which suspends `requestAnimationFrame` in any browser, so the deferred `.focus()` call could not be observed firing; the identical behaviour is covered by `LocationFourDaySheet.test.jsx`'s test with `requestAnimationFrame` mocked to run synchronously, which passes. |
| T6 — the strip | `feature/tide-t6-strip` | `a3521fa8` (#881) | 2026-09-17 | Re-verified every symbol against the tree before editing; no drift since T3's merge. Built all eight T6 tasks: `components/map/MapTideStrip.jsx` mounted as the last child of `.wf-map-chrome-bl`, gated on `!overlayMode && !isMobile` (T7's own phone job) and on `stripModel.visible`; header/chart/footer/collapsed-row copy verbatim per the spec, `TY(l) = (26 − l·20) / 32 · 100` exact, `vector-effect="non-scaling-stroke"` on the curve; `mapTideFit.js` gains `dominantWantCount` (the footer's "N of them want" population, shared with the next-fit scan so the two can never disagree) and `siblingEventTime` (a lookup over the served EV list for the chart's sunrise/sunset clock labels, never a client-side format) — merged cleanly alongside T5's own `wantPhrase` addition to the same file, all three helpers plus `tierOf`/`nextAlignedRow`/`stripModel` surviving intact; `windowFirstRows.js` exports `STATE_WORD`/`DIRECTION_WORD` for reuse (T5 also reuses `STATE_WORD`, for `wantPhrase` — one shared doc comment now names both readers rather than two competing claims about why the export exists). `--tsh` published via the `WindowFirstHeatStrip`-style callback-ref + synchronous-measure-then-`ResizeObserver` shape, toggling `wf-tide-strip-on` and lifting only the bottom-centre counts footer (`calc(var(--tsh, 120px) + 16px)`) — the legend/upsell chips above it clear it by flex order alone. Tokens throughout (`--color-tide`, `--color-badge-tide`, `--color-verdict-marginal`, `--color-plex-text-muted`, `--color-plex-border`, `--font-mono`), no bare hex. ⚠️ **A real wiring gap between T3's plumbing and the actual payload was found and fixed, not merely a T6 addition**: `mapEvents.js#solarRow` already read `served.tide` (T3), but nothing between `GET /api/briefing`'s real `window.tide` and that `served` object ever copied it forward — `buildWindowCards`, `buildHeatStripCards` and `WindowFirstMapPane`'s own `heat.windows` mapper each build their own reduced shape of a window and none of the three carried `tide` through, so the strip was permanently invisible in production while T3's own unit tests (fed a hand-built `served` object that already had `.tide` on it) stayed green. Found only because this phase's browser verification rendered the strip against a live payload rather than stopping at the unit suite. Fixed by threading `tide: win?.tide ?? null` through all three folds, with a dedicated test at each hop — `WindowFirstMapPaneHeat.test.jsx`'s new case is named for exactly this failure mode, mirroring that file's own existing `pickKind` test, which records an identical defect once caught there before. ⚠️ **Adversarial review (6 lenses: runtime behaviour, CSS/tokens, test quality, accessibility, project conventions, impact on T7) found and fixed:** the curve stroke and sunrise/sunset rules hardcoded their hex instead of referencing `--color-tide`/`--color-verdict-marginal` (the values happened to already match, so nothing looked wrong); **the chart's `aria-hidden` sat on the `<svg>` alone**, leaving the sunrise/sunset, extrema and light-height HTML overlay labels — real text siblings of the svg — exposed to a screen reader as disconnected fragments, contradicting "the whole chart is aria-hidden" (design §2, this file's own header doc); moved onto the chart's wrapping div. The collapsed row's "Open ▴" glyph had no `aria-hidden` treatment, unlike `MapCallout`'s identical ▾/▴ toggle — matched to that precedent. The next-fit jump had no focus management: activating it commonly resolves the very fact it existed for and unmounts on the next render, dropping a keyboard/screen-reader user to `<body>` with nothing focused — fixed with a programmatic `tabIndex={-1}` on the strip's root and a focus move after the jump, and the root gained `role="group" aria-label="Tide"` so it is a landmark in the first place. Test quality: the dot-on-curve self-consistency check built its own fixture's `windowLevel` by calling the identical interpolation function the test then used to verify it (tautological — two calls to one deterministic function can never disagree); fixed by deriving `windowLevel` from an independent continuous formula, mirroring `TideSurfaceAgreementTest`'s two-independent-computations shape. An `offsetHeight` stub was restored inline at the end of each test body rather than in `afterEach`, risking a global leak across the file on any earlier failure; fixed. A `siblingEventTime` night-row guard test used a row whose own `time` was already empty, so it would have passed even with the `kind === SOLAR` filter deleted; fixed by giving that row a real time. Added the task list's own named-but-missing coverage: night-rect-width assertions, a rendered (not only pure-function) footer-text check, and the `windowLevel`-not-finite half of the dot-placement guard. Swapped one `container.querySelector('svg')` for a `data-testid`, per this project's own test standards. Declined as a plan-forced rather than accidental divergence: the root class (`wf-map-tide-strip`, needed verbatim for T7's own quoted CSS selector) and every part/the `--tsh` toggle class (`wf-tide-strip-*`, `wf-tide-strip-on` also quoted verbatim in T6's own task text) do not share one flat prefix the way `.wf-callout-tide` does — recorded in `index.css`'s own comment rather than resolved by renaming either literal the plan already fixes. Gate green: `npm run lint && npm test && npm audit --audit-level=high && npm run build` — 6355 tests, 0 vulnerabilities, build succeeded. **Browser-verified with real measurements**, not merely asserted — headless Chromium via `playwright-core` (the Browser pane would not composite in this session), against a locally seeded fixture (three SEASCAPE locations with real `tide_extreme` rows and distinct wants, reaching the null-shortfall two-value case on Tynemouth Priory). At 1280×800: strip height 167px open / 38px collapsed; `--tsh` and the counts footer's computed `bottom` agreed exactly in both states (167+16=183px; 38+16=54px, both measured, not assumed); the light dot's `left`/`top` matched `windowPosition·100`/`TY(windowLevel)` to the percent, and the night rects' `x`/`width` matched the served `sunrisePosition`/`sunsetPosition` exactly; every text node's contrast against the actual rendered (blurred, composited) background sampled from the live page ranged **7.13:1–15.42:1**, clear of the 4.5:1 floor (§7 check 7). Panned to and zoomed in on the Lake District: the strip disappeared, `--tsh` and `wf-tide-strip-on` were removed, and the counts footer's `bottom` returned to exactly `8px` (§7 check 3). **Not seen live**: a night/astro row hiding the strip — the seeded fixture's window control offered only solar rows, so that branch is proven by `mapTideFit.test.js`'s `stripModel` visibility suite and `MapTideStrip.test.jsx`'s own visibility tests rather than observed on screen. ⚠️ **Merged `origin/main` (T5, #879) after this phase's own commit** — three conflicts (`windowFirstRows.js`'s `STATE_WORD` doc comment, `mapTideFit.test.js`'s import list and appended test blocks, this table) resolved by keeping both phases' substance in each; `mapTideFit.js` itself merged with no conflict, all five exports (`tierOf`, `wantPhrase`, `nextAlignedRow`, `stripModel`, `siblingEventTime`) intact. Full gate re-run green post-merge. |
| T7 — phone | `feature/tide-t7-phone` | `de396bd2` (#882) | 2026-09-17 | Re-verified every symbol against the tree before editing — the "five-row lifted stack" comment and its five row anchors (bar 8/48, attribution 76/27, footer 112/28, scored-legend 152/26, chrome-bl 192/28) all matched the tree exactly, no drift since T6 merged. Lifted the `!isMobile` gate on `<MapTideStrip>` in `MapView.jsx`, but **not** by simply removing it: the phone mount is a **second, independent render site**, gated `isMobile &&`, deliberately never nested inside `.wf-map-chrome-bl` the way the desktop mount is — `.wf-map-chrome-bl` is only ever left-anchored (`left: 8px`, no `right`), so a descendant of it could never stretch `left: 8px; right: 8px` edge-to-edge against the frame the way the plan's own literal T7 task-1 CSS quote demands. The phone strip mounts instead as a plain sibling of `.wf-map-counts-footer`/`.wf-map-scored-legend`, inside the same `position: relative` wrapper those already use as their containing block, so its own `position: absolute; left: 8px; right: 8px; bottom: 112px` (`index.css`, phone query) resolves against the frame exactly like theirs — confirmed both by a passing arithmetic test and by live measurement (below). It takes the count footer's own row (`bottom: 112px`); the footer is hidden outright (`display: none`) rather than merely lifted, per design §6; the scored-legend chip and `.wf-map-chrome-bl` (now holding only the LITE viewline-upsell chip on phone) are re-anchored off the strip's *real* published height (`--tsh`, T6) via `calc(112px + var(--tsh, 120px) + Npx)` chains, replacing their old fixed 152px/192px anchors *while the strip is on* — the exact 26px/14px figures the pre-existing chain already used are kept, so nothing about the phone stack's spacing changes on a viewport with no strip to displace the footer with (an inland pan, an astro/aurora window). Two knock-on fixes, both because the phone strip is no longer covered transitively via `.wf-map-chrome-bl`'s own existing entries: `MapLabels.jsx`/`PinsLayer.jsx`'s `OBSTACLE_SELECTOR` and `MapCallout.jsx`'s `BAND_BAR_SELECTOR` each gained `[data-testid="wf-tide-strip"]`, with a doc comment on the latter arguing no `always: true` width-test opt-out is needed (unlike the footer's own existing one) because the phone strip's real `left: 8px; right: 8px` shape comfortably clears `calloutBand`'s ≥50%-frame-width floor/ceiling test. `frontend/src/test/mapPhoneChromeCascade.test.jsx` gains the strip's own static-geometry assertions (desktop-untouched proof included), the footer-hidden-only-while-on positive AND negative cases, and — since `calc(var(--tsh))` is exactly what this suite's jsdom cannot resolve numerically (the same constraint T6's own `--tsh` test in `mapChromeZLadderCascade.test.jsx` already documents) — a raw-CSS-text regex extraction of the scored-legend/chrome-bl lift formulas, replayed as arithmetic in JS with T6's own measured heights (167px open, 38px collapsed) standing in for `--tsh`, proving every pairwise boundary clears by ≥ 8px in both states. `MapCallout.test.jsx` gains a new describe block proving `frame.band` (exposed via the rendered `card.style.maxHeight`, the one value that surfaces it without needing `withMeasuredCard`) actually clamps above a real `wf-tide-strip` DOM sibling, and is a no-op without one. ⚠️ **Adversarial review (5 lenses: runtime behaviour, CSS/tokens, test quality, accessibility, project conventions) found and fixed three real issues, confirmed the rest sound:** (1, CSS) the pre-existing unconditional `.wf-map-tab.wf-tide-strip-on .wf-map-counts-footer { bottom: calc(...) }` rule (outside any media query, predating this phase) still fires on the phone, where the new `display: none` for the same selector already wins — harmless (different properties, no visual conflict) but dead arithmetic on that one viewport; fixed with a comment acknowledging the overlap rather than a second media-query split, since narrowing it risked nothing the comment doesn't already say plainly. (2, test quality) `mapPhoneChromeCascade.test.jsx`'s new pairwise-clearance `it.each` hardcoded the bar's `bottom` as a bare `8` literal, unlike every other check in the same file, which reads it live off the real CSS; fixed to extract it via `computedStyleFor` like its neighbours. (3, test quality, confirmed real) no test anywhere exercised `MapCallout.jsx`'s `BAND_BAR_SELECTOR` extension at all — not even for the three *pre-existing* entries, a gap this phase's own addition inherited rather than introduced; closed with the two new `MapCallout.test.jsx` cases described above (both pass on first run, confirming the arithmetic hand-derived for them was correct). **Confirmed sound, not changed:** the strip's own containing-block resolution (traced by hand against the JSX, not merely asserted); the `--tsh` formula's self-consistency and its non-regression of the strip-off phone stack; CSS specificity of every new `.wf-tide-strip-on`-scoped override against the pre-existing unconditional phone rules for the same selectors; media-query nesting (the whole T7 block is genuinely inside `@media (max-width: 639px)`, brace-traced); every new/modified comment checked against the code it describes, no overclaims found. One accessibility finding recorded as an **owner item rather than fixed**: hiding the counts footer outright (not merely lifting it) removes its count/rated/filtered figures from the accessibility tree for as long as the strip is showing, with no `aria-live`/status substitute anywhere else on the tab — faithful to design §6's own reasoning (screen-space contention), which was never evaluated for a non-visual reader who has none; recorded as §6 Q8 rather than overridden, since fixing it would mean changing the design's stated behaviour rather than reporting on it. Gate green: `npm run lint` (0 warnings), `npm test` (251 files, 6413 tests, 0 failures — the full suite, re-run after every review fix), `npm audit --audit-level=high` (0 vulnerabilities), `npm run build` (succeeded). **Browser-verified at 390×844**, headless Chromium via `playwright-core` (`chromium-1208`'s own binary, `executablePath` override — the worktree's `playwright-core` resolved a revision the local cache did not have), against a freshly seeded local backend (port 8097) and Vite dev server (port 5183): the same three coastal locations/wants as T5/T6 (Bamburgh Beach LOW, Dunstanburgh Castle HIGH, Tynemouth Priory HIGH+LOW), `POST /api/briefing/run`, then real DOM measurement via `boundingBox()`/`getComputedStyle` — no numbers assumed. **The four requested measurements, both states:**

  | check | open | collapsed |
  |---|---|---|
  | strip bottom edge vs bar top edge | 732.0 vs 779.5 → **47.5px clear** | 732.0 vs 779.5 → **47.5px clear** (strip `bottom` is a fixed anchor, so this is identical in both states by construction) |
  | scored-legend bottom edge vs strip top edge | 553.0 vs 565.47 → **12.47px clear** | 656.0 vs 668.5 → **12.5px clear** |
  | callout card bottom edge vs strip top edge | *(measured in the collapsed/Pins-mode state below)* | 660.5 vs 668.5 → **8.0px clear** — exactly `calloutBand`'s own `BAND_EDGE_PAD` |
  | counts footer `display` while the strip is on | `none` | `none` (also `none` in the callout-open state) |

  Strip's own real height: **166.53px open, 63.5px collapsed** — confirming the runtime-behaviour review's own flagged risk was real: the collapsed row's real phone-width height (63.5px) is **not** T6's desktop 38px, diverging by more than the unit test's stubbed value ever claimed to predict (167px open matches T6's desktop figure closely; 38px does not survive to 374px-wide phone content). The geometry stayed correct regardless — proof by measurement that anchoring off `--tsh` rather than an assumed constant is exactly what protects this case, the reason the mechanism exists in the first place; the *test* stub (167/38) is retained as documented arithmetic coverage of the FORMULA's shape, not as a claim that a phone renders at those exact heights. Also seen: the callout (`Bamburgh Beach`, Pins mode) rendered fully above the collapsed strip with no visual overlap, screenshotted (`/tmp/t7_map_callout.png` — not committed). The strip itself rendered its full content correctly at this width: header, HIGH/MID/LOW bands, chart with night shading and the light dot, and the footer sentence "2 of 3 coastal spots are dimmed — they want low water" with the "beyond these four days" denial (no later-window fit existed in the 4-day seeded horizon). **Not separately re-verified**: §7 check 3 (strip absence on an inland pan / astro row) and check 7 (contrast) — both already browser-verified at desktop width in T6's own phase-log entry, and this phase changes neither the strip's rendering nor its palette, only its phone mount point and the chrome around it.

⚠️ **Follow-up (Codex P1 on the T7 PR, addressed same-branch): `MapCallout`'s placement band went stale when the strip resized underneath it.** This phase's own `BAND_BAR_SELECTOR` addition (above) makes `MapCallout.jsx`'s `paint()` treat the tide strip as one of its floor/ceiling bars, but `paint()` only ever runs on a fixed list of triggers (a map event, or one of `MapCallout`'s own repaint-dependency-list entries — `stripOpen`, `event?.id`, the rating fields, `evaluationGateIndex`, `tideAlignmentIndex`) and none of them fire when the STRIP's own rect changes. So a reader who selects a location while the strip is collapsed, then taps `Open ▴`, kept the collapsed band boundary while the strip's real rect grew ~100px upward underneath the card (measured: 63.5px → 166.5px, this phase's own two numbers), until an unrelated pan/zoom forced a re-measure. Fixed by lifting the strip's real height out of `MapTideStrip`'s own `ResizeObserver` callback (the same one that already writes `--tsh`) into a new optional `onHeightChange` prop, threaded through a new `MapView` state (`tideStripHeight`, fed by BOTH mount sites — desktop/tablet and phone, T7 #16, since only one is ever mounted at a time) into a new `MapCallout` prop of the same name, joining the existing repaint-dependency list purely as a trigger — `paint()` itself still re-measures the strip's real rect fresh off the DOM every time it runs, exactly like every other bar; `tideStripHeight` is never read as a number, only its identity changing matters. `null` on unmount (the strip appearing/disappearing) retriggers the same way a resize does. Three new test layers, each pinning one link in the chain rather than one broad integration test: `MapTideStrip.test.jsx` (`onHeightChange` called with the right number on mount/collapse/expand, `null` on unmount, tolerates no callback at all); `MapCallout.test.jsx` (a changing `tideStripHeight` prop — including appearing from `null` — retriggers `paint()`, counted via `latLngToContainerPoint` calls, the file's own established technique); and a new `MapViewTideStripCalloutWiring.test.jsx`, modelled directly on `MapViewSelectionOrdering.test.jsx`'s own probe-mock scaffold, proving `MapView` actually connects the two on BOTH viewports rather than merely being plausible from reading the three files in isolation (one shared `<MapTideStrip>` probe on screen at a time; its `onHeightChange` call reaches the SAME state `MapCallout`'s probe reads, on either `isMobile` value). Gate re-run green after the fix: `npm run lint` (0 warnings), `npm test` (252 files, 6424 tests, 0 failures — 11 new: 2 + 5 + 4), `npm audit --audit-level=high` (0 vulnerabilities), `npm run build` (succeeded). Not re-verified in the browser — the fix is unit/wiring-tested only; a live repro of "select while collapsed, then open, watch the card clamp correctly" was not re-run against a seeded backend for this follow-up.

| T8 — sweep, docs and the OPEN answers | `docs/tide-t8-sweep` | pending PR | 2026-09-18 | Docs-only, no code change. Re-read every phase-log row (including T4's and T7's own follow-up paragraphs) against `origin/main`'s tail and spot-checked the riskiest §4 claims directly against the shipped source — `TideRepresentativeSelector`, `BriefingGatingPolicy.HARD_CONSTRAINT_REASONS`, the `--color-badge-tide`/`--color-tide` token values, the strip's mount position as the last child of `.wf-map-chrome-bl` on desktop (the phone mount is a sibling of that chrome — T7), `tideStripCollapsed`'s `MapView` state, the absence of a `'near'` tier anywhere in shipped code, and `visibleLocations`' `if (getTideOnLightForLocation(loc)) return true` bypass (the function is misleadingly named from bundle rev 2 but its T3/T4 body reads the full tide-alignment-index entry, i.e. the preference fact, not the on-the-light boolean alone) — **found §4's 18 entries already accurate and consistent with what shipped; no renumbering, removal or correction needed.** §1 #12's own "Five members, and only those" underclaimed by one: T6 added `siblingEventTime` (a served-clock-time lookup for the strip chart's sunrise/sunset labels) without bumping the count — corrected to six (detail below). Verified every `§N #M` cross-reference resolves against the (unchanged) numbering, both inside this file (sampled ~20 across §1/§4/§5/§6) and in every other file that cites it (`grep -rln "tide-window-plan"`): `docs/design/tide-window/VENDORING.md` and `docs/engineering/tide-window-prompts.md` (this series' own kickoff-prompts file; noted separately that its own T8 prompt still says "the four members" §1 #12 names, stale since T5 bumped that to five and this sweep to six — not followed, since this sweep names six directly from the plan and the shipped code, but left uncorrected there as a kickoff script rather than a load-bearing doc) — plus, for completeness, the unrelated sibling `docs/design/tide-plan/`/`docs/engineering/tide-plan-card-plan.md` bundle (the Plan-tab tide card, a different increment), whose few citations of THIS plan also resolve correctly. §0 flipped to complete with every phase-log commit column filled from `git log` (T2/T4/T5/T6/T7's real merge-commit hashes, T1's and T3's squash-commit hashes, T3's pre-merge Codex fix `1507aa3c` named separately since the squash hides it from `main`'s own history). CLAUDE.md gained: the Map tab (v2) bullet's strip/dimming/one-question sentences; the Locations bullet's note that the map reads `TideType`; a new seventh Backend-heavy licensed-derivation class (six members: `stripModel`'s viewport filter, `dominantWantCount`, `nextAlignedRow`, `tierOf`, `wantPhrase`, `siblingEventTime`) — "Members: those six, nothing else", the existing convention; and one sentence on the Two-tide-axes bullet naming which axis the map's chip now reads. §6 gained an "OPENs, answered" preamble mapping the design's five `OPEN`s onto what shipped, plus **Q9** (T5's adversarial-review finding #2, accepted as a narrow risk rather than fixed: `TideFitBlock`'s want/denial text reads the live `location.tideType` roster while the served `fitPhrase`'s own "wants" clause is frozen inside the cached forecast row, so an admin edit to a coastal location's want between pipeline cycles could make the two disagree on one card — closing it needs a backend field recording the want the phrase was built against, outside any phase's scope so far). §7 gained a `§7b` table transcribing the phase log's own measured numbers (T4's `.72` contrast computation, T6's 1280×800 strip heights and contrast range, T7's 390×844 heights and clearances) rather than re-deriving them. `docs/design/tide-window/VENDORING.md` gained a line naming §4's closed count (18). ⚠️ **Adversarial review (4 read-only lenses: over-claiming against shipped code with file:line citations, internal consistency of numbering/cross-references, CLAUDE.md convention compliance, completeness against §3 T8's five items) found and fixed four real things, confirmed one charge unfounded**: (1, over-claiming) the pre-existing T2 phase-log row cited a stale pre-merge local commit hash, `ddbfe1ff`, never reachable from `origin/main` (confirmed via a merge-base ancestry check) — replaced with the real squash-merge commit, `3bed6281` (#876); a defect this sweep's own job description exists to catch, not introduced by it. (2, CLAUDE.md convention) the new seventh-class paragraph named six members but, unlike every one of its five precedents, stated no exit — added one: unlike classes four through six (per-user/per-scope questions with no servable answer today), this class answers no such question, so it has none to retire. (3, CLAUDE.md convention) the Two-tide-axes bullet's new sentence fully re-derived the same tideAligned/tideOnTheLight pair the Map tab (v2) bullet's own ⚠️ states two bullets earlier — compressed to a cross-reference, matching the document's own house style elsewhere. (4, CLAUDE.md convention, minor) the Locations bullet's TideType aside said the chip reads TideType "directly", overstating the mechanism (the chip actually keys on the derived `tideAligned`) and breaking the list's terse rhythm — reworded and shortened. **Confirmed unfounded**: a charge that `tideTierHeading`/`tideAccessibleClause` should count as a seventh/eighth licensed-derivation member — both return a FIXED string per tier with no data read, licensed instead under §5 #5's heading-concatenation rule, and both docs now say so explicitly rather than leaving the boundary implicit. One further, self-referential finding from the internal-consistency lens: this row's own first draft named the wrong pair of files (`docs/design/tide-plan/`, an unrelated sibling increment) as what T8 verified cross-references against — corrected to name the two files that actually cite this plan (`docs/design/tide-window/VENDORING.md`, `docs/engineering/tide-window-prompts.md`). Gate: none to run (no `.java`/`.jsx` touched); a diff-stat check confirms only Markdown files changed. |
...
locations, a forward scan over served per-window `tideAligned` for the next fit, `tier = aligned ?
'match' : 'miss'` — a served boolean read — and (T5) `mapTideFit.wantPhrase`, joining the served,
static `location.tideType` set into words ("high water or low water") for the block's jump/denial
line, mirroring `windowFirstRows.js`'s own `STATE_WORD` lexical mapping rather than deciding anything
new; and (T6) `mapTideFit.siblingEventTime`, a lookup over the already-built EV list for the strip
chart's sunrise/sunset clock labels — reading the served, already-formatted `time` off the sibling
solar row, never formatting one itself. **Six members, and only those** (T8 sweep: corrected from
"five" — T6 added `siblingEventTime` without bumping this count when it landed; `tideTierHeading`/
`tideAccessibleClause`, also in this file, are NOT members — they return a fixed string per tier
rather than deriving anything from served data, so they are licensed under §5 #5's heading-
concatenation rule instead). The chart's path
from the served `curve` and the `TY` mapping are rendering, like `tideSparkline`. **Nothing on the
client reads a height, a minute offset or a
...
15. **The sheet's denial states its own numeral horizon, not the design's literal "four days"** —
    qualifying #8's "taken verbatim" claim for this one line. `LocationFourDaySheet` already has a
    horizon word (`leadLine`'s "The next N days here"), computed from `sheet.rows`' own distinct
    dates; T5 reuses it (`"3 days"`/`"4 days"`) rather than mint a second "four days" literal that
    could drift out of sync with the sheet's own kicker if its horizon ever moves off four. The
    callout has no such computed word of its own, so `TideFitBlock`'s default parameter carries the
    design's literal copy unchanged — reached only there. `TideFitBlock`'s actual props are
    `{fact, want, nextFitRow, onSelectEv, horizonWord}`, not the plan's illustrative
    `{fact, evRow, evRows, onSelectEv, horizonWord}`: each CALLER (`MapCallout`,
    `LocationFourDaySheet`) resolves the next-fit row once via T3's tested `nextAlignedRow` (any-want
    reading, §1 #12) and hands the component the resolved row object plus the location's own wanted
    set, rather than have a "pure render" component reach into a briefing-shaped index itself —
    matching T3's own precedent for `stripModel`'s extra params.
16. **T7's phone strip is a SECOND, independent mount, never the `!isMobile` gate simply dropped** —
    qualifying task 1's literal CSS quote (`.wf-map-tab .wf-map-tide-strip { left: 8px; right: 8px;
    width: auto; max-width: none }`), which was written as though the existing desktop mount (a
    descendant of `.wf-map-chrome-bl`, T6) could just widen in place. It cannot: `.wf-map-chrome-bl`
    is only ever left-anchored (`left: 8px`, no `right`), so nothing inside it can ever resolve
    `right: 8px` against the FRAME — only against that narrower box. `MapView.jsx` therefore renders
    `<MapTideStrip>` a second time, gated `isMobile &&`, as a plain sibling of `.wf-map-counts-footer`/
    `.wf-map-scored-legend` instead, inside the same `position: relative` wrapper those already use —
    the identical containing block the plan's own literal CSS quote implicitly assumed. The component
    itself needed no change; `MapTideStrip.jsx` already returns `null` unless `model.visible`, on
    either mount, and both call sites pass byte-identical props.
17. **Everything above the strip clears its REAL height, chained off the same 26px/14px figures the
    pre-existing (strip-off) phone cascade already used — not a re-tuned constant.** `.wf-map-scored-
    legend`/`.wf-map-chrome-bl`'s old phone anchors (152px/192px) were themselves `footer_bottom(112)
    + footer_height(28, assumed) + clearance` chains; T7's `.wf-tide-strip-on`-scoped overrides keep
    the identical clearances (12px, then 26px, then 14px) and substitute the strip's real `--tsh` for
    the footer's assumed 28px — so the phone stack's spacing is unchanged whenever there is no strip
    to displace the footer with, and self-corrects (proven live, not merely asserted — see the phase
    log) when the strip's real height turns out to differ from either of T6's own desktop-measured
    stubs (167px/38px): measured at 390×844 the OPEN height (166.53px) matches T6's figure closely,
    but the COLLAPSED height (63.5px) does not — the collapsed row's content plausibly reflows at
    374px that it did not at 1280px. The chained clearances held regardless, which is the point of
    keying off `--tsh` rather than a constant at all.
18. **`MapCallout.jsx`'s `BAND_BAR_SELECTOR` and `MapLabels.jsx`/`PinsLayer.jsx`'s
    `OBSTACLE_SELECTOR` each gained `[data-testid="wf-tide-strip"]`** — not asked for in T7's own
    three numbered tasks, but a direct, mechanical consequence of #16: on desktop the strip is
    covered transitively as a descendant of `.wf-map-chrome-bl`, already seeded in both lists (§1
    #8); on the phone, as a sibling, it is not covered by that entry at all, so without its own
    selector a label, pin or the anchored callout card could be placed under an uncovered strip on
    that one viewport. No `always: true` width-test opt-out was needed for `BAND_BAR_SELECTOR` the
    way the counts footer got one (`COUNTS_FOOTER_SELECTOR`) — the phone strip's real `left: 8px;
...
**The design's own five `OPEN`s (`docs/design/tide-window/README.md` §9), answered by what
shipped** (T8 sweep):

- **`OPEN 1`** (the one new field) — no new field, column or API field, and no new editor: the
  location's want is already `Set<TideType>` (`entity/LocationEntity.java`), edited by the existing
  `TideToggleChips` (§1 #1, §4 #1). The spec's "null means no preference vs unknown" question
  dissolves with it — there is no "coastal with no preference" state to decide.
- **`OPEN 2`** (the band thresholds) — moot: this codebase's tiers are read off the served
  time-based `tideAligned`, never off a level or the spec's 0.75/0.25 bands (§1 #2, §4 #2). Q3
  below names the one knob that survived — `TideService.HIGH_LOW_THRESHOLD_MINUTES` and the
  dynamic tight window.
- **`OPEN 3`** (one curve for many locations) — answered by the codebase, not decided fresh:
  `TideRepresentativeSelector.select` (anchor-or-biggest-range, chosen once per briefing) supplies
  the one curve, and the strip names that station in its header's `measured at <locationName>`
  clause (§1 #4, §4 #4) — the spec's own requirement that "the strip should say which."
- **`OPEN 4`** (next fit beyond the forecast) — still open. Answering it needs a tide-plus-ephemeris
  query with no forecast horizon at all; Q4 below is the same item, unchanged by this sweep.
- **`OPEN 5`** (dimming and the score — is tide represented twice?) — the pipeline already did
  both before this series existed: `TideVisitor` penalises a tide mismatch inside the star
  (§1 #3), and `BriefingGatingPolicy.HARD_CONSTRAINT_REASONS` separately withholds a hard-mismatched
  coastal slot from Claude entirely (§1 #2–#3). This series draws what the pipeline already knows
  and changes neither (§5 #1) — a gated slot is dimmed with no star to double-penalise, and a
  weather-stood-down-then-rated miss shows its own real, unmodified star dimmed (§5 #2). Q1 below
...
- **Q9 — the jump/denial text can disagree with the served `fitPhrase` on the same card.**
  ...
  wanted set as it stands right now), while `fact.fitPhrase`'s own "wants…" clause is **frozen**
  inside the cached forecast row the pipeline built it from. An admin who edits a coastal location's
  tide preference between pipeline cycles can make the two clauses on one card name different water
  — the block's body saying "wants low water" while its jump line, built from the roster as it now
  stands, offers "Next high water on the light". Recorded as an accepted, narrow risk in
  `TideFitBlock.jsx`'s own doc (adversarial review, T5, finding 2 — P1, runtime) rather than fixed:
  closing it needs a backend field recording the want the phrase was actually built against, which
  is outside a frontend-only phase's scope. Whoever picks this up: serve the want alongside
  `fitPhrase` (or fold it into the phrase itself) so both clauses on the card read the same frozen
  fact. Not this series.
...
### §7b Measurements taken (T8 sweep — transcribed from the phase log, not re-derived)

The numbers below are the real, live-measured figures each phase's own row already reports; this
table exists so a reader does not have to find them inside the prose. Where a phase measured a
check analytically rather than by live sample (T4's contrast), that is stated, not hidden as a
photograph.

| # | check | phase | measured |
|---|---|---|---|
| 1 | Tide never moves the heat | T4 | Pinned as a unit test (plan §7 #1 is owned by T4 as a test — field alpha at a coastal core identical across the three wants); **no live measurement was taken**, and the row below is a different observation, not evidence for this one. |
| — (unnumbered; not a §7 check) | Chip miss opacity — the design's `.72` kept over the `.wf-loc-row` precedent's `.8` | T4 | `.72` shipped. Not a live pixel sample — the Esri basemap tile is cross-origin, so a canvas read throws a tainted-canvas `SecurityError`. Computed from the real CSS values (`rgba(14,11,9,.84)` plate, `#F2E7D3` text, WCAG relative-luminance formula) composited over two representative basemaps: **≈8.5:1** against this app's own darkest sampled tile (`rgb(20,15,12)`) and **≈6.2:1** against an eyeballed light-land tone (`rgb(170,165,150)`) — both clear AA (4.5:1). `.8` computes higher but bought nothing `.72` did not already clear, because the chip's own near-opaque plate (unlike `.wf-loc-row`'s translucent card) absorbs almost all of the outer opacity's effect on local contrast. |
| 3, 4, 7 (desktop) | Strip geometry and contrast at 1280×800 | T6 | Strip height **167px open / 38px collapsed**; `--tsh` and the counts footer's computed `bottom` agreed exactly in both states (167+16=183px; 38+16=54px, both measured). Every text node's contrast against the actual rendered, composited background sampled live: **7.13:1–15.42:1**, clear of the 4.5:1 floor. Panned inland: strip absent, `--tsh`/`wf-tide-strip-on` removed, counts footer `bottom` returned to exactly `8px`. |
| 8 | Phone geometry at 390×844 | T7 | Strip height **166.53px open / 63.5px collapsed** (the collapsed figure diverges from T6's desktop 38px stub — real phone-width content reflows; the geometry stayed correct regardless, which is the point of keying off `--tsh` rather than a constant). Clearances, both open and collapsed unless noted: strip bottom edge vs bar top edge — **47.5px** (fixed anchor, identical in both states by construction); scored-legend bottom edge vs strip top edge — **12.47px** open, **12.5px** collapsed; callout card bottom edge vs strip top edge (collapsed/Pins-mode state) — **8.0px**, exactly `calloutBand`'s own `BAND_EDGE_PAD`; counts footer `display` while the strip is on — `none` in every state measured, including the callout-open state. |
