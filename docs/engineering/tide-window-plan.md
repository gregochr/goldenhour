...
**Status: T1–T7 BUILT (T4 merged to main as #880, T5 as #879, T6 as #881); T7 pending PR; T8 not
started.** T1's `TideCurveCalculator` lift and T2's window
...
new. **Five members, and only those.** The chart's path from the served `curve` and the `TY` mapping
are rendering, like `tideSparkline`. **Nothing on the client reads a height, a minute offset or a
