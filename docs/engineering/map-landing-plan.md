...
**Status: IN PROGRESS — L1–L4 built (unpushed), L5 next. All three blocking owner decisions taken
2026-09-05 (§6): Q1 night cell renders empty, Q2 once per forecast run, Q3 name the highest-mean
region.** Phase log (every phase appends its own row in the same commit as its code):
...
2. Update `CLAUDE.md`'s **Map tab (v2)** bullet and its **Backend-heavy** bullet — the latter gains
   ...
   (§5 D-4, §6 Q4), in the same voice as the existing five classes. ⚠️ **There are now THREE, not
   one, and an earlier wording of this step named only the first** — which would have swept straight
   past the other two, exactly the "Members: those N, nothing else" closure the bullet relies on:
   ...
   All three are per-user because the scope is the reader's own planning area; none re-derives a
