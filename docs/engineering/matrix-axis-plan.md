...
| D8 | Rail CSS | Per the bundle, with app tokens: `.wf-rail { position: sticky; top: calc(var(--wf-mast-h, 128px) + var(--wf-lens-h, 54px) + var(--wf-dh-h, 49px) - 2px); z-index: 14; display: flex; align-items: center; gap: 9px; padding: 4px 2px 3px; background: linear-gradient(180deg, var(--color-plex-bg) 0 70%, transparent); font-family: var(--font-mono); font-size: 9.5px; font-weight: 600; letter-spacing: 0.16em; text-transform: uppercase; line-height: 1; }`; `.wf-rail.am { color: var(--color-plex-dawn); }`; `.wf-rail.am .wf-rail-rule { background: linear-gradient(90deg, color-mix(in srgb, var(--color-plex-dawn) 42%, transparent), transparent); }`; `.wf-rail.pm { color: var(--color-plex-coral); }` + same rule pattern with `--color-plex-coral`; `.wf-rail-rule { flex: 1; height: 1px; }`. | Bundle values; token names per D14. |
...
| A-10 | Sticky-calc fallback literals `48px`/`44px`/`40px` | `128px`/`54px`/`49px` | The README's own rule — offsets are "measured, not hard-coded"; these are the app's measured chrome heights. |
...
4. Tests:
   ...
   - **`--wf-dh-h`**: with the running-RO pattern from `WindowFirstShellSticky.test.jsx`
     ...
     assert the section's style is **`'49px'`** — never merely "a value was written", which
   ...
   - **Stylesheet half** (same file), each block sliced: `.wf-dhrow` contains `position: sticky`,
     ...
     **`position: sticky`** (omit it and `top`/`z-index` are inert — every other assertion stays
     ...
     string `scroll-margin-top: calc(var(--wf-lens-reserve, 188px) + var(--wf-dh-h, 49px) + 17px)`
...
6. **Browser verification — Playwright headless Chromium** (`chromium.launch()` from `frontend/`,
   ...
   - Computed `top` of `.wf-dhrow` equals mast + lens − 1 as measured; **replace the 49px
     `--wf-dh-h` fallback literal with the measured resting tile-row height** (and the test's
     string with it) if it differs.
...
> will be tempted to redesign; don't. The two browser-measured literals (54px lens, 49px tiles)
> are provisional until §6 step 6 measures them — updating a literal *and its test string
...
**Not examined by the review, stated per its own rule:** pixel truth of the 54px/49px literals
(deferred to §6.6 by design); the two shell-tab/sheet test files were grep-checked as likely
