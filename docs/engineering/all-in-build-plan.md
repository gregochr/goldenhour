...
the **resolved positions**, re-verified against the tree at **`d421ef5f`** (`main`). Where the two
...
`StabilitySnapshotProvider` has nine production call sites across eight classes, two of them
display/publish (`BriefingRollupBuilder.java:403`, `BriefingBestBetAdvisor.java:150`). It is the
**only** transport from classifier to Plan tab. Per-region stability moves onto the **slate**, and
those two read it from there. That work lands in phase 1 and **gates** every deletion of the
provider. See the design doc's §4.2 correction.
...
`BriefingService.java:508` computes `aboveThreshold`; the branch at `:537`/`:550-561` shows that
**below threshold the briefing still publishes** — only `persistBriefing()` is skipped and a
last-known-good stale copy is served. One pass promoted this into a `failRun()` before submit,
...
it being 4. `BriefingService.java:118` reads `= 5`; `:410` builds `IntStream.range(0, …)`, i.e.
T+0..T+4. **T+4 is inside the window, so the T+3 ceiling is policy, not structure** — it is the
...
The correction was that there is no H2 at *runtime*. `src/test/resources/application.yml:1-13` is a
separate thing: `jdbc:h2:mem:testdb`, `ddl-auto: create-drop`, `flyway.enabled: false`. Deleting it
...
2. **`ddl-auto: validate` vs the view entities.** All three runtime profiles validate. Hibernate's
   visibility of a Flyway-created view on Postgres 17 must be proven before V139 ships, and
...
- **The visibility gate is inverted against the prompt — removing it should *improve* output.**
  Triage discards anything below 5,000 m (`WeatherTriageEvaluator.java:28`). But the mist guidance
  at `PromptBuilder.java:163-169` reads: **score UP** for *"thin ground mist (visibility 2-8 km)
  with clear sky above … Mist in valleys from an elevated viewpoint — potential cloud inversion"*;
  **score DOWN** only for *"dense fog (visibility < 1 km) with no trend toward clearing"*. So the
  band **2,000–5,000 m is simultaneously "triage: discard" and "prompt: score UP"** — the gate has
  been binning precisely the mornings the entire `InversionScoreCalculator` subsystem exists to
  find, and because triage persists a canned rating of 1, those valley-inversion dawns are recorded
  as the worst possible score. This is a stronger argument for removal than cost: the gate is not
  redundant, it is deleting good mornings. It also means the post-removal rating distribution will
  shift **up** at some locations, not only down — factor that into the confidence re-check above.
- **The triage *display* path is not the triage gate.** `EvaluationViewService.java:458` derives
  ...
  fallback colouring unscored Plan cells; `ForecastDtoMapper.java:240,612` render it into the API
  payload. Delete it and a heavy-cloud slot renders a Claude rating instead of "STANDDOWN — sun
  blocked", and the fallback ladder loses a rung. Keep the display, delete the gate.
...
**And test churn is not deletion.** `BriefingServiceTest` (2,446, split three ways),
