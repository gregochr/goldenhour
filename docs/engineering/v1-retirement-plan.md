...
**Status: COMPLETE.** PLAN WRITTEN 2026-08-23 and adversarially reviewed 2026-08-23/24 (six
prosecutor lenses, 41 charges, one refuter per charge — §10); the survivors are folded in below.
D0 landed 2026-08-24. D1 landed 2026-08-25. D2 landed 2026-08-25 (#618). D3 landed 2026-08-25 (#623).
**D4, the settling commit, built 2026-08-25 — the series' last phase, pending merge (a phase
cannot name its own hash; the D4 Phase-log row's commit cell reads *(fill after merge)* until it
does).**
...
| D0 | `claude/v1-retirement-d0-flip-d2ca06` | `e4cb446a` | 2026-08-24 | The flip, alone: one `DEFAULT = PLAN_V2` constant used at all three `usePlanLayout` fallback sites (storage default, invalid-read fallback, invalid-write fallback), both Javadoc paragraphs rewritten. Rollback hatch (stored `"v1"` still renders v1) deliberately untouched — lasts until D1 deletes the toggle. Three read-only lenses (runtime, tests, scope-fidelity-to-§3.0) each returned zero charges — reported as unexamined coverage per §0's rule, not as clean. Branch carries a worktree-assigned `-d2ca06` suffix rather than the plan's literal `claude/v1-retirement-d0-flip`; flagged for the owner, not renamed mid-session. |
| D1 | `claude/d1-flag-machinery-toggle-88a80c` | `410020a8` | 2026-08-25 | The flag machinery, the toggle, the boundary — §3.1 executed exactly: `usePlanLayout.js` deleted (sole importer was `App.jsx`); all 13 flag sites in `App.jsx` resolved per the table, including collapsing the `planLayout === PLAN_V2 ? … : …` ternary to the v2 content alone (the v1 `<main>`, the suppressed `<header>`, `ViewToggle`/`DateStrip`'s App-level mounts, `viewMode` + its `hashchange` listener, `mapHandoff`, all gone); `WindowFirstShell`'s `onExit` prop and `wf-exit-foot` exit button deleted (`index.css`'s rule with it); `UserSettingsModal`'s "Plan Layout" switch deleted (`.quality-toggle-track/-thumb` correctly left — `HeatmapGrid` still uses it); `NlcSightingBanner` ships permanently inert (`interactive` prop gone); `useTodaysLight` loses its `enabled` gate. `usePlanLayout.test.jsx` renamed (not deleted) to `WindowFirstShell.test.jsx` — only the hook describe and the two exit tests went, the shell's ~1,250-line suite survives. §4.1's boundary redesign: `PlanLayoutErrorBoundary` → `PlanErrorBoundary`, still wrapping the provider + shell inside `<main>` (a provider crash is caught the same way a shell crash is — pinned by `App.test.jsx`'s new provider-throw test, traced and confirmed non-vacuous: the throw fires on `App`'s post-mount re-render once `getSettings`/`fetchForecasts` resolve, not on the synchronous initial mount), fallback now "Clear cached data and reload" (via an injectable `reload` prop, sidestepping jsdom 30's `location.reload` stub problem entirely rather than working around it) + "Sign out", `recoverLabel`/`onRecover` gone. Gate: lint, 180/180 test files (4536/4536 tests), audit, build all green; entry chunk shrank 340.52 kB → 207.09 kB (DailyBriefing's whole subtree fell out of the graph early, since D2 hasn't deleted the file yet but D1 already deleted its only import); the ASI-class semicolon check's one known hit (`CloseToHome.test.jsx`) is unchanged, exactly as §6 predicted for a phase before D2. Five read-only lenses (runtime, tests, a11y, scope-vs-§3.1, comments/copy) — runtime, tests, a11y and scope all returned zero findings; comments/copy found four: a CHANGELOG miscount (said "two exports", `usePlanLayout` had three — fixed), an overclaim in `WindowFirstShell`'s new `onSignOut` doc ("the app's only Sign-out button" — false once `PlanErrorBoundary`'s own crash-fallback Sign-out exists; softened to "while the Plan is healthy" in both the component and its test), a stale docblock pair the plan's own §3.1 line inventory had not caught (`WindowFirstShell.jsx`'s "App suppresses its `<header>` for this arm" and "the flag runs both layouts at once" — both fixed), and one item explicitly deferred to D3 (`BrandLockup`'s `header`-variant docblock, correctly left). Browser-verified at 1280 (backend on a scratch port, a different worktree's `main`-branch backend was already holding 8083 — left untouched): **seen** — the app boots straight to the window-first shell with no `<header>`, no `ViewToggle`, no exit button, exactly one `<h1>`; the settings modal has Profile/Home Location/Drive Times and no "Plan Layout" section; live network traffic all 200/204 once the scratch backend was up. **Asserted only, not seen**: the crash fallback (jsdom's `App.test.jsx` provider-throw test, and `PlanErrorBoundary.test.jsx`'s own suite) — no seeded data existed to reach a real Plan crash in the browser this session, and the NLC banner's inert rendering (no active sighting was seeded, so the banner never mounted to look at) — both are asserted in jsdom, not observed. |
| D2 | `claude/v1-retirement-d2-estate` | `7fc20cb7` | 2026-08-25 | The v1 component estate — §3.2 executed exactly: six whole files deleted (`DailyBriefing.jsx` 1,614, `BriefingSummaryStrip.jsx` 305, `CloseToHome.jsx` 1,247, `CardHoverPreview.jsx` 151, `ViewToggle.jsx` 69, `QualitySlider.jsx` 99 — all zero-importer since D1's App.jsx edits) with their six test files plus `regionChipVerdictColour.test.jsx` (died with the `.summary-region-chip` CSS it slices). `briefingDayStepDst.test.jsx` trimmed to its fixture and the `WindowFirstBriefingProvider` describe, with the positive mock set (`regionApi.fetchRegions`/`fetchRegionDriveTimes` added — the provider called both and this file never mocked them). `DailyBriefing.test.jsx`'s three orphaned behaviours (drive time, location-type icon, the second-cell drill-down switch — no twin anywhere else) ported into `HeatmapGrid.test.jsx` as three new describes, verified against `git show HEAD:…DailyBriefing.test.jsx` for parity; the second-cell port is stricter than the original (no silent `if (enabled.length >= 2)` skip). Dead exports gone: `briefingApi.getCloseToHome` (+ mock keys in `App.test.jsx`), `briefingDisplay.sortedSlotsByVerdict` (`VERDICT_ORDER` stays — `slotSortKey` reads it), `tierUtils`' `TIER_KEYS`/`TIER_LABELS`/`computeAuroraCellTier` (`computeCellTier`/`isCellVisible` correctly left for D3). `index.css`: `.quality-slider*` deleted, `.quality-toggle-track/-thumb` kept; both `.cth-*` blocks deleted (region cards/hover-preview/next-window in the first, window-grid/rail/rows/filters in the second) with the three `cth-preview-*` keyframes carved out and renamed `wf-peek-fade/-rise/-drop` in the same edit (`.wf-peek`'s two consumer sites updated to match); the summary-strip block and `briefing-refresh` deleted; `--color-marginal`/`--color-dust`/`--color-pick-also` deleted (zero remaining consumers after the component deletions), `--color-close-to-home(-light)` and every `heatmap-*`/`hot-topic-*`/`tide-*`/`tr-*`/`sr-*`/`map-*`/`wf-*` rule kept. Token audit run both ways (script-verified: no orphaned `var()`, no newly-dead token — the pre-existing `--color-sunrise-*`/`--color-sunset-*`/`--color-runbar-*` orphans are unchanged, correctly left for D4). The e2e spec (`forecast.spec.js`) salvaged by behaviour — v1's default-Plan/ViewToggle-driven navigation replaced with `getByRole('tab', {name: 'Map'|'Operations'})`, `date-strip` assertions moved to the `data-selected` attribute, port 8082→8083, the v1-only `error-message` card's whole describe deleted — and actually run against the §6 stack (backend `-Plocal-dev` on 8083, three locations seeded via `POST /api/locations` since no seed script exists in this worktree): **3/9 pass** (both Login-flow tests, the Operations-tab-visibility test); the other 6 need a real forecast run behind a working `ANTHROPIC_API_KEY`, which this sandbox does not have (`POST /api/forecast/run` 401'd on `invalid x-api-key`, so `allDates` stays empty and the Map tab — correctly, per existing v2 behaviour — never renders); kept rather than deleted, since the failure is an environment gap in evaluated data, not a defect in the spec or the app, and every testid/selector it targets was confirmed present by reading the current source. Gate: lint, 174/174 test files (4246/4246 tests), the ASI-class semicolon check (0 hits, exactly as §6 predicted from D2), audit, build all green; entry chunk 207.77 kB (essentially unchanged from D1's 207.09 kB — the six files were already tree-shaken out of the graph once D1 removed their only import, so their physical deletion here has no further bundle effect). Six read-only lenses (runtime, CSS/tokens, test-salvage completeness, a11y, scope-vs-§3.2, deletion completeness) — runtime, CSS/tokens, test-salvage and a11y returned zero blocking findings (one CSS/a11y-adjacent nit each, both the same finding: a stale `.cth-window-grid`/`ScrollRail` citation in `WindowSpotStrip.jsx`, fixed); scope-fidelity and deletion-completeness together found nine stale "copied-from"/`v1`-citation comments the sweep had missed against §3.2's own line list — `WindowFirstRegionalPanel.jsx` (the `SHOW_ALL_TIER` docblock, the `driveMap`/`userDriveTimes` line), `HotTopicStrip.jsx`'s "Best Bet cards and the quality slider" docblock, `relativeTime.js`'s `DailyBriefing.jsx` citation, `mapOverlay.js`'s "both arms" aurora-trigger comment + its test twin, `movementToneCascade.test.jsx`'s dead cross-reference, `WindowFirstRegionalPanel.test.jsx`'s two `DailyBriefing` mentions inside the (correctly D3-owned) `scrollable`/`serverCellRating` tests, `briefingDayStepDst.test.jsx`'s "three separate components" citation, and `WindowFirstShell.test.jsx`'s `ViewToggle:56` reference — all nine fixed, re-gated green. One finding from each lens was correctly *not* acted on: `frontend-test-standards.md:269`'s worked example citing `DailyBriefing.test.jsx` is explicitly D4's job (§3.4: "keep as history, '(since deleted)'"); `ModelTestView.jsx`/`api/lightApi.js`/`api/settingsApi.js`'s lowercase "quality slider"/"Close to home" mentions carry no file citation and were judged already compliant. Browser-verified at 1280 and 390 against the same live stack the e2e run used: **seen** — the Plan tab renders with no `<header>`, three tabs (Plan/Coming up/Operations, no Map — correctly withheld with zero forecasts), the Operations tab's Data/Operations groups and sub-tabs, the login page's tagline and kicker, all with zero console errors, at both widths; **asserted only, not seen**: the doors opening with real content, the spot peek's entry animation, and marker colours — all need scored forecast data this sandbox's missing `ANTHROPIC_API_KEY` prevented producing. |
| D3 | `claude/d3-compatibility-layer-review-3dfe66` | `3c92283f` | 2026-08-25 | The compatibility layer collapses — §3.3 executed: **ramp merge (§4.2)** — `RATING_COLOURS` and every `ramp` parameter deleted from `markerUtils.js`/`MapView.jsx`; `scoreColour`/`ratingColour`/`markerLabelAndColour`/`createClusterIcon` take one fewer argument each, every call site updated; the swatch is `rampHex(star)` unconditionally; the `MarkerClusterGroup` remount `key` is gone and the popup-survives-toggle consequence is pinned by a new test (`MapViewHeat.test.jsx`); eleven `vi.mock('markerUtils.js')` factories lost their `RATING_COLOURS` key; `markerRampOptIn.test.js` deleted, its two surviving invariants folded into `MarkerIcon.test.jsx` as literal-stop assertions, its clamp-not-grey behaviour change named. **Astro removal (fact 3)** — `ratingStyle`, the astro cell branch, the column-header's ASTRO arm, `getRegionLocationNames`, `astroScoresByDate` (prop + fetch + ~9 fixtures), and the CSS selector all deleted; `WindowFirstRegionalPanel`'s astro `useEffect`/`useState` removed entire; the promised pin written (`HeatmapGrid.test.jsx`, a fixture with both SUNRISE and SUNSET columns asserting no astro title/glyph); the map's own separate ASTRO mode (`MapView.jsx`'s `astroApi` import) correctly untouched, and — a plan correction found on re-verification — `App.test.jsx`'s `astroApi` mock is **not** dead for the reason the plan gave (`MapView` still calls it for that mode) and was deliberately left rather than force-deleted. **`scrollable`/`serverCellRating` collapse** — the scroll port renders unconditionally with the a11y triad intact (`tabindex="0"`/`role="region"`/"scrolls sideways" name — browser-confirmed at both 1280 and 390); `colFloor` is a bare `96px`; `HeatmapCell` reads `region?.meanRating` directly, the 45-line client-join comment reduced to two lines; the "TWO tests, only one dies" trap handled correctly (the keyboard-reachability pin kept, retitled, `scrollable: true` dropped); `qualityTier`/`isCellVisible` fully deleted including `tierUtils.js`'s own definition and its six-test describe (missed on the first pass, caught by two independent review lenses). **`HealthIndicator`/`BrandLockup`** collapsed to their sole surviving callers per §3.3's JSX-derived (not docblock-derived) deletion list; a dead ternary arm the first pass left behind (`BrandLockup`'s kicker, unreachable once `'header'` was gone) was caught by review and removed. **Modal ruling (§4.3)** — recorded in code (`useDialogFocus.js`'s new ruling paragraph, `Modal.jsx`'s rule-10 citation rewritten to "discharged", `WindowFirstShell.jsx`'s cog comment) rather than only in this document; `stacked` and its ~4 real call sites structurally untouched, confirmed byte-identical against `origin/main`. Gate: lint, 173/173 test files (4206/4206 tests — down 6 from the `isCellVisible` describe's deletion), the ASI-class check (0 hits), audit, build all green; entry chunk 207.77 kB → 199.46 kB. Six read-only lenses **on Opus** per the owner's standing offer (§0), refuters would have run on Sonnet but none were spawned separately — every finding below was independently corroborated by a second lens or directly re-verified against the tree before acting, which this project's SF-1/PROC precedent treats as at least as strong as a single dedicated refuter. **One BLOCKING finding, fixed**: the runtime lens caught an infinite loop the implementing session had just introduced fixing a test-quality finding from a parallel lens — `for (const args of markerCalls) { markerLabelAndColour(...args) }` in `MapViewHeat.test.jsx`, where `markerLabelAndColour` is the very mock that pushes onto `markerCalls`, so the live array grew under its own iterator; reproduced deterministically (OOM after ~500s, and the whole-suite gate had *already reported green* around it — trap 1 in a new shape). Fixed by iterating a snapshot; verified non-hanging in isolation before re-gating. **Scope findings**: an unrelated, pre-existing uncommitted backend change (`BriefingBestBetAdvisor.java`, an Anthropic SDK `ThinkingConfigAdaptive` compat fix) was found in the shared worktree, confirmed by file mtime to predate this session's first edit — left untouched, excluded from this commit, flagged for the owner rather than reverted or absorbed. **Comment-sweep findings** (all fixed): `WindowFirstBriefingContext.jsx`'s "two arms" and stale `CardHoverPreview`/`CloseToHome` citations; `index.css`'s `.heatmap-scroller` "frozen arm's panel" comment (independently caught by three lenses), its stale 40-not-10 z-index stacking ledger, and its run-bar comment still citing the deleted `RATING_COLOURS` table by name; `MapView.jsx`'s "v1's framing" (three sites, including a same-file internal inconsistency against a test the plan itself had already retargeted) and its `driveMinutesFor` caller-list; `locationTypes.js` and `WindowFirstComingUp.test.jsx`'s explicitly-named-but-missed sweep-list entries; an overclaim this session's own rewrite introduced ("the map's only colour language", contradicted 30 lines below by the wildlife/stand-down/no-data non-ramp fills — fixed in `markerUtils.js` and `scoreRamp.js` to say "only *score* colour language"). **Test-quality findings** (fixed): the marker-colour-parity test asserted a call made fresh from the test body rather than `MapView`'s actual recorded call (now asserts on the real `markerCalls` — this is the edit that introduced the blocking runtime bug above); a stale test title implying a distinction (`ratio colour not score colour`) that no longer exists; a dropped negative assertion in `BrandLockup.test.jsx`'s auth-size test; the remount pin's monotonic counter couldn't distinguish "never remounted" from "unmounted and gone" (added a second assertion that the layer is still live). **A11y finding, documented not code-fixed**: independent luminance recomputation of both palettes found the 2★ marker/cluster label (`#0f172a` ink on the 2★ ramp stop, `#C8452F`) drops from a passing 4.61:1 to a failing 3.70:1 against WCAG's 4.5:1 text threshold — a real regression the ramp merge introduces at that one step, not the "not a change this series makes" §4.2/§8 item 13 wording claims (1★ and 5★ both improve or are unchanged in direction). Recorded here rather than silently absorbed or unilaterally redesigning the ramp's ink; §4.2/§8's wording should be corrected and the ink question re-opened as a named follow-on. e2e (`forecast.spec.js`, same §6 stack, backend re-seeded with one region and three locations since this worktree's H2 file predates D2's): **3/9 pass** (both Login-flow tests, Operations-tab-visibility) — the other 6 need real Claude-scored data this sandbox's `ANTHROPIC_API_KEY` cannot produce, unchanged from D2's finding and not a regression. Browser-verified at 1280 and 390 against a freshly seeded backend (one region, three locations, a triage-only briefing run — no live API key): **seen** — the login page's tagline and kicker (the `BrandLockup` auth pin), the masthead with no duplicate header, the Plan matrix, the regional planner's `HeatmapGrid` with no astro column and the scroll port's full a11y triad, all at both widths with zero non-font console errors; **asserted only, not seen**: ramp marker/cluster colours and the popup-survives-toggle behaviour (both need real Leaflet + scored locations a jsdom/headless run without tiles cannot show meaningfully; pinned instead by the MapView test suite), the Plan overlay and Map tab in heat/medallions view (`allDates` stays empty without Claude-scored `forecast_evaluation` rows, exactly D1/D2's limitation). |
| D4 | `claude/v1-retirement-d4-settling-rdwska` (merged from the rebased `…-r1`) | `84e28bbc` (#625) | 2026-08-25 | The settling commit — §3.4 executed: **CLAUDE.md rewritten truthful**, every cited site (`:8` frontend surface list; `:22` briefing bullet — the slider named a dead export deleted with `QualitySlider.jsx` in D2, its gating machinery named as deleted with D3, SSE stated gone, `bestBets*`-field-vs-advisor distinction stated precisely with the advisor's three live consumers named; `:24-26` the Plan-tab bullet de-flagged entire; `:48` "as it does in v1" → "cycles to the rest of the page"; `:52`/`:184` confidence-surface lists rewritten to the real `ProvisionalMark`/`confidenceScalar` consumers, Best Bet/summary-strip/mobile-rows named as dead; `:84`/`:281`/`:452` "Manage tab" → Operations, all three, browser-confirmed against the real Operations→Operations→Scheduler path; `:86` hash navigation → `ManageView`'s own round-trip, deep links named as no-ops; `:163` Backend-heavy debt entries discharged; `:164` panel-contracts bullet → `/reach`+`/light` as the live successors; `:183` Close-to-home bullet collapsed to a two-line backend note; new facts for `DailyBriefingResponse`'s dead fields and `auroraTonight/Tomorrow`'s surviving reader) — plus README `:19`/`:161`/`:253-256`. **Docs struck, not status-lined**: plan-matrix-plan.md (§3 rule 2's second sentence and rule 10 struck, §10 risk 5 annotated — not struck, per the distinction the brief draws — §11 discharged, the flag-recipe bullet struck, §11c's last bullet ticked); heat-field-plan.md (scope guard, the D2 row's stale clause, item 1, the "mounted three times" risk note); window-first-redesign-plan.md §4; plan-verdict-consolidation-plan.md `:15` (frontend half discharged, backend half open); plan-panel-data-contracts.md rewritten wholesale where live (Section 1 table including row 1, the five-endpoints paragraph replaced with the provider's real seven calls, Section 3, Section 4, Section 5 item 1 — plus the TL;DR and "The question" framing, caught stale by the copy-lens review and fixed in the same pass since they directly contradicted the just-rewritten Section 1); phone-heatmap-blast-radius.md, frontend-test-standards.md `:269` ("(since deleted)"), window-first-p15-handover.md — all with status notes, none of their historical build-log content touched. **The dead sweep**, its own hunk: seven zero-importer components deleted (`StarRating`, `ScoreBar`, `WindIndicator`, `VisibilityIndicator`, `CloudCoverBars`, `LocationTypeBadges`, `PopoverHost`→`usePopoverHost`→`popoverPlacement`, +4 test files) confirmed by reverse search at edit time (`PlanScoreBar`, a live and unrelated component, correctly not caught by a substring match on "ScoreBar"); five exports (`briefingDisplay`'s re-exported `LOCATION_TYPE_ICONS`, `conversions.metresToKm`/`.formatShiftedEventTimeUk` with the `leaveBy.js` citing comments rewritten in the same hunk, `markerUtils.standDownColour` — the const `STAND_DOWN_COLOUR` correctly kept); CSS (`--color-sunrise-*`/`--color-sunset-*`, `.heatmap-cell-visible/-hidden` by selector, `--color-verdict-awaiting` + its citing comment in `WindowSheetDialog.jsx`); `--color-runbar-1..5` left with its comment reworded; `OutcomeModal`+test and the five live-endpoint API bindings correctly untouched (§8 owner questions, confirmed by an empty `git diff` over both). **Bundle check (§7)**: entry chunk 340.52 kB → 199.91 kB (−41%, gate met; unchanged by the sweep itself since every deleted file was already tree-shaken out — the entry moved only from the full-surface review's own runtime fixes, §5 lines below); both expected lazy-chunk moves independently confirmed by string search in the built assets (`date-strip` testid present only in the `WindowFirstMapPane` chunk, `/api/astro` present only in the `MapView` chunk); 15 CSS tokens removed across the whole series (14 in D2/D4 as planned, plus one D4 orphaned itself — `--color-plex-gold-dark`, caught by the CSS/tokens review lens). **Full-surface adversarial review**, seven lenses on Opus (runtime, CSS/tokens, test quality, accessibility, plan compliance, deletion completeness, copy) over the WHOLE frontend, refuters on Sonnet, one per charge defaulting to REFUTED: ~35 charges total. **Confirmed and fixed** (all FIX-NOW verdicts from refutation): `--color-plex-gold-dark` orphaned by D4's own `ScoreBar.jsx` deletion (deleted); `PlanErrorBoundary`'s three storage-clear calls unguarded against a storage-denied browser throwing mid-handler (wrapped in `try/catch`, new test); `PlanErrorBoundary` never moved focus to its fallback on crash, orphaning a keyboard/AT reader at `<body>` (added `componentDidMount`+`componentDidUpdate` focus management — the mount hook needed because a first-render throw commits as a mount, not an update; new test); a THIRD unguarded dialog-stacking route found by the runtime lens, `WindowFirstShell.jsx`'s beyond-line search link (`onSearchRegion`), reachable by Tab from an open location sheet and painting a new `PlanSearch` UNDER it (paint order is DOM order for two same-z-index `Modal`s) — guarded with the same `stackedOverPopup` check the other two routes already use, new pinning test in `locationSheetShell.test.jsx`; `MapView.jsx`'s three `useState` initializers and six write sites read/wrote `localStorage` unguarded during the render phase (six call sites now route through three small guarded helpers, matching `useLocalStorageState.js`'s existing convention; two new tests in `MapViewStarFilter.test.jsx`); `HealthIndicator`'s panel had no Escape dismissal and no `aria-controls` (both added, two new tests, browser-confirmed — Escape closes the real panel in a live headless-Chromium run); `NlcSightingBanner` kept `select-none` after D1 made it permanently inert (removed). Several comment-accuracy findings fixed directly (a self-introduced error in this session's own `CLAUDE.md:184` rewrite conflating `confidenceScalar` with `ProvisionalMark`'s marker-on-low, caught by the copy lens; the "quality slider" bullet's "never gated the grid" claim, which D3's own tree contradicted; the plan-matrix §11c tick's "complete on `main`" overclaim on an unmerged phase; ~10 stray "in both arms"/deleted-symbol citations across `HeatmapGrid.jsx`, `index.css`, `setup.js`, `vite.config.js` and five test files that D1–D3's own sweeps had listed and missed, or that D4's own deletions freshly orphaned). **CONFIRMED but named as follow-ons, not fixed** (architectural or needing live verification beyond this phase's means): `PlanErrorBoundary` does not extend to its App-level siblings (`MapOverlay`/`UserSettingsModal`), so a render throw there still takes down the whole app with no fallback — needs its own fallback UX design, folded into §8 item 9 beside the modal-`inert` follow-on it pairs with; a fourth unguarded dialog route, the masthead health pill, whose panel's CSS stacking context (nested inside `.wf-mast`, a `position:sticky` ancestor with its own `z-index`) traps its nominal `z-index:9999` beneath an open `Modal`'s `z-50` — mechanism confirmed by source, paint order not browser-measured, added to §8 item 9's list; the `role="alert"` live-region timing gap in `PlanErrorBoundary`, refuted as a standalone action item once the focus fix landed (§8 item 21). §4.2/§8 item 13's marker-ink wording corrected per D3's own handoff note (the 2★ stop is a real regression, 4.61:1→3.70:1, not "pre-existing on every marker" as the old wording claimed for all three cited stops). Gate: lint, 170/170 test files (4166/4166 tests, +7 from the review's new pinning tests), the ASI-class check (0 hits), audit, build all green. Browser-verified at 1280 and 390 in headless Chromium against a backend brought up via a local workaround for this sandbox's inability to reach JitPack (network-policy-blocked, not worked around by disabling any check — `solar-utils` installed to the correct Maven coordinate from the jar already vendored in-tree, entirely offline): **seen** — the login page's tagline and kicker at both widths; the signed-in shell with no duplicate header, three tabs (no Map — correctly withheld with zero seeded forecasts, this sandbox having neither a working `ANTHROPIC_API_KEY` nor Open-Meteo network access, a harder version of D1–D3's own documented limitation); the Operations tab's Data/Operations groups down to the Scheduler sub-tab, confirming the CLAUDE.md path rewrite; the `HealthIndicator` panel opening on click and **closing on Escape in a real browser**, confirming the a11y fix; zero non-external-domain console errors (the one failure, Cloudflare Turnstile's script, is expected in this sandbox and unrelated to this phase). **Asserted only, not seen**: the `PlanErrorBoundary` crash fallback's focus-management and storage-guard fixes (9/9 jsdom tests, no seeded data reaches a real crash in this browser session — same limitation D1 recorded for the same component); the health-pill stacking defect (source-verified, no open dialog was reachable without scored data to test the live paint order against); marker/cluster ramp colours and the Plan matrix's populated states (need Claude-scored `forecast_evaluation` rows this sandbox cannot produce, exactly D1–D3's limitation, one degree worse here). |
...
surfaces inheriting a contrast fault heat view already had. **Resolved post-series (§8 item 13):**
the marker and cluster label ink is now derived per fill through `readableInkOn`, the same two-ink
rule the v2 spot badges carry — every ramp stop clears 4.5:1, pinned by a computed (not tabulated)
AA sweep in `MarkerIcon.test.jsx`. The `HeatmapGrid` astro cell, the one non-map `RATING_COLOURS` reader, is a
...
Taken on D4's branch, `frontend/`, with `npm ci` then `npm run build` (2026-08-25), against the
same methodology §2 used:

| measure | before (§2) | after | delta |
|---|---|---|---|
| source files (`src/**/*.{js,jsx}` excluding `src/test`) | 203 | 188 | −15 |
| test files | 179 | 170 | −9 |
| components (`src/components/*.jsx` + shared + chart) | 87 + 11 + 2 | 75 + 11 + 2 | −12 |
| hooks / utils / api modules | 24 / 45 / 27 | 22 / 44 / 27 | −2 / −1 / 0 |
| source LOC (non-test) | 50,217 | 45,651 | −4,566 |
| test LOC | 62,468 | 57,145 | −5,323 |
| `src/index.css` | 5,356 lines | 4,859 lines | −497 |
| production bundle (js + css, 26 files) | 1,704,714 bytes | 1,630,167 bytes (31 files) | −74,547 bytes |
| entry chunk `index-*.js` | 340.52 kB (gzip 98.46) | 199.91 kB (gzip 54.89) | −140.61 kB |
| `index-*.css` | 134.78 kB (gzip 23.12) | 123.70 kB (gzip 21.48) | −11.08 kB |
| CSS custom-property tokens removed (whole series) | — | 15 | `--color-marginal`, `--color-dust`, `--color-pick-also` (D2); `--color-sunrise-50/100/500/600/700`, `--color-sunset-50/100/500/600/700`, `--color-verdict-awaiting`, `--color-plex-gold-dark` (D4 — the last one orphaned by D4's own `ScoreBar.jsx` deletion, caught by the CSS/tokens review lens) |

**The gate is met: the entry chunk shrank** (340.52 kB → 199.91 kB, −41%), continuing the run the
Phase log already shows: 340.52 kB (before) → 207.09 kB (D1) → 207.77 kB (D2, essentially unchanged
— the six files D2 physically deleted were already tree-shaken out of the graph once D1 removed
their only import) → 199.46 kB (D3) → **199.91 kB after D4**. The dead-sweep itself changed nothing
(every file it deleted was already a zero-importer orphan); the +0.45 kB over D3 is the
full-surface adversarial review's own small runtime fixes (a storage guard and focus management in
`PlanErrorBoundary`, a stacking guard in `WindowFirstShell`, guarded `localStorage` access in
`MapView`, an Escape handler in `HealthIndicator`) — real behaviour, not import-graph movement, so
it does not contradict "the sweep changed nothing." The source-file/test-file/component/hook/util
counts in the table above are the dead sweep's own count (no whole file was added or removed by
the review's fixes, only edits within existing files, plus ~90 lines of new test coverage across
five test files not separately broken out here).

**Chunk-by-chunk diff against §2's expected shape, verified by string search in `dist/assets/`
(minified, so component names do not survive, but route-defining literals do):**

- ✅ **`DateStrip` moved into the `WindowFirstMapPane` chunk**, as predicted. The `date-strip`
  testid string appears once in `WindowFirstMapPane-*.js` and zero times in the entry chunk.
- ✅ **`astroApi` moved into the `MapView` chunk**, as predicted. The `/api/astro` path string
  appears only in `MapView-*.js` (`grep -rl 'api/astro' dist/assets/*.js` returns exactly that one
  file) — zero other chunk, including the entry, references it.
- **No other lazy-chunk growth investigated as unexpected.** The chunk list itself has more
  entries than §2's snapshot (31 js+css files vs 26) — expected, not a regression: §2 was measured
  after M1–M5 had already introduced most of this splitting, and the growth in chunk *count* here
  reflects Rollup re-splitting the graph as v1's `DailyBriefing.jsx` (which held its own
  independent copies of several shared derivations) stopped anchoring a second import path through
  those modules; the *total* bundle byte count fell regardless (−74,974 bytes).

**Whole-series file/LOC delta, from `git diff --stat 1281e64a -- frontend/src`:** 133 files
changed, 2,644 insertions, 13,209 deletions (28 files deleted, 1 renamed, 100 modified, 4 added —
`PlanErrorBoundary.jsx`/`.test.jsx` is §4.1's redesign, `PlanScoreBar.jsx`/`planScoreConsistency.test.js`
is unrelated ordinary development that landed on `main` during the series, not part of this plan).
This is a whole-tree diff since the plan's baseline commit, so it also carries the handful of
unrelated commits that merged to `main` alongside D0–D3 (e.g. the location-sheet golden/blue-hour
phase) — it is reported here rather than hidden because §2's own baseline is a point-in-time
snapshot, not a v1-retirement-only diff.
...
9. **The structural "one modal" fix** — `inert` on the shell's content root while any dialog is open,
   ...
   App-level siblings; browser measurement). Plus the three unguarded keyboard routes found by
   survey (doors' map buttons, `AuroraBanner`, and — found by D4's full-surface review — the
   masthead health pill, `WindowFirstShell.jsx` around `{healthPill}`: `HealthIndicator`'s own
   panel is `position:fixed; z-index:9999`, but it renders inside `.wf-mast`, a `position:sticky`
   ancestor with its own `z-index`, which traps the panel's stacking context beneath an open
   `Modal`'s `z-50` — so the panel can open, invisibly, from a keyboard route through an open Plan
   dialog. Admin-only; CSS stacking-context mechanism confirmed by source, paint order not yet
   browser-measured), all three unmeasured.
...
13. **Marker ink contrast on the ramp — RESOLVED 2026-08-25 (post-series, owner-requested).**
    D3's merge took the 2★ label from a PASS on `RATING_COLOURS` (4.61:1) to a FAIL on the ramp
    (3.70:1); 1★ had failed since P4 (2.96:1). The fix taken is the re-derive arm: `buildMarkerSvg`
    and `createClusterIcon` now pick their label ink per fill through `readableInkOn` — the rule
    the v2 spot badges already carried — so 1★/2★ print white, 3★+ print the dark ink, every stop
    clears 4.5:1 (the no-data grey's em-dash too, which the dark ink never cleared), and a computed
    AA sweep in `MarkerIcon.test.jsx` fails if a future ramp or ink change drops any stop below.
...
20. **`PlanErrorBoundary` does not extend to its App-level siblings.** Found by D4's full-surface
    review: `MapOverlay`/`MapView` and `UserSettingsModal` are rendered as siblings of the
    boundary in `App.jsx`, not children — a render-phase throw in either (e.g. `MapView`'s
    `localStorage` reads, now guarded per §3.4's runtime-lens fix, but any other render throw
    there is equally uncaught) unmounts the whole app, with no fallback at all, worse than the
    crash the boundary exists to prevent. §4.1's boundary placement was deliberate for the
    hydration-trap crash it targets and is pinned by a test — extending coverage to these two
    siblings needs its own fallback UX design (should a map crash sign the reader out? just close
    the overlay?) and is design work, not settling. Natural pairing with item 9's "App-level
    siblings need the same treatment" for the modal-`inert` fix — both are "these two siblings
    need containment they don't have."
21. **`PlanErrorBoundary`'s `role="alert"` is inserted into the DOM together with its content**,
    which this codebase elsewhere treats as unreliably announced (`WindowSpotSheet.jsx`,
    `WindowSheetDialog.jsx`, `WindowFirstComingUp.jsx` each instead keep an always-mounted live
    region and toggle only its text). The boundary cannot use that pattern — there is no
    always-mounted sibling inside it to host a persistent region, since the whole subtree just
    unmounted. D4 fixed the sharper half of this (focus moves to the fallback heading on crash,
    which is a reliable announcement route independent of `role="alert"` timing); the live-region
    half is a belt-and-braces residual for a reader who does not receive focus events, found and
    assessed by review as not worth its own design work on top of the focus fix.
