...
const S={opt:'A',ei:0,org:null,view:'heat',area:true,rate:'any',reach:'any',subj:new Set(),dark:false,
...
function buildMenu(){
 ...
 h+=`<button class="wmore" data-more="${S.ei}">▤ This window, region by region ›</button>`;
 $('wmenu').innerHTML=h;
...
const evVerd=i=>{
 const e=EV[i],r=regionsFor(i)[0],m=r?r.mean:0;
 if(e.k!=='solar')return{solar:false,mean:m,top:r,tier:'na',
  word:e.k==='astro'?(e.clar>=4?'Clear':'Cloudy'):'Kp '+e.kp};
 return{solar:true,mean:m,word:vWord(m),tier:vTier(m),top:r}};
...
function paintWin(){
 ...
 w.innerHTML=`<span class="wk ${evKind(e)}">${e.name}</span>`
  ...
  +(S.opt==='C'?`<span class="vdot" style="background:${VCOL[v.tier]}"></span>`
   :`<span class="wv ${v.tier}">${v.word}</span>`)
  +`<span class="cv">▾</span>`;
...
function openWindowPanel(i){
 ...
 rpOpen(`<div class="rph"><div><b>${esc(evLabel(e))} ${e.name.toLowerCase()}</b>`
  ...
  +`<div class="rpn">${v.solar
   ?"Verdict is the strongest region's average for this window. Regions rank on that average, not on their ceiling — the reach filter only decides what the map draws."
