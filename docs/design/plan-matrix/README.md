...
- Region cards (`grid auto-fit minmax(128px,1fr)`, iPad `130px`, phone 2 columns, `gap 6px`): `border 1px var(--border)` with `border-left 2px` in the grade colour, `radius 8px`, `background var(--panel)`, `padding 7px 9px 8px`, `min-height 56px`. Name 11.5px 600; verdict word mono 9.5px 600 in the grade colour; footer mono 9px `--ink-3` — `best 5★ · 10 in reach`, or a `--tide`-coloured `2h 38min away` when nothing is in reach. Selected: `rgba(201,162,75,.1)` / `rgba(201,162,75,.5)`. First card is `All 7 regions` (left border `--home`) and acts as the clear. **Adaptation (2026-09-14).** These minimums follow what the prototype *renders*, with one deliberate exception. The rail only ever sits in the popup's side column, where the prototype's more specific `.wside .rrail` wins over the bare `.rrail`: 112px desktop, 130px iPad, two up on phone. iPad ships 130px, not the 150px this line once gave (that is the bare rule, which loses). **Desktop keeps 128px rather than the prototype's 112px, on purpose.** With the field map, the only layout the prototype draws, the two render identically: 3 × 143px, because the popup is capped at 780px and the rail is 442px wide. Without the map, the full-width rail shown while the heat catalogue loads or has failed, which the prototype never draws, 112px gives 6 × 120px and wraps every card's `best 5★ · 10 in reach` onto two lines, growing the cards from 63–76px to 90px; 128px keeps 5 × 145px on one line. So 128px renders the designed layout exactly as 112px would and the undesigned one correctly. Measured in real Chromium before deciding; owner decision.
