...
/* Hot topics: things true of a NIGHT rather than of a location, so they attach to windows.
   Eligibility is DERIVED, not declared: `needs` names a location property, and a region is
   eligible for the topic if it holds a named location with that property. A king tide is only
   true where there is tide; an inversion needs valleys to pool fog in. Aurora and NLC carry no
   `needs` because at this latitude they are sky-wide — the whole planning area sees them or
   none of it does. So moving the origin to the Lake District drops the tide topic by itself. */
const TOPICS={
 aur:{ic:'🌌',n:'Aurora possible',sh:'Aurora',c:'#7C8DD6',w:1,
  sci:'Kp (0–9) tracks how disturbed Earth’s magnetic field is; the higher it climbs the further south the band reaches. Kp 5 puts it within range of a clear northern horizon here.'},
 nlc:{ic:'✦',n:'Noctilucent cloud season',sh:'NLC',c:'#8E86D6',w:2,
  sci:'Nothing forecasts the mesosphere at 80 km, so this names the twilight window when NLC would be visible if it is there — not a promise that it will be.'},
 ktide:{ic:'🌊',n:'King tide',sh:'King tide',c:'#6FA8B0',w:3,needs:'coast',scope:'coastal regions',
  sci:'A spring tide near perigee, so the range is the biggest of the month: more wet sand and reflection at low water, more force and spray at high.'},
 inv:{ic:'☁️',n:'Cloud inversion',sh:'Inversion',c:'#B4BEC0',w:4,needs:'lake',scope:'lake and valley regions',
  sci:'Clear skies and light winds let cold air pool in the valleys overnight, trapping fog beneath warmer air above — so height decides whether you are under it or above it.'},
 dust:{ic:'🏜️',n:'Saharan dust',sh:'Dust',c:'#F5991F',w:5,
  sci:'Dust carried north on southerly winds scatters blue light out of the low sun, pushing sunrise and sunset colour further into red.'}};
const WTOPICS={
 w1:[{t:'nlc',d:'Clear northern horizon, 48 dark-sky locations'},
  {t:'ktide',d:'3.2 m range, high water 36m before sunset'},
  {t:'dust',d:'Elevated dust, vivid colour potential at both ends of the day'}],
 w3:[{t:'aur',d:'Kp 5 forecast, worth watching'},
  {t:'ktide',d:'Low water lands on the window — wet sand the length of the coast'}],
 w4:[{t:'aur',d:'Kp 5 holding through the second half of the night'}],
 w6:[{t:'inv',d:'Strong inversion likely at elevated locations'}]};
/* which regions a topic can honestly apply to, from the location set itself */
const hasKind=(rid,k)=>SPOTS.some(s=>s.rid===rid&&s.named&&s[k]);
const topicRids=t=>{const need=TOPICS[t]&&TOPICS[t].needs;
 return REGIONS.map(r=>r.id).filter(rid=>!need||hasKind(rid,need))};

window.PLAN={REGIONS,HOME,WINS,CONF,RUNAGE,DELTA,NARR,WHY,SPOTS,SETUP:20,TOPICS,WTOPICS,
 GLANCE,nearestMin,areaRids,beyondRids,ridsOf,spotsIn,hasKind,topicRids,
