...
/* ── tide alignment, summarised ───────────────────────────────────────────────
   The same model the Map tab's tide strip uses — one enum per coastal location for the water
   it wants, a cosine curve between extrema, and the level at the moment of the light — read
   here as a COUNT rather than a curve. The plan grid's job is comparison across windows, so
   what it needs from tide is not "what is the water doing" but "is the coast live on this
   one", which is a number you can scan down a row.

   It goes in slots that already exist: a glyph on the spot the card already names, a chip in
   the topics line (alignment is a property of a night, which is what that line is for), and
   the pick legend on the border for the one window in the forecast where the coast is most
   live. No new row, no new caption, no second ranking. */
const HALFC=372.5;
const TIDEX={
 w1:{k:'HW',at:'19:52',range:3.2,low:0.9},w2:{k:'LW',at:'02:34',range:3.4,low:0.9},
 w3:{k:'LW',at:'20:04',range:2.9,low:1.1},w4:{k:'HW',at:'03:12',range:3.1,low:1.0},
 w5:{k:'HW',at:'20:46',range:3.0,low:1.0},w6:{k:'HW',at:'06:04',range:3.6,low:0.8}};
...
function tideWinOf(i){
 const w=WINS[i],x=TIDEX[w.id];if(!x)return null;
 const a=tMin(x.at),t=tMin(w.time),ph=(t-a)/HALFC;
 const lvl=x.k==='HW'?(1+Math.cos(Math.PI*ph))/2:(1-Math.cos(Math.PI*ph))/2;
 const band=lvl>=0.75?'high':lvl<=0.25?'low':'mid';
 return{...x,lvl,band,m:x.low+lvl*x.range,
  state:(band==='mid'?'mid tide':band+' water')+', '+(((x.k==='HW')===(t>=a))?'falling':'rising')};
}
const tideFitP=(s,i)=>{const tw=tideWinOf(i);if(!tw||!s.coast)return null;
 const want=wantOf(s),fit=1-Math.abs(tw.lvl-TGT[want]);
 return{...tw,want,fit,tier:fit>=0.8?'match':fit>=0.62?'near':'miss'}};
/* counted over the reachable named locations, so the number is one you could act on */
function tideStats(i){
 const co=reachPool(i).filter(s=>s.coast);
 const fits=co.map(s=>tideFitP(s,i));
 const m=fits.filter(f=>f&&f.tier==='match');
 return{tw:tideWinOf(i),coastal:co.length,match:m.length,
  miss:fits.filter(f=>f&&f.tier==='miss').length,
  /* how WELL the matched spots are served, which is what breaks the common tie: three
     windows in a run routinely suit the same nine locations, and the one to take is the one
     whose water sits closest to what they each want */
  fit:m.length?m.reduce((a,f)=>a+f.fit,0)/m.length:0};
}
/* CUT: a third mark, a Tide legend on the card border, naming the one window in the forecast
   where the most coastal locations get their water. Built, tested, removed — see the note on
   the page. Short version: that border slot belongs to the PICK system, which is a ranking
   (one Best bet per forecast, mutually exclusive with Also good). Tide is not a ranking, so a
   count-derived badge in a ranking slot either reads as a pick it is not, or gets skipped as
   chrome — and it had to be hunted for. It also carried no fact the chip did not already
   carry. The floor survives because the chip uses it. */
const TIDE_FLOOR=3;
/* What replaced the cut legend. The badge failed because it was a NEW ELEMENT in a slot that
   means something else. The fact it was trying to carry — "of the windows where the coast is
   live, this is the one where it is most live" — is a difference of DEGREE, so it belongs in
   the chip that already states the degree: same element, same position, more weight. Nothing
   new to find, and the eye is already there because it is reading the numbers. */
/* Alignment is rarely one window. The tide runs on a ~14-day beat, so a good phase puts the
   coast live on SEVERAL consecutive windows — which turns the useful question from "is the
   coast live" into "which of these do I take". That is what the run count and the emphasis
   answer together: N windows are live, and this is the one where the most spots get their
   water. The chip states both, in the slot it already occupies. */
const tdShowOf=i=>{const t=tideStats(i);
 return t.coastal>=4&&t.match>=Math.max(TIDE_FLOOR,Math.ceil(t.coastal*0.5))};
let _live=null,_tw=null;
const tideLive=()=>_live||(_live=WINS.map((w,i)=>i).filter(tdShowOf));
function bestTideWin(){
 if(_tw!==null)return _tw;
 /* count first, then mean fit, then the earlier window — a tie on both is a genuine tie and
    the earlier one is the one you can act on sooner */
 const r=tideLive().slice().sort((a,b)=>{const x=tideStats(a),y=tideStats(b);
  return y.match-x.match||y.fit-x.fit||a-b});
 return(_tw=r.length?r[0]:-1);
}
let TD_EMPH=true;
const TWAVE='<svg class="twv" viewBox="0 0 14 8" aria-hidden="true">'
...
/* Geography on the field. Every number on a card — drive time, leave-by, the reach filter —
   is measured from one point the picture never showed, so the picture now states it. Area
   names come next, BBC-style: the ground gets named, the hottest region brightest. */
const HOMEPT=[-1.573,54.855];   // DH3 4NG
const SHORT={ntw:'NORTHUMBERLAND',penn:'NORTH PENNINES',nymc:'NORTH YORK MOORS',lakes:'LAKE DISTRICT',dales:'YORKSHIRE DALES',borders:'BORDERS',peak:'PEAK DISTRICT'};
const TINY={ntw:'NORTHUMB.',penn:'PENNINES',nymc:'N Y MOORS',lakes:'LAKES',dales:'DALES',borders:'BORDERS',peak:'PEAK'};
const SVGNS='http://www.w3.org/2000/svg';
/* px per km, measured off the projection rather than assumed, so a ring is real distance */
const kmPx=proj=>{const a=proj(HOMEPT),b=proj([HOMEPT[0],HOMEPT[1]+1]);return Math.abs(b[1]-a[1])/111.2};
const mkLab=(cls,html)=>{const d=document.createElement('span');d.className=cls;d.innerHTML=html;return d};
/* one greedy pass, priority order: a label that cannot find clear air is dropped, never
   stacked — an unreadable name is worse than a missing one */
function placeLabels(host,items,w,h,boxes){
 boxes=boxes||[];
 const hit=(a,b)=>a.x<b.x+b.w+3&&b.x<a.x+a.w+3&&a.y<b.y+b.h+2&&b.y<a.y+a.h+2;
 items.forEach(it=>{
  const el=it.el;el.style.left='-9999px';el.style.top='0';host.appendChild(el);
  const bw=el.offsetWidth,bh=el.offsetHeight;let ok=false;
  for(const dy of [0,-13,13,-24,24,-36,36]){
   const b={x:it.x-bw/2,y:it.y-bh/2+dy,w:bw,h:bh};
   if(b.x<1||b.y<1||b.x+b.w>w-1||b.y+b.h>h-1)continue;
   if(boxes.some(o=>hit(b,o)))continue;
   boxes.push(b);el.style.left=b.x+'px';el.style.top=b.y+'px';ok=true;break;
  }
  if(!ok)el.remove();
 });
 return boxes;
}
function labelThumb(gl,proj,w,h,i){
 if(!gl)return;gl.innerHTML='';
 if(!proj)return;
 const rids=railRids(),hot=rids.reduce((a,b)=>rScore(b,i).avg>rScore(a,i).avg?b:a,rids[0]);
 const tiny=w<215,items=[];
 if(isHome(S.origin)){const p=proj(HOMEPT);
  items.push({el:mkLab('hm','<i class="mk"></i><span class="lb">HOME</span>'),x:p[0],y:p[1]})}
 rids.forEach(rid=>{
  const c=HeatField.centroid(scopeSpots(),rid,s=>proj([s.lng,s.lat]));if(!c)return;
  items.push({el:mkLab('rg2'+(rid===hot?' hot':''),(tiny?TINY[rid]:SHORT[rid]).replace(/ /g,'&nbsp;')),x:c[0],y:c[1]})});
 placeLabels(gl,items,w,h);
}
...
function bumpCache(){_area=_rids=_spots=_rank=null;_tw=_live=null}
...
const sunMark=w=>{const am=/sunrise/i.test(w.when);return `<span class="ar ${am?'am':'pm'}">${am?'Sunrise':'Sunset'}</span>`};
...
function renderStrip(){
 ...
 const gap=(slot)=>{const g=document.createElement('div');g.className='hg';
  g.innerHTML=`<span>${slot==='am'?'this morning has gone':'past the end of the forecast'}</span>`;return g};
 if(isMob()){
  /* the phone reads day by day: a heading, then that day's one or two windows */
  days.forEach(d=>{
   const solo=(d.am==null)!==(d.pm==null);
   const hd=document.createElement('div');hd.className='dh'+(d===days[0]?' tdy':'');
   hd.innerHTML=`<div class="cal"><span class="cd">${d.dow}</span><span class="cn">${d.dn}</span></div><i class="cr"></i>`;
   el.appendChild(hd);
   ['am','pm'].forEach(slot=>{const i=d[slot];
    if(i==null){el.appendChild(gap(slot));return}
    const b=card(i);if(solo)b.classList.add('solo');el.appendChild(b)})});
  drawThumbs();return}
 /* the day tiles, one block: it pins under the lens bar as a row rather than as four
    separate sticky cells */
 const dhr=document.createElement('div');dhr.className='dhrow';
 days.forEach((d,c)=>{
  const hd=document.createElement('div');hd.className='dh'+(c===0?' tdy':'');
  hd.innerHTML=`<div class="cal"><span class="cd">${d.dow}</span><span class="cn">${d.dn}</span></div><i class="cr"></i>`;
  dhr.appendChild(hd)});
 el.appendChild(dhr);
 /* one block per window, each with its own sticky label: the row names itself for exactly
    as long as its cards are on screen */
 [['am','Sunrise'],['pm','Sunset']].forEach(([slot,lbl])=>{
  const row=document.createElement('div');row.className='hrow '+slot;
  row.innerHTML=`<div class="rail ${slot}"><span>${lbl}</span><i class="rln"></i></div>`;
  const cg=document.createElement('div');cg.className='hcards';
  days.forEach(d=>{const i=d[slot];
   if(i==null){cg.appendChild(gap(slot));return}
   cg.appendChild(card(i))});
  row.appendChild(cg);el.appendChild(row)});
 measureMast();
 drawThumbs();
}
function card(i){
  ...
  /* Alignment is a fact about a NIGHT, which is exactly what the topics line carries — so it
     is a topic chip, not new furniture. It is derived, not declared: the existing King tide
     topic only fires on a spring range, and the commoner and more useful fact is simply
     whether this window's water suits the spots you can reach. It appears only when the coast
     is genuinely live — half the coastal locations in reach or better, never below three. A
     chip on all six cards would be a caption, not a signal. */
  const tdShow=tdShowOf(i),live=tideLive().length;
  const tdTop=tdShow&&TD_EMPH&&i===bestTideWin();
  /* "best of 3" only where there is something to be best of, and only on the best one — a
     rank on every chip in the run would be three claims to read instead of one */
  const tchip=tdShow
   ? `<span class="tw tdw${tdTop?' tdtop':''}" title="${ts.match} of ${ts.coastal} coastal locations in reach get the water they want — ${esc(ts.tw.state)}, ${ts.tw.m.toFixed(1)} m at ${w.time}${live>1?' · '+live+' windows in this run are live':''}${tdTop?' · the most of any of them':''}">${TWAVE}${ts.match} on tide${tdTop&&live>1?'<em class="tdr">best of '+live+'</em>':''}</span>`
   : '';
  const hs=histOf(i);
  ...
  /* the card's own pick, marked when the water suits it: the card is already asking "is the
     place it is sending me going to work", and this is the half of that answer the light
     score cannot give. A miss is SILENT here — the pick is chosen on score, and marking it
     down in the same slot would put two rankings in one line. */
  const bsf=bs?tideFitP(bs,i):null,bsm=bsf&&bsf.tier==='match';
  const bsHtml=bs?`<span class="rt" style="background:${bsc};color:${HeatField.ink(ramp(bs.r[i]))}">${bs.r[i]}★</span><span class="pv2"><span class="st1${bsm?' tdm':''}" title="${esc(bs.n)} · ${esc(RNAME[bs.rid])} · ${fmtDrive(driveOf(bs))} · leave ${leaveBy(w,bs)}${bsm?' · '+esc(BANDW[bsf.want])+' on the light':''}">${bsm?TWAVE:''}${esc(bs.n)}</span></span>`
  ...
  ${bsHtml}
  <span class="tps2">${tchip}${twords}</span></div>${legend}`;
  b.onclick=()=>openWin(i);
...
function drawThumbs(tries){
 ...
 cvs.forEach(cv=>{const i=+cv.dataset.w;
  const cw=Math.max(120,Math.floor(cv.closest('.hc').clientWidth)-12),ch=Math.round(cw*ar);
  const proj=HeatField.drawGeo(cv,cw,ch,sp,i,
   {grid:4,radius:Math.max(10,cw*0.155),blur:2.4,line:0.5,conf:CONF[i],fit,
     bloom:1,bloomFrom:3,bloomA:155,bloomBlur:0.9});
  labelThumb(cv.parentElement.querySelector('.tlab'),proj,cw,ch,i)});
}
...
function spotCards(w,i){
 ...
 const cards=pool.slice(0,8).map(s=>{const c=ramp(s.r[i]);
  return `<button class="spot" data-pick="${esc(s.n)}"><span class="top"><span class="nm">${esc(s.n)}</span><span class="st" style="background:${rgb(c)};color:${HeatField.ink(c)};box-shadow:inset 0 0 0 1px ${rgb(c,.4)}">${s.r[i]}★</span></span>
...
function renderWinSheet(){
 ...
 const l2=mob?`<span>best ${wMax(i)}★ · avg ${wAvg(i).toFixed(1)}★</span><span>◐ ${Math.round(CONF[i]*100)}%</span><span>${dChip(wDelta(i))} in ${RUNAGE}</span>`
  :`${tideStats(i).tw?`<span class="tdl">${TWAVE}${esc(tideStats(i).tw.state)} · ${tideStats(i).match} of ${tideStats(i).coastal} coastal spots on tide</span>`:''}<span>best ${wMax(i)}★ within reach</span><span>average ${wAvg(i).toFixed(1)}★ across ${reachPool(i).length} locations</span><span>◐ ${Math.round(CONF[i]*100)}% confidence</span><span>${dChip(wDelta(i))} since ${RUNAGE} ago</span>`;
...
function drawBig(tries){
 ...
 const proj=HeatField.drawGeo(cv,bw,bh,scopeSpots(),i,{grid:6,radius:Math.max(20,bw*0.072),blur:3.6,line:0.85,
  focus,conf:CONF[i],fit:fitOf(),bloom:1,bloomFrom:3,bloomA:170,bloomBlur:2});
 if(!proj)return;
 ...
 /* home, and the reach rings around it: the popup is where you decide whether to drive, so
    the field says which warmth is tonight and which is a day out. Drawn from real distance
    — 40 km and 80 km — not from the filter's opinion of it. */
 if(isHome(S.origin)){
  const hp=proj(HOMEPT),kps=kmPx(proj),svg=document.createElementNS(SVGNS,'svg');
  svg.setAttribute('class','rings');
  [[40,'45 min'],[80,'1h 30min']].forEach(([km,lbl])=>{
   const r=km*kps;if(r<18||r>Math.max(bw,bh)*1.15)return;
   const c=document.createElementNS(SVGNS,'circle');
   c.setAttribute('cx',hp[0]);c.setAttribute('cy',hp[1]);c.setAttribute('r',r);
   const el=mkLab('ringlb',lbl);
   placeLabels(gl,[{el,x:hp[0],y:hp[1]-r}],bw,bh,boxes);
   svg.appendChild(c)});
  if(svg.childNodes.length)gl.insertBefore(svg,gl.firstChild);
  placeLabels(gl,[{el:mkLab('hm','<i class="mk"></i><span class="lb">HOME</span>'),x:hp[0],y:hp[1]}],bw,bh,boxes);
 }
 ...
 for(const s of cand){
  ...
  el.innerHTML=`<i style="background:${rgb(c2)}"></i><b>${esc(s.n)}</b><em>${s.r[i]}\u2605</em>`;
...
function renderSpot(){
 ...
 const evs=sp.r.map((v,i)=>{
  ...
  <div><div class="ttl"><span class="w">${/sunrise/i.test(w.when)?'Sunrise':'Sunset'}</span><span class="t2">${w.time}</span>${i===bi?'<span class="tag">◎ best</span>':''}
   <span class="st" style="background:${rgb(c)};color:${HeatField.ink(c)};box-shadow:inset 0 0 0 1px ${rgb(c,.4)}">${v}★</span><span class="car">${op?'▲':'▾'}</span></div>
...
function measureMast(){const m=document.getElementById('mast'),l=document.getElementById('lens'),s=document.getElementById('scroll');
 s.style.setProperty('--mastH',m.offsetHeight+'px');
 /* the day tiles pin under the lens bar, so the columns keep their headings while the cards scroll */
 s.style.setProperty('--lensH',l.offsetHeight+'px');
 /* and each row's label pins under the day tiles, so it needs their height too */
 const dh=document.querySelector('.dhrow');
 if(dh)s.style.setProperty('--dhH',dh.offsetHeight+'px')}
const bt=document.getElementById('bT');
if(bt)bt.onclick=()=>{TD_EMPH=!TD_EMPH;bt.classList.toggle('on',TD_EMPH);render()};
