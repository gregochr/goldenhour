...
public class ComingUpConditionsBuilder {
    ...
    /**
     * The peak gate's own name (plan D5) — a third alignment window beside the existing
     * tide-in-light (60 min) and surge-peak-in-light (90 min) constants, scoped to standing-
     * condition peaks only. Satisfied by construction at first ship: every survivor row is keyed to
     * {@link TargetType#SUNRISE} or {@link TargetType#SUNSET}, both light windows by definition, so
     * the gate only ever excludes a hypothetical {@link TargetType#HOURLY} candidate — stated in
     * code (not just prose) so the rule exists before sub-daily intensities make it load-bearing.
     */
    static final int PEAK_LIGHT_WINDOW_MINUTES = 90;
    ...
    private ComingUpCondition buildCoastalTides(LocalDate builtFor, List<AlmanacEvent> allEvents,
        ...
        for (AlmanacEvent event : tideEvents) {
            ...
            ComingUpConditionOccurrence occurrence =
                    tideOccurrence(event, score, builtFor, lastPlanDate, entriesById, entries);
        ...
        return new ComingUpCondition("COASTAL_TIDES", "Coastal tides", "deterministic", anyColdStart,
    ...
    private ComingUpConditionOccurrence tideOccurrence(AlmanacEvent event, TideRunScore score,
            LocalDate builtFor, LocalDate lastPlanDate, Map<String, ComingUpEntry> entriesById,
            List<ComingUpEntry> entries) {
        ...
        if (matchedEntry != null && matchedEntry.bits() != null) {
            ...
            if (!matchedEntry.coincidence().isEmpty() && displayBits > score.bits() + 0.05) {
                // matchedEntry is the WINNER of the merge (its coincidence line names the LOSER,
                // which here is this very tide run) — so the winner to name in the reason text is
                // matchedEntry itself, never the coincidence line.
                reason = "max w/ " + matchedEntry.title().toLowerCase(Locale.UK);
            }
        }
        ...
        } else if (!event.startDate().isBefore(builtFor) && !event.endDate().isAfter(lastPlanDate)) {
        ...
        return new ComingUpConditionOccurrence(representativeDate, DATE_LABEL.format(representativeDate),
                valueLabel, round1(displayBits), reason, status, entryId);
    }
     ...
     * this run's own family ({@code "coastal"}) — the same family {@code ComingUpAssembler} always
     * assigns a tide run before it can ever lose a merge.
     */
    ...
    private ComingUpCondition buildDust(LocalDate builtFor) {
        ...
        for (Burst burst : bursts) {
            ...
            occurrences.add(new ComingUpConditionOccurrence(burst.peakDate(), DATE_LABEL.format(burst.peakDate()),
                    "AOD " + fmt2(burst.peakValue()), round1(bits), null, STATUS_HELD_BACK, null));
        }
        ...
        if (forwardPeak != null) {
            ...
            occurrences.add(new ComingUpConditionOccurrence(forwardPeak.date(), DATE_LABEL.format(forwardPeak.date()),
                    "AOD " + fmt2(aod), round1(bits), null, STATUS_INSIDE_PLAN, null));
        }
        ...
        return new ComingUpCondition("DUST", "Saharan dust", "recurrent", true, rateLabel, quantLabel,
                peak, occurrences);
    }
    ...
    private ComingUpCondition buildInversion(LocalDate builtFor) {
        ...
        for (LocalDate date : dates) {
            ...
            occurrences.add(new ComingUpConditionOccurrence(date, DATE_LABEL.format(date), score + "/10",
                    round1(bits), null, STATUS_HELD_BACK, null));
        }
        ...
        if (forwardPeak != null) {
            ...
            occurrences.add(new ComingUpConditionOccurrence(forwardPeak.date(), DATE_LABEL.format(forwardPeak.date()),
                    score + "/10", round1(bits), null, STATUS_INSIDE_PLAN, null));
        }

        String quantLabel = "rarity " + fmt1(rarityBits) + " · promoted above "
                + fmt0(inversionConfig.getMagnitudeThresholdScore()) + "/10 (interim)";
        return new ComingUpCondition("VALLEY_INVERSIONS", "Valley inversions", "persistent", true,
                rateLabel, quantLabel, peak, occurrences);
    }
    ...
    // ── The peak gate (plan D5) ──────────────────────────────────────────

    /**
     * Whether a candidate lands within {@value #PEAK_LIGHT_WINDOW_MINUTES} minutes of a light
     * window — satisfied by construction for every survivor row today, since both are keyed to
     * {@link TargetType#SUNRISE} or {@link TargetType#SUNSET}. Written as a real predicate, not
     * assumed true, so the rule exists in code before sub-daily intensities (P7's
     * {@code landed_on_window}) make it load-bearing.
     */
    static boolean passesPeakGate(TargetType eventType) {
        return eventType == TargetType.SUNRISE || eventType == TargetType.SUNSET;
    }
