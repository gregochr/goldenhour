 ...
 *
 * <p>Feature clusters (tide, directional cloud, cloud approach, storm surge, inversion,
 * triage) live in {@link Embedded} value objects mapping to the same columns — Hibernate
 * materialises a cluster whose columns are all null as a {@code null} embeddable, so readers
 * go through each type's {@code orEmpty} view.
 */
@Entity
...
public class ForecastEvaluationEntity {
    ...
    /** Tide context at the solar event, or null for inland locations. */
    @Embedded
    private TideDetails tide;

    /** Directional cloud sampling (solar/antisolar/far-field), or null if unavailable. */
    @Embedded
    private DirectionalCloudDetails directionalCloud;
    ...
    /** Cloud approach risk (solar-horizon trend + upwind sample), or null if unavailable. */
    @Embedded
    private CloudApproachDetails cloudApproach;

    /** Storm surge components and adjusted range, or null for inland locations. */
    @Embedded
    private StormSurgeDetails surge;

    /** Cloud inversion likelihood, or null for ineligible locations. */
    @Embedded
    private InversionDetails inversion;

    /** Triage stand-down details, or null if the row was scored by Claude. */
    @Embedded
