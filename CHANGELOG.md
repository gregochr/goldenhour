...
- **Result.** St Mary's went from 6/8 (misses to 2) → 5/8 (misses to 3) → **8/8 at `{4}`**. Unit tests green (prompt content + fixtures); behavioural fix confirmed by re-running the fixture. A full-suite `SkyRatingEvalTest` run guards against the edits regressing other fixtures (the overfitting check on a single-fixture finding).
