...
- **Sighting endpoint + NLCNET scraper (backend).** `GET /api/nlc/sighting` (ADMIN/PRO; LITE → 403). `NlcSightingClient` scrapes the NLCNET sightings page with Jsoup (verified `div.caption` "Observer from Location on YYYY, MM, DD from HH:MM UT" format), cached and **fail-open** — any fetch/parse error leaves the banner dark. `NlcSightingService` gates on **season + freshness (≤6 h) + confirmed clear skies tonight** (reusing the `NlcClarityService` scan), so the banner only claims "clear skies" when it can. Scheduled `nlc_sighting_scrape` job (V120) via `DynamicSchedulerService`, self-gating on `nlc.sighting-enabled`.
