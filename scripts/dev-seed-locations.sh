...
⚠️ The block below was WRONG until 2026-08-20 and cost a session: it named a `created_at` column
that has never existed, omitted four NOT NULL columns, and keyed on `date|targetType`. Every
statement failed with "Column CREATED_AT not found".

⚠️ And check the schema against the ENTITY, not the migration. `application-local.yml` sets
`flyway.enabled: false` and `ddl-auto: update`, so the local H2 is generated by Hibernate from
`CachedEvaluationEntity` — V91's `DEFAULT CURRENT_TIMESTAMP` on `evaluated_at`/`updated_at` is a
Postgres-only fact and does NOT exist locally. Both are `nullable = false` with no default here, so
omitting them fails with "NULL not allowed for column EVALUATED_AT". Supply all eight columns; that
is correct on both engines.

Four facts the shape turns on:

  1. THE KEY IS PER REGION, not per date. `cache_key` is `regionName|date|targetType`
     (`CacheKeyFactory.build`), UNIQUE, so a day's ratings are ONE ROW PER REGION PER EVENT — four
     regions x two events x five days = 40 rows for the catalogue this script seeds. `region_name`,
     `evaluation_date` and `target_type` repeat the key's three parts as real columns and are all
     NOT NULL with no default, so all four must be supplied together.
  2. EVERY NAME IN `results_json` MUST BELONG TO THAT REGION. Enrichment looks a location up by
     name *within its own region's* entry, so a name in the wrong region's row joins to nothing and
     the cell renders unscored, with nothing on screen saying why. (Same trap in `--dense` mode:
     the "Scatter N" locations are real rows in the roster and need ratings of their own, or the
     field is mostly holes.)
  3. TODAY OR FUTURE DATES ONLY. `rehydrateCacheOnStartup` loads
     `findByEvaluationDateGreaterThanEqual(today)` on Europe/London, so a past date is inserted,
     ignored, and looks like a broken join.

  4. SUPPLY `evaluated_at` AND `updated_at`. See the warning above — locally they have no default.

`source` is 'BATCH' (the only value any writer uses). `results_json` is a JSON list of
`BriefingEvaluationResult` — `locationName`, `rating` (1-5 or null), `fierySkyPotential`,
`goldenHourPotential`, `summary`, and optionally `headline`; a rating outside 1-5 is clamped to null
at rehydration and logged as such.

This statement is verbatim what was run and accepted on 2026-08-20 — do not retype it from memory:
  ...
  VALUES (
      'Northumberland & Tyneside|2026-08-20|SUNSET',
      'Northumberland & Tyneside',
      DATE '2026-08-20',
      'SUNSET',
      '[{"locationName":"Bamburgh Beach","rating":5,"fierySkyPotential":78,
         "goldenHourPotential":71,"summary":"Broken cloud clearing west."},
        {"locationName":"Sycamore Gap","rating":3,"fierySkyPotential":52,
         "goldenHourPotential":48,"summary":"High cirrus only; pale colour, long light."}]',
      'BATCH', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);

Repeat per region x date x event. Forty hand-written rows is not a good use of an afternoon —
generate them (a dozen lines of Python emitting this statement in a loop over the REGIONS and ROWS
tables above is what the P8 session did) and feed the file to RunScript.

⚠️ THE RUNNING BACKEND HOLDS THE H2 FILE LOCK. The web console above works while it runs, because
it shares the app's own connection; an external tool does not, and fails with an MVStore
"file is locked" trace. So an external insert is stop -> insert -> start:
...
⚠️ RESTART THE BACKEND after inserting either way, then POST /api/briefing/run. The briefing
enrichment reads an in-memory ConcurrentHashMap populated from the DB only by
`rehydrateCacheOnStartup` (@EventListener(ApplicationReadyEvent)), so rows inserted while it is
running are invisible to it — which is why the console's shared connection buys you nothing here.
Startup logs `[EVAL HYDRATE] Loaded N entries`; if N is 0 your dates are in the past.
...
Drive times need OpenRouteService, which is not configured locally, so `POST
/api/user/settings/drive-times/refresh` returns distances with no minutes and every leave-by line
is correctly absent. To exercise them, insert during the same stopped window:
`user_drive_time (user_id, location_id, drive_duration_seconds)` for the home arm and
`region_drive_time (region_id, location_id, drive_duration_seconds)` for an away origin.
NOTE
